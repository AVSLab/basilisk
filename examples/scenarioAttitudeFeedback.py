...
The message connection figure generated by the nominal scenario setup shows the configured module message graph,
including recorder modules and the stand-alone vehicle configuration message supplied through ``extraMessages``.

.. image:: /_images/Scenarios/scenarioAttitudeFeedbackMessageConnectionsGraphviz0000.svg
   :align: center
...
def run(show_plots, useUnmodeledTorque, useIntGain, useKnownTorque, useCMsg):
    ...
    if useCMsg:
        # create stand-alone commanded torque msg and setup recorder()
    ...
    # the simulation execution or logged state data.  Stand-alone messages are
    # supplied through extraMessages so their links appear in the figure.
    extraMessages = [{"vehicleConfigMsg": configDataMsg}]
    if useCMsg:
        extraMessages.append({"cmdTorqueMsg": cmdTorqueMsg})
    pltName = (
        fileName + "MessageConnectionsGraphviz"
