...
the residual is not bookkeeping-limited at the integrator's RK4 order.
``test_effectorBranching_integrated`` instead accumulates its reference with the
trapezoidal rule for angular momentum and a running rectangle sum for the
accumulated delta-V, so its residual reports the quadrature error rather than
the simulator's.
