...
def run(show_plots, scCase):
    ...
    gravBodies["earth"].useSphericalHarmonicsGravityModel(str(ggm03s_path), 100)
    ...
    if scCase == "NewHorizons":
        scEphemerisPath = get_path(DataFile.EphemerisData.nh_pred_od077)
        scSpiceName = "NEW HORIZONS"
    else:  # default case
        scSpiceName = "HUBBLE SPACE TELESCOPE"
        scEphemerisPath = get_path(DataFile.EphemerisData.hst_edited)

    pyswice.furnsh_c(str(scEphemerisPath))  # Hubble Space Telescope data
    ...
    pyswice.unload_c(str(scEphemerisPath))
    pyswice.unload_c(str(de430_path))  # solar system bodies
    pyswice.unload_c(str(naif0012_path))  # leap second file
    pyswice.unload_c(str(de403masses_path))  # solar system masses
    pyswice.unload_c(str(pck00010_path))  # generic Planetary Constants Kernel
