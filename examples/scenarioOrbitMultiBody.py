...
The message connection figure generated by the Hubble scenario setup shows the configured module message graph,
including the stand-alone epoch message supplied through ``extraMessages``.

.. image:: /_images/Scenarios/scenarioOrbitMultiBodyMessageConnectionsGraphvizHubble.svg
   :align: center
...
def run(show_plots, scCase):
    ...
    # the simulation execution or logged state data.  The gravity factory creates
    # a stand-alone epoch message, so extraMessages is used to include it.
    pltName = fileName + "MessageConnectionsGraphviz" + scCase
    messageFigure = scSim.ShowMessageConnectionFigure(
        show_plots=show_plots,
        extraMessages={"epochMsg": gravFactory.epochMsg},
