...
This scenario demonstrates the capabilities of :ref:`bskPrinciples-11` in simulating dynamic
effectors attached directly to state effectors rather than the central rigid hub of a spacecraft.
A :ref:`constraintDynamicEffector` modeling a grappling device is attached at the end of
:ref:`linearTranslationOneDOFStateEffector` modeling a 1 degree-of-freedom (DOF) telescoping robotic arm. This arm then
grapples onto a target spacecraft, coupling the two vehicles together. Finally, a berthing maneuver
is performed by the chaser spacecraft, pulling the target closer to it before locking the effector
into a hard-docked configuration.

What makes this example particularly powerful is that the simulation is continuous. It is able to
seemlessly transition from proximity operations all the way to a post-docked configuration without
needing to be paused to switch to a single spacecraft model or dynamic solvers.

Note that the :ref:`constraintDynamicEffector` requires attention to gain tuning, which is
demonstrated in :ref:`scenarioConstrainedDynamicsManeuverAnalysis`.

The script is found in the folder ``basilisk/examples`` and executed by using::

    python3 scenarioRoboticGrappling.py

The scenario outputs plots.
...
#
#   Purpose:            Illustrates robotic grappling with effector branching
...
from Basilisk.simulation import (spacecraft, constraintDynamicEffector, gravityEffector, svIntegrators, extForceTorque, simpleNav, linearTranslationOneDOFStateEffector)
...
def set_up_vizard(scSim):
    ...
    vizSupport.createCustomModel(viz,
                                 simBodiesToModify=[scSim.scObject2.ModelTag],
                                 modelPath='bskSat',
