...
def plot_orientation(
    timeLineSet, vectorPosData, vectorVelData, vectorMRPData, id=None, livePlot=False
):
...
def plot_rw_cmd_actual_torque(
    timeData, dataUsReq, dataRW, numRW, id=None, livePlot=False
):
...
def plot_rw_friction(
    timeData, dataFrictionRW, numRW, dataFaultLog=[], id=None, livePlot=False
):
    plt.figure(id)
    for idx in range(numRW):
        plt.plot(
    ...
    if dataFaultLog:
        ...
        plt.scatter(
            [row[3] for row in dataFaultLog],
            np.zeros(len(dataFaultLog)),
            marker="x",
            color=(1, 0, 0),
            label="Faults",
        )
...
def plot_surface_rel_velocity(timeData, r_BN_N, v_BN_N, sigma_PN, omega_PN_P, id=None):
    ...
    for idx in range(0, len(r_BN_N)):
        ...
        s1Hat_N = np.cross(omega_PN_N, r_BN_N[idx]) / np.linalg.norm(
            np.cross(omega_PN_N, r_BN_N[idx])
        )
        s3Hat_N = r_BN_N[idx] / np.linalg.norm(r_BN_N[idx])
        s2Hat_N = np.cross(s3Hat_N, s1Hat_N) / np.linalg.norm(
            np.cross(s3Hat_N, s1Hat_N)
        )
        dcmSN = np.array(
            [s1Hat_N.transpose(), s2Hat_N.transpose(), s3Hat_N.transpose()]
        )
