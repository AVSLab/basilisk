...
class BSK_GpsSatDynamics:
    ...
    def SetSpacecraftHub(self):
        ...
        self.I_sc = [900., 0., 0., #TODO update according to Nano Avionics M16P
                     0., 800., 0.,
                     0., 0., 600.]
        self.scObject.hub.mHub = 36.0  # kg - spacecraft mass | Based on NanoAvionics M16P max spacecraft mass
...
#        SimBase.EnvModel.groundStationTron.addSpacecraftToModel(self.scObject.scStateOutMsg)
    ...
    def SetReactionWheelDynEffector(self):
        ...
        maxRWMomentum = 50.  # Nms # TODO REMOVE update according to Nano Avionics M16P

        # Define orthogonal RW pyramid
        # -- Pointing directions
        rwElAngle = np.array([40.0, 40.0, 40.0, 40.0]) * mc.D2R # TODO update according to Nano Avionics M16P
        rwAzimuthAngle = np.array([45.0, 135.0, 225.0, 315.0]) * mc.D2R # TODO update according to Nano Avionics M16P
        rwPosVector = [[0.8, 0.8, 1.79070], # TODO update according to Nano Avionics M16P
                       [0.8, -0.8, 1.79070],
                       [-0.8, -0.8, 1.79070],
                       [-0.8, 0.8, 1.79070]]
...
#                                  rWB_B=posVector)
    ...
    def SetThrusterDynEffector(self):
        ...
        for pos_B, dir_B in zip(location, direction):
            self.thrusterFactory.create('MOOG_Monarc_90HT', pos_B, dir_B, useMinPulseTime=False)  #TODO update to match Nano Avionics M16P -> ENPULSION IFM Nano thruster
    ...
    def SetFuelTank(self):
        ...
        """
        # Define the tank
        self.tankModel = fuelTank.FuelTankModelUniformBurn() # TODO update to match NanoAvionics M16P -> check if this is the right tank model
    ...
    def SetReactionWheelPower(self):
        ...
        for item in range(self.numRW):
            ...
            self.rwPowerList[item].mechToElecEfficiency = 0.5 # TODO What is this? update according to Nano Avionics M16P
