...
including the stand-alone vehicle and RW configuration messages supplied through ``extraMessages``.

.. image:: /_images/Scenarios/scenarioAttitudeFeedbackRWMessageConnectionsGraphviz00.svg
   :align: center
...
def run(show_plots, useJitterSimple, useRWVoltageIO):
    ...
    # supplied through extraMessages so their links appear in the figure.  The
    # recorder modules are hidden so this complex scenario focuses on module flow.
    extraMessages = [
        {"vehicleConfigMsg": vcMsg},
        {"rwConfigMsg": fswRwParamMsg},
    ]
    pltName = (
        fileName + "MessageConnectionsGraphviz"
        + str(int(useJitterSimple))
        + str(int(useRWVoltageIO))
    )
