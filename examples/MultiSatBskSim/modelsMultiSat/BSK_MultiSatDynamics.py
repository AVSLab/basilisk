...
class BSKDynamicModels:
    ...
    def SetSimpleMassPropsObject(self):
        ...
        Defines the navigation module.
        """
        self.simpleMassPropsObject.ModelTag = "SimpleMassProperties" + str(
            self.spacecraftIndex
        )
        self.simpleMassPropsObject.scMassPropsInMsg.subscribeTo(
            self.scObject.scMassOutMsg
        )
    ...
    def SetReactionWheelDynEffector(self):
        ...
        self.rwFactory.addToSpacecraft(
            "RWArray" + str(self.spacecraftIndex), self.rwStateEffector, self.scObject
        )
    ...
    def SetThrusterDynEffector(self):
        ...
        for pos_B, dir_B in zip(location, direction):
            self.thrusterFactory.create(
                "MOOG_Monarc_90HT", pos_B, dir_B, useMinPulseTime=False
            )

        self.numThr = self.thrusterFactory.getNumOfDevices()

        # create thruster object container and tie to spacecraft object
        self.thrusterFactory.addToSpacecraft(
            "thrusterFactory", self.thrusterDynamicEffector, self.scObject
        )
    ...
    def SetReactionWheelPower(self):
        ...
        for item in range(self.numRW):
            self.rwPowerList[item].ModelTag = (
                self.scObject.ModelTag + "RWPower" + str(item)
            )
            self.rwPowerList[item].basePowerNeed = 5.0  # baseline power draw, Watt
            self.rwPowerList[item].rwStateInMsg.subscribeTo(
                self.rwStateEffector.rwOutMsgs[item]
            )
    ...
    def SetSolarPanel(self, SimBase):
        ...
        self.solarPanel.sunEclipseInMsg.subscribeTo(
            SimBase.EnvModel.eclipseObject.eclipseOutMsgs[0]
        )  # choose the earth message
        ...
        self.solarPanel.setPanelParameters(
            self.solarPanelAxis,  # panel normal vector in the body frame
    ...
    def SetPowerSink(self):
        ...
        self.powerSink.nodePowerOut = -2.0  # Watt

    def SetBattery(self):
        """Sets up the battery with all the power components"""
        self.powerMonitor.ModelTag = "powerMonitor"
        self.powerMonitor.storageCapacity = (
            2 * 60.0 * 3600.0
        )  # Convert from W-hr to Joule
        self.powerMonitor.storedCharge_Init = (
            self.powerMonitor.storageCapacity * 0.6
        )  # 40% depletion
        ...
        for item in range(self.numRW):
            self.powerMonitor.addPowerNodeToModel(
                self.rwPowerList[item].nodePowerOutMsg
            )
