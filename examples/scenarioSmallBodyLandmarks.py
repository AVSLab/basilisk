...
def plot_3D(t, r, xyz_vert, order_face, posLmk, isvisibleLmk, dcm_CP):
    ...
    ax.plot(
        posLmk[idxOff, 0] / 1000,
        posLmk[idxOff, 1] / 1000,
        posLmk[idxOff, 2] / 1000,
        "r",
        linestyle="",
        marker=".",
        markersize=5,
...
def plot_pixel(pixelLmk, statusLmk):
    ...
    ax = fig.gca()
    plt.plot(
        pixelOn[:, 0], pixelOn[:, 1], linestyle="", marker="s", color="b", markersize=5
    )
...
def plot_orientation(t, dcm_HP, dcm_CP):
    ...
    for i in range(len(t)):
        data[i, 0:3] = [
            np.dot(dcm_HP[i, 0:3, 0], dcm_CP[i, 0:3, 2]),
...
def run(show_plots, useBatch):
    ...
    poly_path = get_path(DataFile.LocalGravData.eros007790)
    vert_list, face_list, n_vert, n_face = loadPolyFromFileToList(str(poly_path))
    n_lmk = 100
    pos_lmk, normal_lmk = landmark_distribution(
        vert_list, face_list, n_vert, n_face, n_lmk
    )
    ...
    dcm_CB = np.array([[0, 0, -1], [0, 1, 0], [1, 0, 0]])
    ...
    if useBatch:
        ...
        camera.processBatch(
            r_BP_P, mrp_BP, -r_PN_P / np.linalg.norm(r_PN_P, axis=1)[:, None], False
        )
        ...
        batch_diff = np.array(
            [
                np.linalg.norm(nvisibleBatchLmk - nvisibleLmk)
                / np.linalg.norm(nvisibleLmk),
                np.linalg.norm(isvisibleBatchLmk - isvisibleLmk)
                / np.linalg.norm(isvisibleLmk),
    ...
    plot_3D(
        t0, r0, np.array(vert_list), np.array(face_list), pos_lmk, isvisibleLmk0, dcm0
    )
    pltName = fileName + "1"
    figureList[pltName] = plt.figure(1)

    plot_3D(
        tf, rf, np.array(vert_list), np.array(face_list), pos_lmk, isvisibleLmkf, dcmf
    )
    pltName = fileName + "2"
