...
JOINT_START_END = [(np.pi / 2, 0), (np.pi, 0), (np.pi, 0)]
MAX_PROFILE_VEL = np.deg2rad(0.05)
MAX_PROFILE_ACCEL = np.deg2rad(0.0001)
...
def generateProfiles(
    ...
    for values in (position, velocity):
        ...
        # inteprolation through the generated profile data points.
...
def run(initialSpin: bool = False, showPlots: bool = False, visualize: bool = False):
    ...
    if initialSpin:
        scene.getBody("hub").setAttitudeRate([0, 0.8, 0])
