...
def run(show_plots):
    ...
    if POST:
        ...
        position_N = unitTestSupport.addTimeColumn(
            timeAxis, monteCarloData["messages"][retainedMessageName1 + "." + var1]
        )
        sigma_BN = unitTestSupport.addTimeColumn(
            timeAxis, monteCarloData["messages"][retainedMessageName1 + "." + var2]
        )
        validCircle = unitTestSupport.addTimeColumn(
            timeAxis, monteCarloData["messages"][retainedMessageName2 + "." + var3]
        )

        renderRate = 60 * 1e9
        sigma_CB = [0.0, 0.0, 0.0]  # Arbitrary camera orientation
        sizeOfCam = [512, 512]
        sizeMM = [10.0 * 1e-3, 10.0 * 1e-3]  # in m
        fieldOfView = np.deg2rad(55)  # in degrees
        focal = sizeMM[0] / 2.0 / np.tan(fieldOfView / 2.0)  # in m
        ...
        for i in range(len(validCircle[:, 0])):
            if validCircle[i, 1] > 1e-5 or (
                validCircle[i, 0] % renderRate == 0 and validCircle[i, 0] > 1
            ):
                ...
                writer.writerow(
                    [
                        str("{0:.6f}".format(position_N[i, 0] * 1e-9)) + ".jpg",
                        validCircle[i, 1],
                        trueCircles[i, 1],
                        trueCircles[i, 2],
                        trueCircles[i, 3],
