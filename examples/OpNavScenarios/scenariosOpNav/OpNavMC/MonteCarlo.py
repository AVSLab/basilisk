...
def displayPlots(data, retentionPolicy):
    ...
    plt.figure(102, figsize=(3.5, 2.0), facecolor="w", edgecolor="k")
    plt.plot(diffVel[:, 0] * ns2min, diffVel[:, 1])
    plt.ylabel("$\dot{\mathbf{r}}_\mathrm{" + "Circ" + "}$ errors ($\%$)")
    plt.xlabel("Time (min)")
    plt.savefig("MCErrorVel.pdf")

    plt.figure(104, figsize=(3.5, 2.0), facecolor="w", edgecolor="k")
    ...
    if RUN:
        ...
        monteCarlo.addDispersion(
            UniformDispersion(
                dispFOV,
                [
                    np.deg2rad(40) - np.deg2rad(0.001),
                    np.deg2rad(40) + np.deg2rad(0.001),
                ],
            )
        )
        monteCarlo.addDispersion(
            OrbitalElementDispersion(disp1Name, disp2Name, dispDict)
        )

        # Add retention policy
        retentionPolicy = RetentionPolicy()
        retentionPolicy.addMessageLog(
            retainedMessageNameSc, ["r_BN_N", "v_BN_N", "sigma_BN"]
        )
        retentionPolicy.addMessageLog(
            retainedMessageNameOpNav,
            ["r_BN_N", "covar_N", "r_BN_C", "covar_C", "valid"],
        )
