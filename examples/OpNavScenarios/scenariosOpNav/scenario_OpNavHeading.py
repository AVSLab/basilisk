...
def DCM(bVec, d):
    ...
    if np.linalg.norm(np.cross(bVec, d)) < 1e-5:
        ...
        DCM_exp[:, 1] = np.cross(DCM_exp[:, 0], bVec) / np.linalg.norm(
            np.array(np.cross(DCM_exp[:, 0], bVec))
        )
...
class scenario_OpNav(BSKScenario):
    ...
    def configure_initial_conditions(self):
        ...
        if self.filterUse == "relOD":
            self.masterSim.get_FswModel().relativeOD.stateInit = (
                rN.tolist() + vN.tolist()
            )
        if self.filterUse == "bias":
            self.masterSim.get_FswModel().pixelLineFilter.stateInit = (
        ...
        # self.masterSim.get_FswModel().opNavPoint.opnavDataInMsgName = "heading_filtered"
        self.masterSim.get_FswModel().imageProcessing.noiseSF = 0.5
        self.masterSim.get_FswModel().headingUKF.noiseSF = 1.001
        self.masterSim.get_FswModel().opNavPoint.opnavDataInMsg.subscribeTo(
            self.masterSim.get_FswModel().headingUKF.opnavDataOutMsg
        )
    ...
    def log_outputs(self):
        ...
        self.rwMotorRec = FswModel.rwMotorTorque.rwMotorTorqueOutMsg.recorder(
            samplingTime
        )
    ...
    def pull_outputs(self, showPlots):
        ...
        Outomega_BN = unitTestSupport.addTimeColumn(
            self.scRec.times(), self.scRec.omega_BN_B
        )

        ## Image processing
        circleCenters = unitTestSupport.addTimeColumn(
            self.circlesRec.times(), self.circlesRec.circlesCenters
        )
        circleRadii = unitTestSupport.addTimeColumn(
            self.circlesRec.times(), self.circlesRec.circlesRadii
        )
        validCircle = unitTestSupport.addTimeColumn(
            self.circlesRec.times(), self.circlesRec.valid
        )

        frame = unitTestSupport.addTimeColumn(
            self.headingBVecLog.times(), self.headingBVecLog.bVec_B
        )
        ...
        for i in range(numRW):
            dataRW.append(
                unitTestSupport.addTimeColumn(
                    self.rwMotorRec.times(), self.rwLogs[i].u_current
                )
            )
        ...
        covar_B = unitTestSupport.addTimeColumn(
            self.opNavRec.times(), self.opNavRec.covar_B
        )

        FilterType = "Switch-SRuKF"
        numStates = 5
        # Get the filter outputs through the messages
        stateLog = unitTestSupport.addTimeColumn(
            self.filtRec.times(), self.filtRec.state
        )
        r_BN_C = unitTestSupport.addTimeColumn(
            self.opNavFiltRec.times(), self.opNavFiltRec.r_BN_C
        )
        postFitLog = unitTestSupport.addTimeColumn(
            self.filtRec.times(), self.filtRec.postFitRes
        )
        covarLog = unitTestSupport.addTimeColumn(
        ...
        for i in range(len(stateLog[:, 0])):
            ...
            filterOmega_BN[i, 1:] = np.dot(
                DCM_BS, np.array([0.0, stateLog[i, 4], stateLog[i, 5]])
            )
            ...
            covarLog_B[i, -4:] = np.dot(
                np.dot(DCM_BS, tempCovar), np.transpose(DCM_BS)
            )[1:, 1:].flatten()
        ...
        for i in range(len(errorDeg[:, 0])):
            ...
            errorDeg[i, 1] = np.arccos(
                np.dot(stateLogSEKF[i, 1:4], expectedSEKF[i, 1:4])
            )
            rateError[i, 1] = np.linalg.norm(errorVsTruthSEKF[i, 4:])

            covarVec = np.array(
                [
                    stateLog[i, 1] + np.sqrt(covarLog[i, 1]),
                    stateLog[i, 2] + np.sqrt(covarLog[i, 2 + numStates]),
        ...
        show_plots = True
        m2km = 1e-3
        covar_B[:, 1:] *= 1.0 / (self.semiMajAxis**2)
        covarOmega[:, 1, 1] *= 3
        r_NB_hat_C = np.copy(r_BN_C)
        r_NB_hat_C[0, 3] = 1.0  # adjust first state to have a non-zero norm
        for i in range(r_NB_hat_C.shape[0]):
            r_NB_hat_C[i, 1:] *= -1.0 / np.linalg.norm(r_NB_hat_C[i, 1:])
            trueRhat_C[i, 1:] *= 1.0 / np.linalg.norm(trueRhat_C[i, 1:])
        ...
        BSK_plt.vecTrack(
            trueRhat_C[modeSwitch:, :],
            r_NB_hat_C[modeSwitch:, :],
            covarC[modeSwitch:, :],
        )
