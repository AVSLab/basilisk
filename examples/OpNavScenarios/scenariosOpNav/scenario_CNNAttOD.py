...
class scenario_OpNav(BSKScenario):
    ...
    def configure_initial_conditions(self):
        ...
        if self.filterUse == "relOD":
            self.masterSim.get_FswModel().relativeOD.stateInit = (
                rN + rError
            ).tolist() + (vN + vError).tolist()
        if self.filterUse == "bias":
            self.masterSim.get_FswModel().relativeOD.stateInit = (
                (rN + rError).tolist() + (vN + vError).tolist() + bias
            )
        ...
        self.masterSim.get_FswModel().relativeOD.noiseSF = 5  # 7.5
    ...
    def log_outputs(self):
        ...
        if self.filterUse == "bias":
            self.filtRec = FswModel.pixelLineFilter.filtDataOutMsg.recorder(
                samplingTime
            )
            self.masterSim.AddModelToTask(DynModel.taskName, self.filtRec)
    ...
    def pull_outputs(self, showPlots):
        ...
        if self.filterUse == "bias":
            ...
            navPostFits = unitTestSupport.addTimeColumn(
        ...
        if self.filterUse == "relOD":
            ...
            r_C = unitTestSupport.addTimeColumn(
                self.opNavRec.times(), self.opNavRec.r_BN_C
            )
        ...
        for i in range(len(circleCenters[:, 0])):
            if circleCenters[i, 1:].any() > 1e-8 or circleCenters[i, 1:].any() < -1e-8:
                ...
                trueCircles[i, 3] = (
                    focal
                    * np.tan(np.arcsin(Rmars / np.linalg.norm(position_N[i, 1:4])))
                    / pixelSize[0]
                )
                ...
                if self.filterUse == "relOD":
                    ...
                    measError_C[i, 1:4] = trueRhat_C[i, 1:] - r_C[
                        i, 1:4
                    ] / np.linalg.norm(r_C[i, 1:4])
            else:
                if self.filterUse == "relOD":
        ...
        BSK_plt.nav_percentages(
            truth[switchIdx:, :], navState, navCovar, validCircle, "CNN"
        )
        ...
        BSK_plt.imgProcVsExp(
            trueCircles, circleCenters, circleRadii, np.array(sizeOfCam)
        )
...
def run(showPlots, simTime=None):
    ...
    simulationTime = macros.min2nano(5.0)
