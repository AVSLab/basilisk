...
class scenario_OpNav(BSKScenario):
    ...
    def configure_initial_conditions(self):
        ...
        MRP = [0, -0.3, 0]
        self.masterSim.get_FswModel().relativeOD.stateInit = (rN + rError).tolist() + (
            vN + vError
        ).tolist()
    ...
    def pull_outputs(self, showPlots):
        ...
        ## Image processing
        limb = unitTestSupport.addTimeColumn(
            self.limbRec.times(), self.limbRec.limbPoints
        )
        numLimbPoints = unitTestSupport.addTimeColumn(
            self.limbRec.times(), self.limbRec.numLimbPoints
        )
        validLimb = unitTestSupport.addTimeColumn(
            self.limbRec.times(), self.limbRec.valid
        )
        ...
        BSK_plt.nav_percentages(
            truth[switchIdx:, :], navState, navCovar, validLimb, "Limb"
        )
