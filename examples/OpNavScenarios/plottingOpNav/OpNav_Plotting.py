...
def omegaTrack(rError, covar):
    colorsInt = len(mpl.pyplot.get_cmap("inferno").colors) / (10.0)
    colorList = []
    for i in range(10):
    ...
    plt.plot(t, rError[:, 1] * 180.0 / np.pi, color=colorList[2], label="Error")
    plt.plot(
        t,
        3 * np.sqrt(covar[:, 0, 0]) * 180.0 / np.pi,
        color=colorList[8],
        linestyle="--",
    ...
    plt.legend(loc="best")
    plt.ylabel(
        r"$\mathbf{\omega}_{" + str(2) + r"}$ Error in $\mathcal{C}$ ($^\circ$/s)"
    )
    plt.ylim([-0.07, 0.07])
    plt.xlabel("Time (min)")
    ...
    plt.plot(t, rError[:, 2] * 180.0 / np.pi, color=colorList[2])
    plt.plot(
        t,
        3 * np.sqrt(covar[:, 1, 1]) * 180.0 / np.pi,
        color=colorList[8],
        linestyle="--",
    )
    plt.plot(
    ...
    plt.ylabel(
        r"$\mathbf{\omega}_{" + str(3) + r"}$ Error in $\mathcal{C}$ ($^\circ$/s)"
    )
    plt.ylim([-0.07, 0.07])
    plt.xlabel("Time (min)")
...
def vecTrack(ref, track, covar):
    ...
    for i in range(len(errorDeg[:, 0])):
        ...
        errorDeg[i, 1] = np.arccos(np.dot(ref[i, 1:4], track[i, 1:4])) * 180.0 / np.pi
        covDeg[i, 1] = np.arccos(np.dot(ref[i, 1:4], track[i, 1:4]))
        covarVec = np.array(
            [
                track[i, 1] + np.sqrt(covar[i, 0, 0]),
                track[i, 2] + np.sqrt(covar[i, 1, 1]),
                track[i, 3] + np.sqrt(covar[i, 2, 2]),
            ]
        )
        covarVec = covarVec / np.linalg.norm(covarVec)
        covDeg[i, 1] = 3 * np.arccos(np.dot(covarVec, track[i, 1:4])) * 180.0 / np.pi

    t = ref[:, 0] * ns2min
    plt.figure(num=101011, figsize=(2.7, 1.6), facecolor="w", edgecolor="k")
    plt.plot(t, errorDeg[:, 1], color=colorList[1], label="Off-point")
    plt.plot(
        t, covDeg[:, 1], color=colorList[5], linestyle="--", label=r"Covar (3-$\sigma$)"
    )
...
def plot_faults(dataFaults, valid1, valid2):
    ...
    plt.scatter(
        valid1[:, 0] * ns2min, valid1[:, 1], color=colorList[5], alpha=0.1, label="Limb"
    )
    plt.scatter(
        valid2[:, 0] * ns2min, valid2[:, 1], color=colorList[8], alpha=0.1, label="Circ"
    )
    plt.scatter(
        dataFaults[:, 0] * ns2min,
        dataFaults[:, 1],
...
def diff_methods(vec1, meth1, meth2, val1, val2):
    ...
    for i in range(len(validIdx1)):
        ...
        diffNorms1[i, 1] = np.linalg.norm(vec1[validIdx1[i], 1:]) - np.linalg.norm(
            meth1[validIdx1[i], 1:]
        )
    for i in range(len(validIdx2)):
        ...
        diffNorms2[i, 1] = np.linalg.norm(vec1[validIdx2[i], 1:]) - np.linalg.norm(
            meth2[validIdx2[i], 1:]
        )
    ...
    plt.plot(
        ...
        diff1[:, 1] * m2km,
        color=colorList[1],
        label=r"$\mathbf{r}_\mathrm{Limb}$",
    )
    ...
    plt.plot(
        ...
        diff2[:, 1] * m2km,
        color=colorList[1],
        label=r"$\mathbf{r}_\mathrm{Circ}$",
        linestyle="--",
        linewidth=2,
    )
    plt.plot(
...
def diff_vectors(vec1, vec2, valid, string):
    ...
    for i in range(len(validIdx)):
        ...
        diffNorms[i, 1] = np.linalg.norm(vec1[validIdx[i], 1:]) - np.linalg.norm(
            vec2[validIdx[i], 1:]
        )
    # plt.figure(1, figsize=(2.7, 1.6), facecolor='w', edgecolor='k')
    plt.figure(1, figsize=(3.5, 2.0), facecolor="w", edgecolor="k")
    plt.xlabel("Time")
    plt.plot(
        diff[:, 0] * ns2min,
        diff[:, 1] * m2km2,
        color=colorList[1],
        label=r"$x_\mathrm{" + string + "}$",
    )
    plt.plot(
        diff[:, 0] * ns2min,
    ...
    # plt.figure(2, figsize=(2.7, 1.6), facecolor='w', edgecolor='k')
    plt.figure(2, figsize=(3.5, 2.0), facecolor="w", edgecolor="k")
    plt.xlabel("Time")
...
def nav_percentages(truth, states, covar, valid, string):
    ...
    for i in range(len(validIdx)):
        ...
        diffPos[i, 1] = (
            np.linalg.norm(states[validIdx[i], 1:4] - truth[validIdx[i], 1:4])
            / np.linalg.norm(truth[validIdx[i], 1:4])
            * 100
        )
        diffVel[i, 0] = states[validIdx[i], 0]
        diffVel[i, 1] = (
            np.linalg.norm(states[validIdx[i], 4:7] - truth[validIdx[i], 4:7])
            / np.linalg.norm(truth[validIdx[i], 4:7])
            * 100
        )
        ...
        posVec = np.sqrt(
            np.array(
                [
                    covar[validIdx[i], 1],
                    covar[validIdx[i], 1 + 6 + 1],
                    covar[validIdx[i], 1 + 2 * (6 + 1)],
                ]
            )
        )
        covarPos[i, 1] = (
            3 * np.linalg.norm(posVec) / np.linalg.norm(truth[validIdx[i], 1:4]) * 100
        )
        covarVel[i, 0] = states[validIdx[i], 0]
        velVec = np.sqrt(
            np.array(
                [
                    covar[validIdx[i], 1 + 3 * (6 + 1)],
                    covar[validIdx[i], 1 + 4 * (6 + 1)],
                    covar[validIdx[i], 1 + 5 * (6 + 1)],
                ]
            )
        )
        covarVel[i, 1] = (
            3 * np.linalg.norm(velVec) / np.linalg.norm(truth[validIdx[i], 4:7]) * 100
        )
    ...
    plt.plot(
        covarVel[:, 0] * ns2min, covarVel[:, 1], color=colorList[8], linestyle="--"
    )
...
def plot_TwoOrbits(r_BN, r_BN2):
    ...
    fig = plt.figure(5, figsize=(3.5, 2.0), facecolor="w", edgecolor="k")
...
def centerXY(centerPoints, size):
    ...
    colorsInt = len(mpl.cm.get_cmap("inferno").colors) / (10.0)
    colorList = []
    for i in range(10):
    ...
    plt.figure(100, figsize=(3.5, 2.0), facecolor="w", edgecolor="k")
...
def pixelAndPos(x, r, centers, size):
    ...
    plt.figure(200, figsize=(3.5, 2.0), facecolor="w", edgecolor="k")
    ...
    plt.figure(201, figsize=(3.5, 2.0), facecolor="w", edgecolor="k")
    ...
    plt.figure(202, figsize=(3.5, 2.0), facecolor="w", edgecolor="k")
    ...
    plt.figure(203, figsize=(3.5, 2.0), facecolor="w", edgecolor="k")
    plt.plot(t, x[:, 4] * m2km, ".", color=colorList[1])
    plt.xlabel("Time (min)")
...
def imgProcVsExp(true, centers, radii, size):
    ...
    plt.plot(
        t, centers[:, 1], ".", label="ImagProc Xpix", color=colorList[5], alpha=0.7
    )
    plt.legend(loc="best")
    try:
    ...
    plt.plot(
        t, centers[:, 2], ".", label="ImagProc Ypix", color=colorList[5], alpha=0.7
    )
    plt.legend(loc="best")
    try:
        plt.ylim([centerline[-1, 1] - 15, centerline[-1, 1] + 15])
    ...
    plt.plot(
        t, radii[:, 1], ".", label=r"ImagProc $\rho$", color=colorList[5], alpha=0.7
    )
    plt.legend(loc="best")
    plt.ylabel(r"$\rho$ (px)")
    plt.xlabel("Time (min)")
    ...
    plt.plot(
        ...
        true[:, 3] - radii[:, 1],
        ".",
        label=r"$\mathrm{\rho}_\mathrm{true} - \mathrm{\rho}_\mathrm{hough}$",
        color=colorList[1],
    )
    plt.legend(loc="best")
...
def plotPostFitResiduals(Res, noise):
    ...
    for i in range(len(Res[:, 0])):
        ...
        if isinstance(noise, float):
            MeasNoise[i, :] = np.array(
                [3 * np.sqrt(noise), 3 * np.sqrt(noise), 3 * np.sqrt(noise)]
            )
        else:
            if not np.isnan(noise[i, 1]):
                ...
                MeasNoise[i, :] = (
                    np.array(
                        [
                            3 * np.sqrt(Noiselast[0, 0]),
                            3 * np.sqrt(Noiselast[1, 1]),
                            3 * np.sqrt(Noiselast[2, 2]),
                        ]
                    )
                    / 5
                )
...
def plot_cirlces(centers, radii, validity, resolution):
    ...
    for i in range(len(circleIndx)):
        if i % 30 == 0:
            ell_xy = Ellipse(
                xy=(centers[circleIndx[i], 1], centers[circleIndx[i], 2]),
                width=radii[circleIndx[i], 1],
                height=radii[circleIndx[i], 1],
                angle=0,
                linestyle="-",
                linewidth=3,
                color=colorList[i],
...
def plot_limb(limbPoints, numLimb, validity, resolution):
    ...
    for i in range(len(indx)):
        for j in range(1, int(numLimb[indx[i], 1])):
            if (
                np.abs(limbPoints[indx[i], 2 * j] + limbPoints[indx[i], 2 * j - 1])
                > 1e-1
            ):
    ...
    for i in range(len(indx)):
        if i % 30 == 0:
            ax.scatter(
                limbX[i, : int(numLimb[indx[i], 1]) - 1],
                limbY[i, : int(numLimb[indx[i], 1]) - 1],
                color=colorList[i],
                marker=".",
                alpha=0.2,
            )
...
class AnimatedCircles(object):
    ...
    def __init__(self, size, centers, radii, validity):
        ...
        self.ani = mpl.animation.FuncAnimation(
            ...
            frames=len(self.circleIndx),
            blit=True,
        )
        self.ani.save(
            "../../TestImages/circlesAnimated.gif", writer="imagemagick", fps=60
        )
    ...
    def data_stream(self, i):
        while self.idx < len(self.circleIndx):
            xy = np.array(
                [
                    [self.sensorSize[0] / 2 + 0.5, self.sensorSize[0] / 2 + 0.5],
                    [self.centers[i, 1], self.centers[i, 2]],
                    [self.centers[i, 1], self.centers[i, 2]],
                ]
            )
    ...
    def update(self, i):
        ...
        # Set sizes...
        self.scat.set_sizes((data[:, 2] / 2.0) ** 2.0)
        # Set colors..
    ...
    for i in range(3):
        if i == 0:
            ...
            unitTestSupport.saveFigurePDF(
                "StateCovarHeading" + FilterType + str(i), plt, "./"
            )
            if show_plots:
                plt.show()
            plt.close("all")
    ...
    if nstates > 3:
        for i in range(3, nstates):
            if i == 3:
                ...
                plt.xlabel("Time (min)")
                unitTestSupport.saveFigurePDF(
                    "StateCovarRate" + FilterType + str(i), plt, "./"
                )
                if show_plots:
                    plt.show()
                plt.close("all")
