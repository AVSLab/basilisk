...
\newcommand{\summary}{The Basilisk Inetial Measurement Unit (IMU) module is responsible for producing sensed body rates and acceleration from calculated values.  It also provides a change in velocity and change in attitude value for the time between IMU calls. The IMU module applies Gauss-Markov process noise to the true body rates and acceleration. A unit test has been written which validates clean run outputs of the IMU (no error application).}
