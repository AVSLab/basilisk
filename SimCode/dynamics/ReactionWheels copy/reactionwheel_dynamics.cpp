...
//  double* wheelDirectionPointer = &(ReactionWheelData[0].ReactionWheelDirection[0]);

//  wheelDirection = ReactionWheelData[0].ReactionWheelDirection;
//  v3Copy(ReactionWheelData[0].ReactionWheelDirection, wheelDirection);
  v3Scale(CmdIt->TorqueRequest, &(ReactionWheelData[0].ReactionWheelDirection[0]), StrTorque);

//  v3Scale(CmdIt->TorqueRequest, &(ReactionWheelData[1].ReactionWheelDirection[0]), StrTorque);

//  v3Scale(CmdIt->TorqueRequest, &(ReactionWheelData[2].ReactionWheelDirection[0]), StrTorque);


//
//  double wheelLocation[3];
//  v3Copy(&(ReactionWheelData.ReactionWheelLocation), wheelLocation);
//  wheelLocation= ReactionWheelData[0].ReactionWheelLocation[2];

//  StrTorque[0] = ReactionWheelData[0].ReactionWheelLocation[2];
//  StrTorque[1] = 2.0;


//  v3Scale(CmdIt->TorqueRequest, *(ReactionWheelData[0].ReactionWheelLocation), *(ReactionWheelData[0].ReactionWheelLocation));

//  StrTorque[1] = CmdIt->TorqueRequest;
...
// */
//void ReactionWheelDynamics::ComputeThrusterFire(ThrusterConfigData *CurrentThruster,
//                                           double CurrentTime)
//{
//    //! Begin method steps
...
// @param CurrentTime The current simulation clock time converted to a double
// */
//void ReactionWheelDynamics::ComputeThrusterShut(ThrusterConfigData *CurrentThruster,
//                                           double CurrentTime)
//{
//    //! Begin method steps
//    std::vector<ThrusterTimePair>::iterator it;
...
// NewRWCmds.data();
 ...
 // want to just set the reaction wheel torque to the request?
 // later add in transient/ramping
