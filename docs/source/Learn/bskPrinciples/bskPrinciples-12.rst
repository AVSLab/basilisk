...
.. math::
    ...
    \text{d}x = f(t,x)\,\text{d}t + \sum_{k=1}^{m} g_k(t,x)\,\text{d}W_k
...
and :math:`\text{d}W_k` are independent increments of a Wiener process (Brownian motion),
each distributed as :math:`\mathcal{N}(0,\text{d}t)`. This lets a simulation model process
noise, disturbances, actuator/sensor noise, or any physical effect best represented as a
...
ODE method: the stochastic integral :math:`\int g\,\text{d}W` must be approximated with
