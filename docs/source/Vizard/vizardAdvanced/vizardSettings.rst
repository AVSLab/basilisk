...
There are also applications where it could be useful to display other shape primitives, such as cuboids, cylinders, or ellipsoids for visualizing electrostatically-charged pads and surfaces. See :ref:`ANANYASCENARIO` for an example implementing cuboid-shaped pads.

To generalize different shape primitive use cases, the BSK/Vizard support structure is named ``MultiShape``. The MSM information is added using the ``enableUnityVisualization()`` method with the
...
.. list-table:: ``MultiShape`` variables
    ...
    * - ``shape``
      ...
      -
      - No
      - Set shape to use "CUBE", "CYLINDER", or "SPHERE" (default)
    * - ``dimensions``
      - double(3)
      - meters
      - Yes
