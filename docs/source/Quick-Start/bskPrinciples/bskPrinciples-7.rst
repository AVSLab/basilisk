...
Consider a more advanced Basilisk simulation setup where you have 2 modules that both need to write to the same stand-alone message.  The motivation here could a 2 flight guidance algorithm modules where depending on the flight mode only one gets executed.  Regardless of which guidance module is executed, the guidance output message must be feed to the same control module.  This cannot be accomplished if the 3rd module subscribes either to the output message of module 1 or 2.  To avoid trying to re-subscribe to different module output messages when switching flight modes, we can choose to have both modules 1 and 2 write to the same stand-alone message as conceptualized shown below.
...
In the following sample code both a C and C++ Basilisk modules are created.  To create a C wrapped stand-alone message the ``cMsgCInterfacePy`` package must be imported from ``Basilisk.architecture``.  Next, assume a message of type ``SomeMsg`` needs to be created.  This is done using::
