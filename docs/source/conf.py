...
with open(output_file, "w") as f:
    ...
    f.write(".. note::\n\n")
    f.write(
        "    This folder contains a listing of all the data files in the folder ``basilisk/supportData`` "
...
latex_documents = [
    (master_doc, "Basilisk.tex", "Basilisk Documentation", "AVS Lab", "manual"),
]
...
man_pages = [(master_doc, "basilisk", "Basilisk Documentation", [author], 1)]
...
class fileCrawler:
    ...
    def grabRelevantFiles(self, dir_path):
        ...
        for i in range(len(dirs_in_dir)):
            if (
                "_Documentation" in dirs_in_dir[i]
                or "__pycache__" in dirs_in_dir[i]
                or "_VizFiles" in dirs_in_dir[i]
                or "Support" in dirs_in_dir[i]
                or "cmake" in dirs_in_dir[i]
                or "topLevelModules" in dirs_in_dir[i]
                or "outputFiles" in dirs_in_dir[i]
                or "msgAutoSource" in dirs_in_dir[i]
                or "alg_contain" in dirs_in_dir[i]
                or "dataForExamples" in dirs_in_dir[i]
                or "tests" in dirs_in_dir[i]
                or "mujocoUtils" in dirs_in_dir[i]
            ):
                removeList.extend([i])
        ...
        for i in range(len(files_in_dir)):
            if (
                "__init__" in files_in_dir[i]
                or "conftest.py" in files_in_dir[i]
                or "*.xml" in files_in_dir[i]
                or "vizMessage.pb.cc" in files_in_dir[i]
                or "vizMessage.pb.h" in files_in_dir[i]
                or "vizMessage.proto" in files_in_dir[i]
                or "EGM9615.h" in files_in_dir[i]
                or "SunLineKF_test_utilities.py" in files_in_dir[i]
                or "datashader_utilities.py" in files_in_dir[i]
                or "reportconf.py" in files_in_dir[i]
            ):
                removeList.extend([i])
        for i in sorted(removeList, reverse=True):
            del files_in_dir[i]
    ...
    def generateAutoDoc(self, path, files_paths):
        ...
        c_file_paths = sorted(
            [
                ...
                if ".c" in s or ".cpp" in s or ".h" in s or ".hpp" in s
            ]
        )
        ...
        if not c_file_paths == []:
            ...
            for c_file_basename in c_file_basenames:
                ...
                lines = ""
                if (
                    c_file_basename == "orbitalMotion"
                    or c_file_basename == "rigidBodyKinematics"
                ):
                    pathToFolder = src_path.split("/" + c_file_basename)[0]
                    lines += (
                        ".. _" + c_file_basename + pathToFolder.split("/")[-1] + ":\n\n"
                    )
                else:
                ...
                # file name
                c_file_list_coarse = [
                    s for s in c_file_local_paths if c_file_basename in s
                ]
                c_file_list = [
                    file_name
                    for file_name in c_file_list_coarse
                    if file_name.rsplit(".", 1)[0] == c_file_basename
                ]
