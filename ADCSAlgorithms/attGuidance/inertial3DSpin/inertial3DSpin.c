...
void SelfInit_inertial3DSpin(inertial3DSpinConfig *ConfigData, uint64_t moduleID)
{
    ...
    ConfigData->outputMsgID = CreateNewMessage(ConfigData->outputDataName,
                                               sizeof(attGuidOut),
                                               "attGuidOut",          /* add the output structure name */
                                               moduleID);
...
void Update_inertial3DSpin(inertial3DSpinConfig *ConfigData, uint64_t callTime, uint64_t moduleID)
{
    ...
    ReadMessage(ConfigData->inputMsgID, &clockTime, &readSize,
                sizeof(attGuidOut), (void*) &(ConfigData->inputVector));
     ...
     */
    v3Copy(Lr, ConfigData->attGuidOut.sigma_BR);                      /* populate the output message */
