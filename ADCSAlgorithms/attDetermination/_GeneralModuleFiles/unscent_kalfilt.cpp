...
void UnscentKalFilt::UKFInit()
{
   ...
   this->Wm.vec_vals[0] = this->lambda/(this->NumStates+this->lambda);
   this->Wc.vec_vals[0] = this->lambda/(this->NumStates+this->lambda) + 
      (1-this->alpha*this->alpha + beta);
   for(i=1; i<this->CountHalfSPs*2+1; i++)
   {
      this->Wm.vec_vals[i] = 1.0/2.0*1.0/(this->NumStates+this->lambda);
