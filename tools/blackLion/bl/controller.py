...
class MasterController(object):
    ...
    def listen_at_frontend(self):
        ...
        while not (local_msg_count >= self.total_pub_msg_next_count):
            ...
            unique_msg_set.add(str(publication[0]))
            #self.logger.debug('len(unique_msg_set) =%d; ' % len(unique_msg_set))
            self.current_payload_routing.append(publication)
            #local_msg_count += 1
            local_msg_count = len(unique_msg_set)
    ...
    def advanceClock(self):
        ...
        self.logger.debug("Received all TOCKS.total_pub_msg_next_count = %d" % self.total_pub_msg_next_count)
