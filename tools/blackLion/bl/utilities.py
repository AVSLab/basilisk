...
class BlacklionNode(object):
    ...
    def get_args_list(self):
        ...
        for (key, value) in self._args.items():
            args_list.append("%s" % key)
            if value is not None:
                args_list.append("%s" % value)
        return args_list
...
class Launcher(object):
    def __init__(self):
        ...
        self.input_fd, self.output_fd = os.pipe()
    ...
    # def launch_bash(self, bash):
    #     print("\nLAUNCHER: launch_bash")
    #     params = [bash.exec_type, bash.path + bash.file_name]
    #     print("bash_params = ", params)
    #     try:
    #         bash_process = Popen(params, stdout=self.output_fd, close_fds=self.ON_POSIX, cwd=bash.path, shell=bash.shell)
    #         bash_process.wait()
    #     except:
    #         print("Bash Failed to Launch")
    ...
    def execute_all_processes(self, nodes, bashes):
        ...
        for (name, node) in nodes.items():
            ...
            processes.append(
                Popen(params + node.get_args_list(), stdout=self.output_fd,
...
class Bootstrapper(object):
    ...
    def add_python_node(self, name, file_name, path, protocol_type="tcp", port=None, ip_address=None, node_args={}):
        self.add_executable_node("python", name, path, protocol_type, port, ip_address, node_args, file_name=file_name)

    def add_executable_node(self, exec_name, name, path, protocol_type="tcp", port=None, ip_address=None, node_args={}, file_name=None):
