...
class cssFactory:
    def __init__(self, thetaInc, dict):
        self.defaults = 0

    def generate(self):
        tmpCss =  coarse_sun_sensor.CoarseSunSensor()
        print self.__dict__
        return tmpCss
...
def createThruster(
    ...
    if norm>1e-10:
        ...
        exit(1)
    TH.ThrusterDirection = thruster_dynamics.DoubleVector(tHat_S)

    # set thruster position vector
    TH.ThrusterLocation = thruster_dynamics.DoubleVector(r_S)
