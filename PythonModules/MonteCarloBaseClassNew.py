...
class MonteCarloBaseClass:
    ...
    def __init__(self):
        ...
        self.simParams = SimulationParameters(
            ...
            retentionFunction=None,
            shouldDisperseSeeds=False,
            shouldArchive=False,
            filename="",
            dispersions=[],
            retainSimulationData=False
        )
    ...
    def setRetentionFunction(self, newFunction):
        ...
        """
        self.simParams.retainSimulationData = newFunction != None
        self.simParams.retentionFunction = newFunction
    ...
    def setConfigureModule(self, newModule):
        """ Set an optional configuration function that configures a simulation instance.
        Args:
            configureFunction: (sim: SimulationBaseClass) => None
                A function with one parameter, a simulation instance.
                The function can modify the simulation instance as desired.
                The function will be called after the creationFunction in each simulation run.
                Its return value is not used
        """
    ...
    def setRetainSimulationData(self, retainData):
        """ Retain data from each simulation, after it is completed
        Args:
            retainData: bool
                Whether to retain data after executing each simulation in the MonteCarlo
        """
        self.simParams.retainSimulationData = retainData
...
class SimulationParameters():
    ...
    '''
    def __init__(self, creationFunction, configureFunction, executionFunction, retentionFunction, dispersions, shouldDisperseSeeds, shouldArchive, filename, retainSimulationData, index=None, verbose=False):
        self.index = index
        self.creationFunction = creationFunction
        self.configureFunction = configureFunction
        self.executionFunction = executionFunction
        self.retentionFunction = retentionFunction
        self.dispersions = dispersions
        self.shouldDisperseSeeds = shouldDisperseSeeds
        self.shouldArchive = shouldArchive
        self.filename = filename
        self.retainSimulationData = retainSimulationData
...
class SimulationExecutor():
    ...
    def __call__(cls, simParams, dispersionFunction=None):
        ...
        try:
            ...
            if simParams.shouldArchive:
                # open a file to save a copy of this run to.
                fHandle = open(simParams.filename + ".py", 'w')

            # create the users sim by calling their supplied creationFunction
            simInstance = simParams.creationFunction()

            # if archiving, this run's settings is saved in its own python script
            if simParams.shouldArchive:
                # defining a function that modifies a simulation to have the same
                # dispersions and random seeds as this run does
                fHandle.write("def disperseVariables(simInstance): \n\n")

            # if dispersionFunction is None we generate parameters here.
            if dispersionFunction is None:

                # generate the desired parameters
                for disp in simParams.dispersions:
                    # for each dispersion in the MonteCarlo, generate a parameter
                    # and modify the simulation
                    execString = cls.generateExec(disp, simInstance)
                    exec execString
                    if simParams.shouldArchive:
                        # note the simulation parameter and save it to the archive
                        # for repeatability
                ...
                if simParams.shouldDisperseSeeds:
                    # disperse the random seeds for the model
                    statement = cls.disperseSeeds(simInstance)
                    if simParams.shouldArchive:
                        # we also note the random seed in the archive for repeatability
                        fHandle.write(statement)

            # if using a dispersion function, disperse parameters and seeds of the simulation instance
            else:
                dispersionFunction(simInstance)

            # configure the simulation, with the user-supplied configureFunction
            if simParams.configureFunction is not None:
                simParams.configureFunction(simInstance)

            if simParams.shouldArchive:  # thats all we archive so close the file
            ...
            if simParams.retainSimulationData:
                ...
                with gzip.open(retentionFile, "w") as archive:
                    pickle.dump(simParams.retentionFunction(simInstance), archive)
    ...
    def disperseSeeds(simInstance):
        ...
        for Task in simInstance.TaskList:
            ...
            for model in Task.TaskModels:
                execString = 'simInstance.TaskList[' + str(i) + '].TaskModels'
    ...
    @staticmethod
    def generateExec(disp, simInstance):
        """ Takes a dispersion, returns a statement that modifies a variable simInstance to use that dispersion

        Args:
            disp: Dispersion
                A dispersion to generate a parameter from.
            simInstance: SimulationBaseClass
                A basilisk simulation to apply the dispersion to
        Returns:
            statement: string
                The statement that modifies a simulation to use this generated parameter.
                In the format simInstance.varName = parameter
        """

        nextValue = disp.generate(simInstance)

        execString = ''
        if isinstance(disp, NormalThrusterUnitDirectionVectorDispersion):
            separator = '.'
            execString = 'simInstance.' + \
                separator.join(disp.varNameComponents[0:-1]) + '.thrDir_B = ['
            for i in range(3):
            ...
            execString = 'simInstance.' + disp.varName + ' = ['
            for i in range(3):
            ...
            execString = 'simInstance.' + disp.varName + ' = ' + str(nextValue)
        return execString
