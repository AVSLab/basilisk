 ...
 * at a site fixed on a body. The model reads:
 ...
 *
 * Force direction is opposite the relative flow. Magnitude follows
 * \f$ F = \tfrac{1}{2}\rho\,v^2 C_D A \f$.
...
{
public:
    /** @brief Initializes the C output messages */
...
public:
    ...
    /** @brief Output force at the site in the site reference frame (C message). */
    ForceAtSiteMsg_C forceOutMsgC = {};

    /** @brief Output torque in the site reference frame.*/
    Message<TorqueAtSiteMsgPayload> torqueOutMsg;

    /** @brief Output torque in the site reference frame (C message). */
