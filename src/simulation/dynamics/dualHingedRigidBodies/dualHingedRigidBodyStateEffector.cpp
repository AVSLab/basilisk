...
void DualHingedRigidBodyStateEffector::updateContributions(double integTime, BackSubMatrices & backSubContr, Eigen::Vector3d sigma_BN, Eigen::Vector3d omega_BN_B, Eigen::Vector3d g_N)
{
    ...
    Eigen::Vector3d attBodyForce_H1 = Eigen::Vector3d::Zero();
    Eigen::Vector3d attBodyTorquePntS1_H1 = Eigen::Vector3d::Zero();
    Eigen::Vector3d attBodyForce_H2 = Eigen::Vector3d::Zero();
    Eigen::Vector3d attBodyTorquePntS2_H2 = Eigen::Vector3d::Zero();
    std::vector<DynamicEffector*>::iterator dynIt;
    for(dynIt = this->dynEffectors.begin(); dynIt != this->dynEffectors.end(); dynIt++)
    {
        ...
        if ((*dynIt)->getPropName_inertialPosition() == this->nameOfInertialPositionProperty1) {
            attBodyForce_H1 += (*dynIt)->forceExternal_B;
            attBodyTorquePntS1_H1 += (*dynIt)->torqueExternalPntB_B;
        }
        else if ((*dynIt)->getPropName_inertialPosition() == this->nameOfInertialPositionProperty2) {
            attBodyForce_H2 += (*dynIt)->forceExternal_B;
            attBodyTorquePntS2_H2 += (*dynIt)->torqueExternalPntB_B;
        }
    }
    ...
    Eigen::Vector3d externalForce2 = gravForcePan2 + attBodyForce_H2;
    Eigen::Vector3d externalTorque1 = gravTorquePan1PntH1 + attBodyTorquePntS1_H1;
    Eigen::Vector3d externalTorque2 = gravTorquePan2PntH2 + attBodyTorquePntS2_H2;
    ...
    this->vectorVDHRB(0) =  -(this->IPntS1_S1(0,0) - this->IPntS1_S1(2,2))*this->omega_PN_S1(2)*this->omega_PN_S1(0)
                            + this->u1 - this->k1*this->theta1 - this->c1*this->theta1Dot + this->k2*this->theta2 + this->c2*this->theta2Dot + this->sHat12_P.dot(externalTorque1) + this->l1*this->sHat13_P.dot(externalForce2) -
                            ...
                            - this->mass2*this->l1*this->sHat13_P.transpose()*(2*this->omegaTildePNLoc_P*this->rPrimeS2P_P + this->omegaTildePNLoc_P*this->omegaTildePNLoc_P*this->r_S2P_P + this->l1*this->theta1Dot*this->theta1Dot*this->sHat11_P + this->d2*(this->theta1Dot + this->theta2Dot)*(this->theta1Dot + this->theta2Dot)*this->sHat21_P);

    this->vectorVDHRB(1) =  -(this->IPntS2_S2(0,0) - this->IPntS2_S2(2,2))*this->omega_PN_S2(2)*this->omega_PN_S2(0)
                            + this->u2 - this->k2*this->theta2 - this->c2*this->theta2Dot + this->sHat22_P.dot(externalTorque2) - this->mass2*this->d2*this->sHat23_P.transpose()*(2*this->omegaTildePNLoc_P*this->rPrimeS2P_P + this->omegaTildePNLoc_P*this->omegaTildePNLoc_P*this->r_S2P_P + this->l1*this->theta1Dot*this->theta1Dot*this->sHat11_P);
