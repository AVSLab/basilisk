...
def executeSimRun(simContainer, thrusterSet, simRate, totalTime):
    ...
    while simContainer.TotalSim.CurrentNanos < newStopTime:
        ...
        thrusterSet.computeForceTorque(
            simContainer.TotalSim.CurrentNanos * macros.NANO2SEC, timeStep
        )
        thrusterSet.computeForceTorque(
            simContainer.TotalSim.CurrentNanos * macros.NANO2SEC
            + simRate * macros.NANO2SEC / 2.0,
            timeStep,
        )
        thrusterSet.computeForceTorque(
            simContainer.TotalSim.CurrentNanos * macros.NANO2SEC
...
@pytest.mark.parametrize(
    "ramp, thrustNumber, duration, long_angle, lat_angle, location, rate, cutoff, rampDown, swirlTorque, blowDown",
    [
        (
            "OFF",
            1,
        ...
        (
            ...
            0.0,
            "OFF",
        ),  # Short fire test with higher test rate
        (
            "OFF",
            1,
            5.0,
        ...
        (
            ...
            0.5,
            "OFF",
        ),  # Basic ramp with swirl torque test
        (
            "OFF",
            1,
            5.0,
...
def unitThrusters(
    ...
    thruster1.thrDir_B = [
        [math.cos(anglerad_long) * math.cos(anglerad_lat)],
        [math.sin(anglerad_long) * math.cos(anglerad_lat)],
        [math.sin(anglerad_lat)],
    ]
    thruster1.MaxThrust = 1.0
    thruster1.steadyIsp = Isp
    thruster1.MinOnTime = 0.006
    thruster1.MaxSwirlTorque = swirlTorque
    for thr_coeff in thrust_blow_down_coeff:
        thruster1.thrBlowDownCoeff.push_back(thr_coeff)
    for isp_coeff in isp_blow_down_coeff:
        thruster1.ispBlowDownCoeff.push_back(isp_coeff)
    thrusterSet.addThruster(thruster1)

    loc1 = np.array(
        [thruster1.thrLoc_B[0][0], thruster1.thrLoc_B[1][0], thruster1.thrLoc_B[2][0]]
    )
    dir1 = np.array(
        [thruster1.thrDir_B[0][0], thruster1.thrDir_B[1][0], thruster1.thrDir_B[2][0]]
    )

    if thrustNumber == 2:
        thruster2 = thrusterDynamicEffector.THRSimConfig()
        thruster2.thrLoc_B = np.array([[1.0], [0.0], [0.0]]).reshape([3, 1])
        thruster2.thrDir_B = np.array(
            [
                [
                    math.cos(anglerad_long + math.pi / 4.0)
                    * math.cos(anglerad_lat - math.pi / 4.0)
                ],
                [
                    math.sin(anglerad_long + math.pi / 4.0)
                    * math.cos(anglerad_lat - math.pi / 4.0)
                ],
                [math.sin(anglerad_lat - math.pi / 4.0)],
            ]
        ).reshape([3, 1])
        ...
        loc2 = np.array(
            [
                thruster2.thrLoc_B[0][0],
                thruster2.thrLoc_B[1][0],
                thruster2.thrLoc_B[2][0],
            ]
        )
        dir2 = np.array(
            [
                thruster2.thrDir_B[0][0],
                thruster2.thrDir_B[1][0],
                thruster2.thrDir_B[2][0],
            ]
        )
    ...
    sparetime = 3.0 * 1.0 / macros.NANO2SEC
    thrStartTime = sparetime
    thrDurationTime = duration * 1.0 / macros.NANO2SEC  # Parametrized thrust duration

    # Configure a single thruster firing, create a message for it
    thrusterSetLog = thrusterSet.logger(
        ["forceExternal_B", "torqueExternalPntB_B", "mDotTotal"]
    )
    ...
    if thrustNumber == 2:
        ThrustMessage.OnTimeRequest = [0.0, 0.0]
    ...
    TotalSim.newManager.createProperty(
        "r_BN_N", [[0], [0], [0]]
    ...
    thrusterSet.setPropName_inertialPosition(
        TotalSim.scObject.gravField.inertialPositionPropName
    )
    ...
    if ramp == "OFF":
        ...
        if thrustNumber == 2:
            ThrustMessage.OnTimeRequest = [
                thrDurationTime * macros.NANO2SEC,
                thrDurationTime * macros.NANO2SEC,
            ]
        ...
        mDotData = unitTestSupport.addTimeColumn(
            thrusterSetLog.times(), thrusterSetLog.mDotTotal
        )
        ...
        snippetName = (
            ...
            + str(int(loc1[2]))
            + "_Rate"
            + str(int(1.0 / (testRate * macros.NANO2SEC)))
            + "_Swirl"
            + str(int(swirlTorque))
            + "_BlowDown"
            + blowDown
        ...
        if thrustNumber == 1:
            texSnippet = (
                "The thruster is set at "
                + str(int(long_angle))
                + r"$^\circ$ off the x-axis "
                + str(int(lat_angle))
                + r"$^\circ$ off the z-axis, in the position $\bm r = \left("
                + str(loc1[0])
                + ","
                + str(loc1[1])
                + ","
                + str(loc1[2])
                + r"\right)$. The test is launched using "
                + str(thrustNumber)
                + " thruster, for "
                + str(duration)
                + " seconds. The test rate is "
                + str(int(1.0 / (testRate * macros.NANO2SEC)))
                + " steps per second. Swirl torque is set to "
                + str(int(swirlTorque))
                + " Newton meters and blow down effects are "
                + blowDown
                + "."
            )
        if thrustNumber == 2:
            texSnippet = (
                "The first thruster is set at "
                + str(int(long_angle))
                + r"$^\circ$ off the x-axis "
                + str(int(lat_angle))
                + r"$^\circ$ off the z-axis, in the position $\bm r = \left("
                ...
                + ","
                + str(loc1[2])
                + r"\right)$. The second thruster is set at "
                + str(int(long_angle + 45))
                + r"$^\circ$ off the x-axis "
                + str(int(lat_angle + 45))
                + r"$^\circ$ off the z-axis, in the position $\bm r = \left("
                + str(loc2[0])
                + ","
                + str(loc2[1])
        ...
        for i in range(
            np.shape(thrForce)[0]
        ):  # Thrust fires 2 times steps after the pause of sim and restart
            if (
                i > int(round(thrStartTime / testRate)) + 1
                and i < int(round((thrStartTime + thrDurationTime) / testRate)) + 2
            ):
                if thrustNumber == 1:
        ...
        for i in range(0, len(np.array(mDotData))):
            if not unitTestSupport.isArrayEqual(
                np.array(mDotData)[i, :], expMDot[i, :], 1, ErrTolerance
            ):
                testFailCount += 1
        ...
        for i in range(
            ...
            if (
                ...
                if thrustNumber == 1:
                    expectedpointstor[0:3, i] = (
                        np.cross(loc1, dir1) + swirlTorque * dir1
                    ) * fuel_ratio
                else:
                    expectedpointstor[0:3, i] = (
                        np.cross(loc1, dir1) + swirlTorque * dir1 + np.cross(loc2, dir2)
                    ) * fuel_ratio
    ...
    if ramp == "ON":
        format = r"width=0.8\textwidth"
        rampsteps = 10
        sparetime = 3.0 * 1 / macros.NANO2SEC
        thrStartTime = sparetime - 1.0 * 1 / macros.NANO2SEC
        ...
        for i in range(rampsteps):
            ...
            newElement.TimeDelta = (i + 1.0) * 0.1
        ...
        # Set up the ramps
        thruster1.ThrusterOnRamp = thrusterDynamicEffector.ThrusterTimeVector(
            rampOnList
        )
        thruster1.ThrusterOffRamp = thrusterDynamicEffector.ThrusterTimeVector(
            rampOffList
        )
        ...
        if rampDown == "OFF":
            if cutoff == "OFF":
                ...
                ThrustMessage.OnTimeRequest = [thrDurationTime * macros.NANO2SEC]
                thrCmdMsg.write(
                    ThrustMessage, TotalSim.TotalSim.CurrentNanos + testRate
                )
                executeSimRun(
                    TotalSim, thrusterSet, testRate, int(thrDurationTime + sparetime)
                )
                ...
                texSnippet = (
                    "We test the ramped thrust with "
                    + str(rampsteps)
                    + " incremental steps. The single thruster is set at the default "
                    + str(int(long_angle))
                    + r"$^\circ$ off the x-axis "
                    + str(int(lat_angle))
                    + r"$^\circ$ off the z-axis, at $\bm r = \left("
                    + str(loc1[0])
                    + ","
                    + str(loc1[1])
                    + ","
                    ...
                    + str(duration)
                    + " seconds with a test rate of "
                    + str(int(1.0 / (testRate * macros.NANO2SEC)))
                    + " steps per second. The Cutoff test is "
                    + cutoff
                    + ", swirl torque is set to "
                    + str(int(swirlTorque))
                    + " Newton meters, and blow down effects are "
                    + blowDown
                    + "."
                ...
                PlotTitle = (
                    "All Forces and Torques with "
                    + str(rampsteps)
                    + " step Ramp, thrust for "
                    + str(int(duration))
                    + "s. Cutoff "
                    + cutoff
                    + ", testRate "
                ...
                plt.plot(thrForce[:, 0] * 1.0e-9, thrForce[:, 1], "b", label="x Force")
                plt.plot(
                    thrTorque[:, 0] * 1.0e-9, thrTorque[:, 1], "b--", label="x Torque"
                )
                plt.plot(thrForce[:, 0] * 1.0e-9, thrForce[:, 2], "g", label="y Force")
                plt.plot(
                    thrTorque[:, 0] * 1.0e-9, thrTorque[:, 2], "g--", label="y Torque"
                )
                plt.plot(thrForce[:, 0] * 1.0e-9, thrForce[:, 3], "r", label="z Force")
                plt.plot(
                    thrTorque[:, 0] * 1.0e-9, thrTorque[:, 3], "r--", label="z Torque"
                )
                plt.legend(loc="upper right")
                plt.xlabel("Time(s)")
                plt.ylim(-1.5, 2)
                ...
                if ramplength < thrDurationTime * macros.NANO2SEC:
                    for i in range(np.shape(thrForce)[0]):
                        ...
                        if (
                            ...
                            < int(
                                round(
                                    (thrStartTime + ramplength * 1.0 / macros.NANO2SEC)
                                    / testRate
                                )
                            )
                            + 2
                            ...
                            RampFunction[i] = (
                                i - int(round(thrStartTime / testRate)) - 2 + 1.0
                            ) * (macros.NANO2SEC * testRate)
                        if (
                            i
                            > int(
                                round(
                            ...
                            + 1
                            and i
                            < int(round((thrStartTime + thrDurationTime) / testRate))
                            + 2
                        ):
                            RampFunction[i] = 1.0
                        if (
                            ...
                            < int(
                                round(
                                    (
                                        thrStartTime
                                        + thrDurationTime
                                        + ramplength * 1.0 / macros.NANO2SEC
                                    )
                                    / testRate
                                )
                            )
                            ...
                            RampFunction[i] = 1.0 - (
                                i
                                - int(
                                    round((thrStartTime + thrDurationTime) / testRate)
                                )
                                - 2
                                + 1.0
                            ) * (macros.NANO2SEC * testRate)
                    ...
                    for i in range(np.shape(thrForce)[0]):
                        ...
                        if (
                            ...
                            and i
                            < int(
                                round((thrStartTime + 2 * thrDurationTime) / testRate)
                            )
                            + 2
                        ):
                            RampFunction[i] = RampFunction[
                                int(round((thrStartTime + thrDurationTime) / testRate))
                                + 1
                            ] - (
                                i
                ...
                expMDot = np.zeros([np.shape(np.array(mDotData))[0], 1])
                for i in range(
                    np.shape(RampFunction)[0]
                    - (int(round(thrStartTime / testRate)) + 1)
                ):
                    if (
                        i > 0
                        and RampFunction[i + int(round(thrStartTime / testRate)) + 1]
                        != 0.0
                    ):
                        expMDot[i, 0] = thrustNumber / (g * Isp)
            ...
            if cutoff == "ON":
                ...
                executeSimRun(TotalSim, thrusterSet, testRate, int(thrStartTime))
                ThrustMessage.OnTimeRequest = [COtime * 10.0]
                thrCmdMsg.write(
                    ThrustMessage, TotalSim.TotalSim.CurrentNanos + testRate
                )
                executeSimRun(
                    TotalSim, thrusterSet, testRate, int(COtime * 1.0 / macros.NANO2SEC)
                )
                ...
                executeSimRun(
                    ...
                    thrusterSet,
                    testRate,
                    int(COrestart * 1.0 / macros.NANO2SEC + sparetime),
                )
                ...
                for i in range(np.shape(thrForce)[0]):
                    ...
                    if (
                        ...
                        < int(
                            round(
                                (thrStartTime + 2 * ramplength * 1.0 / macros.NANO2SEC)
                                / testRate
                            )
                        )
                        + 2
        ...
        if rampDown == "ON":
            ...
            executeSimRun(
                ...
                thrusterSet,
                testRate,
                int((RDstart + RDrestart) * 1.0 / macros.NANO2SEC),
            )
            ...
            executeSimRun(
                ...
                thrusterSet,
                testRate,
                int(RDlength * 1.0 / macros.NANO2SEC + sparetime),
            )
            ...
            texSnippet = (
                ...
                + r"\right)$. The thrust is set for "
                + str(RDstart)
                + " seconds initially with a test rate of "
                + str(int(1.0 / (testRate * macros.NANO2SEC)))
                + " steps per second. The Cutoff test is "
                + cutoff
                + ", the RampDown test is "
                + rampDown
                + ", swirl torque is set to "
                + str(int(swirlTorque))
                + " Newton meters, and blow down effects are "
            ...
            for i in range(np.shape(thrForce)[0]):
                ...
                if (
                    ...
                    < int(
                        round(
                            (thrStartTime + RDstart * 1.0 / macros.NANO2SEC) / testRate
                        )
                    )
                    + 2
                ):  # ramp up
                ...
                if (
                    ...
                    < int(
                        round(
                            (
                                thrStartTime
                                + (RDstart + RDrestart) * 1.0 / macros.NANO2SEC
                            )
                            / testRate
                        )
                    )
                    ...
                    RampFunction[i] = RampFunction[
                        int(
                            round(
                                (thrStartTime + RDstart * 1.0 / macros.NANO2SEC)
                                / testRate
                            )
                        )
                        + 1
                ...
                if (
                    ...
                    < int(
                        round(
                            (
                                thrStartTime
                                + (RDstart + RDrestart + (1.0 - RDstart + RDrestart))
                                * 1.0
                                / macros.NANO2SEC
                            )
                            / testRate
                ...
                if (
                    ...
                    > int(
                        round(
                            (
                                thrStartTime
                                + (RDstart + RDrestart + RDlength + ramplength)
                                * 1.0
                                / macros.NANO2SEC
                            )
                            / testRate
            ...
            expMDot = np.zeros([np.shape(np.array(mDotData))[0], 1])
            for i in range(
                1, np.shape(RampFunction)[0] - (int(round(thrStartTime / testRate)) + 1)
            ):
                if RampFunction[i + int(round(thrStartTime / testRate)) + 1] != 0.0:
                    expMDot[i, 0] = thrustNumber / (g * Isp)
                    expMDot[i + 1, 0] = thrustNumber / (
                        g * Isp
                    )  # The way the last ramp is set up, we need another mdot value
