...
@pytest.mark.parametrize("useFlag, testCase", [(False, 1), (False, 2), (False, 3)])
...
def sphericalPendulumTest(show_plots, useFlag, testCase):
    ...
    scSim.pendulum1.pHat_01 = [
        [np.sqrt(2) / 2],
        [0],
        [np.sqrt(2) / 2],
    ]  # first unit vector of the Pendulum frame
    scSim.pendulum1.pHat_02 = [
        [0],
        [1],
        [0],
    ]  # second unit vector of the Pendulum frame
    scSim.pendulum1.pHat_03 = [
        [-np.sqrt(2) / 2],
        [0],
        [np.sqrt(2) / 2],
    ]  # third unit vector of the Pendulum frame
    ...
    if testCase == 3:
        stateLog = pythonVariableLogger.PythonVariableLogger(
            {
                "sphericalPendulumMass1": lambda _: scObject.dynManager.getStateObject(
                    scSim.pendulum1.nameOfMassState
                ).getState(),
                "sphericalPendulumMass2": lambda _: scObject.dynManager.getStateObject(
                    scSim.pendulum2.nameOfMassState
                ).getState(),
            }
        )
    ...
    if testCase == 3:
        ...
        mass1Out = unitTestSupport.addTimeColumn(
            stateLog.times(), stateLog.sphericalPendulumMass1
        )
        mass2Out = unitTestSupport.addTimeColumn(
            stateLog.times(), stateLog.sphericalPendulumMass2
        )
    ...
    if testCase != 3:
        ...
        unitTestSupport.writeFigureLaTeX(
            "ChangeInOrbitalAngularMomentum" + testCaseName,
            "Change in Orbital Angular Momentum " + testCaseName,
            plt,
        ...
        unitTestSupport.writeFigureLaTeX(
            "ChangeInOrbitalEnergy" + testCaseName,
            "Change in Orbital Energy " + testCaseName,
            plt,
            r"width=0.8\textwidth",
            path,
        )
        ...
        unitTestSupport.writeFigureLaTeX(
            "ChangeInRotationalAngularMomentum" + testCaseName,
            "Change in Rotational Angular Momentum " + testCaseName,
            plt,
            r"width=0.8\textwidth",
            path,
        )
        ...
        plt.plot(
            rotEnergy[:, 0] * 1e-9,
            (rotEnergy[:, 1] - rotEnergy[0, 1]) / rotEnergy[0, 1],
        )
        plt.xlabel("Time (s)")
        plt.ylabel("Relative Rotational Energy Variation")
        unitTestSupport.writeFigureLaTeX(
            "ChangeInRotationalEnergy" + testCaseName,
            "Change in Rotational Energy " + testCaseName,
            plt,
            r"width=0.8\textwidth",
            path,
        )
    if testCase == 3:
        ...
        mDotParicle1True = mDotFuel * (20.0 / 100.0)
        mDotParicle2True = mDotFuel * (40.0 / 100.0)
        mDotParicle1Data = [
            0,
            (mass1Out[2, 1] - mass1Out[1, 1])
            / ((mass1Out[2, 0] - mass1Out[1, 0]) * 1e-9),
        ]
        mDotParicle2Data = [
            0,
            (mass2Out[2, 1] - mass2Out[1, 1])
            / ((mass2Out[2, 0] - mass2Out[1, 0]) * 1e-9),
        ]
    ...
    if testCase != 3:
        ...
        for k in range(len((rotAngMom_N[:, 1]))):
            if (
                abs((rotAngMom_N[k, 1] - rotAngMom_N[0, 1]) / rotAngMom_N[0, 1])
                > accuracy
            ):
                testFailCount += 1
                testMessages.append(
                    "FAILED: SphericalPendulum does not conserve Rotational Angular Momentum around x axes (timeStep={}s)".format(
                        timeStep
                    )
                )
            if (
                abs((rotAngMom_N[k, 2] - rotAngMom_N[0, 2]) / rotAngMom_N[0, 2])
                > accuracy
            ):
                testFailCount += 1
                testMessages.append(
                    "FAILED: SphericalPendulum does not conserve Rotational Angular Momentum around y axes (timeStep={}s)".format(
                        timeStep
                    )
                )
            if (
                abs((rotAngMom_N[k, 3] - rotAngMom_N[0, 3]) / rotAngMom_N[0, 3])
                > accuracy
            ):
                testFailCount += 1
                testMessages.append(
                    "FAILED: SphericalPendulum does not conserve Rotational Angular Momentum around z axes (timeStep={}s)".format(
                        timeStep
                    )
                )
            if (
                abs((orbAngMom_N[k, 1] - orbAngMom_N[0, 1]) / orbAngMom_N[0, 1])
                > accuracy
            ):
                testFailCount += 1
                testMessages.append(
                    "FAILED: SphericalPendulum does not conserve Orbital Angular Momentum around x axes (timeStep={}s)".format(
                        timeStep
                    )
                )
            if (
                abs((orbAngMom_N[k, 2] - orbAngMom_N[0, 2]) / orbAngMom_N[0, 2])
                > accuracy
            ):
                testFailCount += 1
                testMessages.append(
                    "FAILED: SphericalPendulum does not conserve Orbital Angular Momentum around y axes (timeStep={}s)".format(
                        timeStep
                    )
                )
            if (
                abs((orbAngMom_N[k, 3] - orbAngMom_N[0, 3]) / orbAngMom_N[0, 3])
                > accuracy
            ):
                testFailCount += 1
                testMessages.append(
                    "FAILED: SphericalPendulum does not conserve Orbital Angular Momentum around z axes (timeStep={}s)".format(
                        timeStep
                    )
                )
            if abs((rotEnergy[k, 1] - rotEnergy[0, 1]) / rotEnergy[0, 1]) > accuracy:
                testFailCount += 1
                testMessages.append(
                    "FAILED: SphericalPendulum does not conserve Rotational Energy (timeStep={}s)".format(
                        timeStep
                    )
                )
            if abs((orbEnergy[k, 1] - orbEnergy[0, 1]) / orbEnergy[0, 1]) > accuracy:
                testFailCount += 1
                testMessages.append(
                    "FAILED: SphericalPendulum does not conserve Orbital Energy (timeStep={}s)".format(
                        timeStep
                    )
                )
    if testCase == 3:
        ...
        if not unitTestSupport.isDoubleEqual(
            mDotParicle1Data[1], mDotParicle1True, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: Linear Spring Mass Damper unit test failed mass 1 dot test"
            )
        if not unitTestSupport.isDoubleEqual(
            mDotParicle2Data[1], mDotParicle2True, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: Linear Spring Mass Damper unit test failed mass 2 dot test"
            )
