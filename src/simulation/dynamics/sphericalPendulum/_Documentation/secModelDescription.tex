...
\begin{equation}
	...
	\label{eq:dotH_T}
\end{equation}
Where $\bm{L}_{T,j}$ is the external torque on the slosh mass about the tank point. The damping is a linear viscous force, $-\bm{D}\bm{l_j'}$, obtained by applying the damping matrix $\bm{D}$ to the relative velocity between the tank and the fuel, $\bm{l_j'}$. This force acts on the slosh mass, so, like the gravitational force $\bm{F_g}$, it enters the torque balance through the moment arm $\bm{l_j}$. Any additional external torque is captured by $\bm{\tau}_{ext,j}$.
\begin{equation}
