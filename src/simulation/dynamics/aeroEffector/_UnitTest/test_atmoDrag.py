...
from Basilisk.simulation import aeroDynamicEffector
...
def run(show_plots, orbitCase, planetCase):
    ...
    #   Initialize new atmosphere and aero model, add them to task
    newAtmo = exponentialAtmosphere.ExponentialAtmosphere()
    atmoTaskName = "atmosphere"
    newAtmo.ModelTag = "ExpAtmo"

    projArea = 10.0  # Set aero area in m^2
    dragCoeff = 2.2  # Set drag ceofficient

    aeroEffector = aeroDynamicEffector.AeroDynamicEffector()
    aeroEffector.ModelTag = "AeroEff"

    aeroEffectorTaskName = "aero"
    aeroEffector.coreParams.projectedArea = projArea
    aeroEffector.coreParams.dragCoeff = dragCoeff
    aeroEffector.coreParams.comOffset = [1., 0., 0.]

    dynProcess.addTask(scSim.CreateNewTask(atmoTaskName, simulationTimeStep))
    dynProcess.addTask(scSim.CreateNewTask(aeroEffectorTaskName, simulationTimeStep))
    ...
    scObject.addDynamicEffector(aeroEffector)

    # add spacecraft object to the simulation process
    scSim.AddModelToTask(simTaskName, scObject)
    scSim.AddModelToTask(aeroEffectorTaskName, aeroEffector)
    # clear prior gravitational body and SPICE setup definitions
    gravFactory = simIncludeGravBody.gravBodyFactory()

    aeroEffector.atmoDensInMsg.subscribeTo(newAtmo.envOutMsgs[0])
    ...
    scSim.AddVariableForLogging('AeroEff.forceExternal_B', samplingTime, StartIndex=0, StopIndex=2)
    #
    ...
    aeroForce = scSim.GetLogVariableData('AeroEff.forceExternal_B')
    densData = dataNewAtmoLog.neutralDensity
    np.set_printoptions(precision=16)

    #   Compare to expected values

    endInd = aeroForce.shape[0]

    refAeroForce = np.zeros([endInd,3])
    ...
    for ind in range(1,endInd-1):
        if not unitTestSupport.isArrayEqual(aeroForce[ind,1:4], refAeroForce[ind],3,accuracy):
            testFailCount += 1
            testMessages.append(
                "FAILED:  AeroEffector failed force unit test with a value difference of "
                + str(np.linalg.norm(aeroForce[ind,1:4]-refAeroForce[ind])))
    ...
    if testFailCount == 0:
        print("PASSED: " + aeroEffector.ModelTag)
    else:
        print("Failed: " + aeroEffector.ModelTag)
