...
void SpinningBody::setMass(double mass) {
    if (mass > 0.0)
        ...
        bskLogger.bskLog(BSK_ERROR, "Mass must be greater than 0.");
    }
}
...
{
    spinningBody.theta = this->thetaState->getState()(spinningBodyIndex, 0);
    spinningBody.thetaDot = this->thetaDotState->getState()(spinningBodyIndex, 0);
                                                        ...
                                                        Eigen::Vector3d g_N)
{
    ...
    Eigen::MatrixXd MTheta = Eigen::MatrixXd::Zero(this->N, this->N);
    Eigen::MatrixXd AThetaStar = Eigen::MatrixXd::Zero(this->N, 3);
    Eigen::MatrixXd BThetaStar = Eigen::MatrixXd::Zero(this->N, 3);
    Eigen::VectorXd CThetaStar = Eigen::VectorXd::Zero(this->N);
