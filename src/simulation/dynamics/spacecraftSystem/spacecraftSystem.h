...
class SpacecraftUnit
{
  public:
    ...
    Eigen::Vector3d dvAccum_CN_B; //!< [m/s] Accumulated delta-v of center of mass relative to inertial frame in body
                                  //!< frame coordinates
    Eigen::Vector3d
      dvAccum_BN_B; //!< [m/s] accumulated delta-v of body frame relative to inertial frame in body frame coordinates
  ...
  public:
    ...
    void writeOutputMessagesSC(uint64_t clockTime,
                               int64_t moduleID);         //!< Method to write all of the class output messages
...
class SpacecraftSystem : public DynamicObject
{
  public:
    ...
    std::vector<SpacecraftUnit*>
      spacecraftDockedToPrimary; //!< vector of spacecraft currently docked with primary spacecraft
    std::vector<SpacecraftUnit*>
      unDockedSpacecraft;            //!< vector of spacecraft currently detached from all other spacecraft
  ...
  public:
    ...
    void computeEnergyMomentumSC(
      double time,
      SpacecraftUnit& spacecraft);                 //!< This method computes the total energy and momentum of the s/c
    void computeEnergyMomentumSystem(double time); //!< This method computes the total energy and momentum of the s/c
    void updateSpacecraftMassProps(
      double time,
      SpacecraftUnit& spacecraft);           //!< This method computes the total mass properties of the s/c
    ...
    void equationsOfMotionSC(
      double integTimeSeconds,
      double timeStep,
      SpacecraftUnit& spacecraft); //!< This method computes the equations of motion for the whole system
    void equationsOfMotionSystem(
      double integTimeSeconds,
      double timeStep); //!< This method computes the equations of motion for the whole system
    ...
    void attachSpacecraftToPrimary(
      SpacecraftUnit* newSpacecraft,
      std::string dockingPortNameOfNewSpacecraft,
      std::string dockingToPortName); //!< Attaches a spacecraft to the primary spacecraft chain
    void addSpacecraftUndocked(
      SpacecraftUnit* newSpacecraft);              //!< Attaches a spacecraft to the primary spacecraft chain
