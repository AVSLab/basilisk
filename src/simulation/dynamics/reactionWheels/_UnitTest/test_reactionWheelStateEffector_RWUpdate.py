...
def RWUpdateTest(show_plots, accuracy):
    ...
    for i in range(numTests + 1):
        ...
        if not unitTestSupport.isArrayEqual(
            dataRW[:, i], trueTorque[i], numRW, accuracy
        ):
            testFailCount += 1
            if i == 0:
                ...
                testMessages.append(
                    "FAILED: Reaction Wheel Update Test failed test " + str(i) + "\n"
                )
