...
def test_reactionWheelIntegratedTest(show_plots, useFlag, testCase):
    """Module Unit Test"""
    [testResults, testMessage] = reactionWheelIntegratedTest(
        show_plots, useFlag, testCase
    )
...
def reactionWheelIntegratedTest(show_plots, useFlag, testCase):
    ...
    varMaxMomentum = 100.0  # Nms

    if (
        testCase == "BalancedWheels"
        or testCase == "BOE"
        or testCase == "FrictionSpinDown"
        or testCase == "FrictionSpinUp"
    ):
    ...
    if (
        testCase == "BalancedWheels"
        or testCase == "JitterSimple"
        or testCase == "JitterFullyCoupled"
    ):
        rwFactory.create(
            "Honeywell_HR16",
            [1, 0, 0],  # gsHat_B
        ...
        assert rwFactory.rwList["RW1"].fCoulomb == 10, (
            "wasn't able to set custom fCoulomb value"
        )
        assert rwFactory.rwList["RW1"].fStatic == 5, (
            "wasn't able to set custom fStatic value"
        )
        assert rwFactory.rwList["RW1"].cViscous == 3, (
            "wasn't able to set custom cViscous value"
        )
        ...
        if testCase == "BOE":
            ...
            for i in range(5):
                if timeBOE[i] > timeTorqueOn:
                    omegaBOE[i] = scObject.hub.omega_BN_BInit[2][
                        0
                    ] - motorTorque / I1Hub * (timeBOE[i] - timeTorqueOn)
                    thetaBOE[i] = (
                        scObject.hub.omega_BN_BInit[2][0] * (timeBOE[i] - timeTorqueOn)
                        - 0.5 * motorTorque / I1Hub * (timeBOE[i] - timeTorqueOn) ** 2
                        + scObject.hub.omega_BN_BInit[2][0] * (timeTorqueOn)
                    )
                    wheelSpeedBOE[i] = initialWheelSpeed * macros.RPM + (
                        I1Hub + wheelJs
                    ) * motorTorque / (I1Hub * wheelJs) * (timeBOE[i] - timeTorqueOn)
                else:
    ...
    if testCase == "BalancedWheels":
        truePos = [[-4.02553766e06, 7.48712857e06, 5.24933964e06]]

        trueSigma = [[1.99853994e-02, 2.45647716e-03, 8.45356279e-06]]

    elif testCase == "JitterSimple":
        truePos = [[-4.02553766e06, 7.48712857e06, 5.24933964e06]]

        trueSigma = [[1.98964221e-02, 2.24474932e-03, -5.66618270e-05]]

    elif testCase == "JitterFullyCoupled":
        truePos = [[-4.02085866e06, 7.49022306e06, 5.24840326e06]]
    ...
    initialRotEnergy = [[rotEnergy[int(len(rotEnergy) / 2) + 1, 1]]]
    ...
    if testCase == "BalancedWheels" or testCase == "JitterFullyCoupled":
        ...
        unitTestSupport.writeFigureLaTeX(
            "ChangeInOrbitalAngularMomentum" + testCase,
            "Change in Orbital Angular Momentum " + testCase,
            plt,
        ...
        plt.plot(
            orbEnergy[:, 0] * 1e-9,
            (orbEnergy[:, 1] - orbEnergy[0, 1]) / orbEnergy[0, 1],
        )
        plt.xlabel("Time (s)")
        plt.ylabel("Relative Difference")
        unitTestSupport.writeFigureLaTeX(
            "ChangeInOrbitalEnergy" + testCase,
            "Change in Orbital Energy " + testCase,
            plt,
            r"width=0.8\textwidth",
            path,
        )
        ...
        plt.ylabel("Relative Difference")
        unitTestSupport.writeFigureLaTeX(
            "ChangeInRotationalAngularMomentum" + testCase,
            "Change in Rotational Angular Momentum " + testCase,
            plt,
            r"width=0.8\textwidth",
            path,
        )
        ...
        plt.clf()
        plt.plot(
            rotEnergy[int(len(rotEnergy) / 2) + 1 :, 0] * 1e-9,
            (
        ...
        plt.ylabel("Relative Difference")
        unitTestSupport.writeFigureLaTeX(
            "ChangeInRotationalEnergy" + testCase,
            "Change in Rotational Energy " + testCase,
            plt,
            r"width=0.8\textwidth",
            path,
        )
    ...
    if testCase == "BOE":
        ...
        unitTestSupport.writeFigureLaTeX(
            "ReactionWheelBOEBodyRate",
            "Reaction Wheel BOE Body Rate",
            plt,
            r"width=0.8\textwidth",
            path,
        )
        ...
        unitTestSupport.writeFigureLaTeX(
            "ReactionWheelBOERWRate",
            "Reaction Wheel BOE RW Rate",
            plt,
            r"width=0.8\textwidth",
            path,
        )
    ...
    if testCase == "FrictionSpinDown" or testCase == "FrictionSpinUp":
        ...
        plt.ylabel("Body Rate (rad/s)")
        unitTestSupport.writeFigureLaTeX(
            "ReactionWheel" + testCase + "TestBodyRates",
            "Reaction Wheel " + testCase + " Test Body Rates",
            plt,
            r"width=0.8\textwidth",
            path,
        )
        ...
        plt.ylabel("Wheel Speed (rad/s)")
        unitTestSupport.writeFigureLaTeX(
            "ReactionWheel" + testCase + "TestWheelSpeed",
            "Reaction Wheel " + testCase + " Test Wheel Speed",
            plt,
            r"width=0.8\textwidth",
            path,
        )
        ...
        plt.xlim([-15, 15])
        unitTestSupport.writeFigureLaTeX(
            "ReactionWheel" + testCase + "TestFrictionTorque",
            "Reaction Wheel " + testCase + " Test Friction Torque",
            plt,
            r"width=0.8\textwidth",
            path,
        )
    ...
    if (
        ...
        for i in range(0, len(truePos)):
            ...
            if not unitTestSupport.isArrayEqualRelative(
                dataPos, truePos[i], 3, accuracy
            ):
                testFailCount += 1
                testMessages.append(
                    "FAILED: Reaction Wheel Integrated Test failed pos unit test"
                )

        for i in range(0, len(trueSigma)):
            # check a vector values
            if not unitTestSupport.isArrayEqualRelative(
                dataSigma, trueSigma[i], 3, accuracy
            ):
                testFailCount += 1
                testMessages.append(
                    "FAILED: Reaction Wheel Integrated Test failed attitude unit test"
                )
    ...
    if testCase == "BalancedWheels" or testCase == "JitterFullyCoupled":
        for i in range(0, len(initialOrbAngMom_N)):
            ...
            if not unitTestSupport.isArrayEqualRelative(
                ...
                testFailCount += 1
                testMessages.append(
                    "FAILED: Reaction Wheel Integrated Test failed orbital angular momentum unit test"
                )
        ...
        for i in range(0, len(initialRotAngMom_N)):
            ...
            if not unitTestSupport.isArrayEqualRelative(
                ...
                testFailCount += 1
                testMessages.append(
                    "FAILED: Reaction Wheel Integrated Test failed rotational angular momentum unit test"
                )
        ...
        for i in range(0, len(initialOrbEnergy)):
            ...
            if not unitTestSupport.isArrayEqualRelative(
                ...
                testFailCount += 1
                testMessages.append(
                    "FAILED: Reaction Wheel Integrated Test failed orbital energy unit test"
                )
        ...
        for i in range(0, len(initialRotEnergy)):
            ...
            if not unitTestSupport.isArrayEqualRelative(
                ...
                testFailCount += 1
                testMessages.append(
                    "FAILED: Reaction Wheel Integrated Test failed rotational energy unit test"
                )
    ...
    if testCase == "BOE":
        for i in range(5):
            if (
                abs(
                    (omegaBOE[i] - omegaData[int(timeBOE[i] / stepSize), 2])
                    / omegaBOE[i]
                )
                > accuracy
            ):
                testFailCount += 1
                testMessages.append(
                    "FAILED: Reaction Wheel Integrated Test failed BOE body rates unit test"
                )
            if (
                abs((thetaBOE[i] - thetaOut[int(timeBOE[i] / stepSize)]) / thetaBOE[i])
                > accuracy
            ):
                testFailCount += 1
                testMessages.append(
                    "FAILED: Reaction Wheel Integrated Test failed BOE theta unit test"
                )
            if (
                abs(
                    (wheelSpeedBOE[i] - wheelSpeeds[int(timeBOE[i] / stepSize)])
                    / wheelSpeedBOE[i]
                )
                > accuracy
            ):
                testFailCount += 1
                testMessages.append(
                    "FAILED: Reaction Wheel Integrated Test failed BOE wheel speed unit test"
                )
