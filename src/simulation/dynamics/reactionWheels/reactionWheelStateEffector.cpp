...
void
ReactionWheelStateEffector::linkInStates(DynParamManager& statesIn)
{
...
void
ReactionWheelStateEffector::registerStates(DynParamManager& states)
{
...
void
ReactionWheelStateEffector::updateEffectorMassProps(double integTime)
{
    ...
    for (std::size_t i = 0; i < ReactionWheelData.size(); ++i) {
        ...
        if (rw.RWModel == JitterFullyCoupled) {
            ...
            IRWPntWc_W << rw.Js, 0., rw.J13, 0., rw.Jt, 0., rw.J13, 0., rw.Jg;
            ...
            IPrimeRWPntWc_W << 0., -rw.J13, 0., -rw.J13, 0., 0., 0., 0., 0.;
            ...
            this->effProps.IEffPrimePntB_B += rw.IPrimeRWPntWc_B +
                                              rw.mass * rPrimeTildeWcB_B * rw.rTildeWcB_B.transpose() +
                                              rw.mass * rw.rTildeWcB_B * rPrimeTildeWcB_B.transpose();
            thetaCount++;
...
void
ReactionWheelStateEffector::updateContributions(double integTime,
                                                ...
                                                Eigen::Vector3d g_N)
{
    ...
    for (const auto& rwPtr : ReactionWheelData) {
        ...
        if (rw.frictionStribeck == 1) {
            frictionForce = sqrt(2.0 * exp(1.0)) * (rw.fStatic - rw.fCoulomb) *
                              exp(-(rw.Omega / rw.betaStatic) * (rw.Omega / rw.betaStatic) / 2.0) * rw.Omega /
                              (rw.betaStatic * sqrt(2.0)) +
                            rw.fCoulomb * tanh(rw.Omega * 10.0 / rw.betaStatic) + rw.cViscous * rw.Omega;
            frictionForceAtLimitCycle =
              sqrt(2.0 * exp(1.0)) * (rw.fStatic - rw.fCoulomb) *
                exp(-(rw.omegaLimitCycle / rw.betaStatic) * (rw.omegaLimitCycle / rw.betaStatic) / 2.0) *
                rw.omegaLimitCycle / (rw.betaStatic * sqrt(2.0)) +
              rw.fCoulomb * tanh(rw.omegaLimitCycle * 10.0 / rw.betaStatic) + rw.cViscous * rw.omegaLimitCycle;
        } else {
        ...
        if (rw.RWModel == BalancedWheels || rw.RWModel == JitterSimple) {
            ...
            backSubContr.vecRot -=
              rw.gsHat_B * (rw.u_current + rw.frictionTorque) + rw.Js * rw.Omega * omegaLoc_BN_B.cross(rw.gsHat_B);
            ...
            rw.bOmega = -1.0 / (rw.Js + rw.mass * dSquared) *
                        ((rw.Js + rw.mass * dSquared) * rw.gsHat_B + rw.J13 * rw.w3Hat_B +
                         rw.mass * rw.d * rw.rWB_B.cross(rw.w3Hat_B));
            rw.cOmega = 1.0 / (rw.Js + rw.mass * dSquared) *
                        (omegaw2 * omegaw3 * (-rw.mass * dSquared) - rw.J13 * omegaw2 * omegas -
                         rw.mass * rw.d * rw.w3Hat_B.transpose() * omegaLoc_BN_B.cross(omegaLoc_BN_B.cross(rw.rWB_B)) +
                         (rw.u_current + rw.frictionTorque) + rw.gsHat_B.dot(gravityTorquePntW_B));
            ...
            backSubContr.matrixC +=
              (rw.IRWPntWc_B * rw.gsHat_B + rw.mass * rw.d * rw.rWcB_B.cross(rw.w3Hat_B)) * rw.aOmega.transpose();
            backSubContr.matrixD +=
              (rw.IRWPntWc_B * rw.gsHat_B + rw.mass * rw.d * rw.rWcB_B.cross(rw.w3Hat_B)) * rw.bOmega.transpose();
            backSubContr.vecTrans += rw.mass * rw.d * (OmegaSquared * rw.w2Hat_B - rw.cOmega * rw.w3Hat_B);
            backSubContr.vecRot +=
              rw.mass * rw.d * OmegaSquared * rw.rWcB_B.cross(rw.w2Hat_B) - rw.IPrimeRWPntWc_B * rw.Omega * rw.gsHat_B -
              omegaLoc_BN_B.cross(rw.IRWPntWc_B * rw.Omega * rw.gsHat_B + rw.mass * rw.rWcB_B.cross(rw.rPrimeWcB_B)) -
              (rw.IRWPntWc_B * rw.gsHat_B + rw.mass * rw.d * rw.rWcB_B.cross(rw.w3Hat_B)) * rw.cOmega;
        }
    }
}

void
ReactionWheelStateEffector::computeDerivatives(double integTime,
        ...
        if (rw.RWModel == BalancedWheels || rw.RWModel == JitterSimple) {
            ...
            if (std::abs(OmegasDot(i, 0)) > this->maxWheelAcceleration) {
                bskLogger.bskLog(BSK_WARNING,
                                 "Excessive reaction wheel acceleration detected (%.2e rad/s^2). This may be caused by "
                                 "using unlimited torque (useMaxTorque=False) with a small spacecraft inertia. "
                                 "Consider using torque limits or increasing spacecraft inertia.",
                                 std::abs(OmegasDot(i, 0)));
...
void
ReactionWheelStateEffector::updateEnergyMomContributions(double integTime,
                                                         ...
                                                         Eigen::Vector3d omega_BN_B)
{
    ...
    for (const auto& rwPtr : ReactionWheelData) {
        ...
        if (rw.RWModel == BalancedWheels || rw.RWModel == JitterSimple) {
            ...
            rotEnergyContr +=
              1.0 / 2.0 * rw.Js * rw.Omega * rw.Omega + rw.Js * rw.Omega * rw.gsHat_B.dot(omegaLoc_BN_B);
        } else if (rw.RWModel == JitterFullyCoupled) {
            ...
            rotEnergyContr +=
              0.5 * omega_WN_B.transpose() * rw.IRWPntWc_B * omega_WN_B + 0.5 * rw.mass * rDot_WcB_B.dot(rDot_WcB_B);
 ...
 */
void
ReactionWheelStateEffector::addReactionWheel(std::shared_ptr<RWConfigPayload> NewRW)
{
 ...
 */
void
ReactionWheelStateEffector::Reset(uint64_t CurrenSimNanos)
{
 ...
 */
void
ReactionWheelStateEffector::WriteOutputMessages(uint64_t CurrentClock)
{
    for (std::size_t i = 0; i < ReactionWheelData.size(); ++i) {
        auto& rw = *ReactionWheelData[i];
        if (numRWJitter > 0) {
 ...
 */
void
ReactionWheelStateEffector::writeOutputStateMessages(uint64_t integTimeNanos)
{
    for (std::size_t i = 0; i < ReactionWheelData.size(); ++i) {
        auto& rw = *ReactionWheelData[i];
        if (numRWJitter > 0) {
 ...
 */
void
ReactionWheelStateEffector::ReadInputs()
{
 ...
 */
void
ReactionWheelStateEffector::ConfigureRWRequests(double CurrentTime)
{
    for (std::size_t i = 0; i < NewRWCmds.size(); ++i) {
        ...
        if (this->ReactionWheelData[i]->u_max > 0) {
            ...
            if (!warningIssued && std::abs(cmd.u_cmd) > this->largeTorqueThreshold) { // Threshold for "large" torque
                bskLogger.bskLog(
                  BSK_WARNING,
                  "Using unlimited reaction wheel torque (u_max <= 0). This can cause numerical instability with small "
                  "spacecraft inertia. Consider setting useMaxTorque=True or increasing spacecraft inertia.");
                warningIssued = true;
            }
        }
        ...
        if (this->ReactionWheelData[i]->P_max > 0) {
            if (std::abs(cmd.u_cmd * this->ReactionWheelData[i]->Omega) >= this->ReactionWheelData[i]->P_max) {
                cmd.u_cmd =
                  std::copysign(this->ReactionWheelData[i]->P_max / this->ReactionWheelData[i]->Omega, cmd.u_cmd);
            }
        }
        ...
        if (std::abs(this->ReactionWheelData[i]->Omega) >= this->ReactionWheelData[i]->Omega_max &&
            ...
            && this->ReactionWheelData[i]->Omega * cmd.u_cmd >=
 ...
 */
void
ReactionWheelStateEffector::UpdateState(uint64_t CurrentSimNanos)
{
