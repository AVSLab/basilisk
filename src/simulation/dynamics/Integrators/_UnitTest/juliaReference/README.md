...
Concretely, the time-dependent cases use ``SRIW1`` (drift nodes ``{0, 3/4, 0, 0}``) and
``SRA1`` (nodes ``{0, 3/4}`` / ``{1, 0}``), whose stage nodes are exact rationals. The
stability-optimized ``SOSRI`` and ``SOSRA`` are deliberately **not** given time-dependent
cases: their optimized tableaux have irrational stage nodes (e.g. ``SOSRI`` drift node
``3.7504…`` and ``SOSRA`` diffusion node ``0.041248…``), so a coefficient sampled at
``t_n + c_i*h`` would differ from Julia's exact value by the nanosecond-quantization
error and break the ``atol=1e-11`` match. Their time-*independent* behaviour is fully
covered; only the time-dependent equivalence check is infeasible at this tolerance for
them.
