...
@pytest.mark.parametrize(
    "integratorCase",
    ["rk4", "rkf45", "rkf78", "euler", "rk2", "rk3", "bogackiShampine"],
)
...
def run(doUnitTests, show_plots, integratorCase):
    ...
    if integratorCase == "rkf78":
        ...
        integratorObject = svIntegrators.svIntegratorRungeKutta(
            scObject,
            a_coefficients=[[0, 0, 0], [0.5, 0, 0], [-1, 2, 0]],
    ...
    if doUnitTests:
        ...
        if testFailCount == 0:
            ...
            passFailText = "FAILED"
            colorText = (
                "Red"  # color to write auto-documented "FAILED" message in in LATEX
            )
        ...
        unitTestSupport.writeTeXSnippet(snippetMsgName, snippetContent, path)
        snippetPassFailName = fileNameString + "TestMsg-" + integratorCase
        snippetContent = r"\textcolor{" + colorText + "}{" + passFailText + "}"
        unitTestSupport.writeTeXSnippet(snippetPassFailName, snippetContent, path)
...
if __name__ == "__main__":
    run(
        ...
        True,  # show_plots
        "bogackiShampine",
    )  # integrator case(0 - RK4, 1 - RKF45, 2 - Euler, 3 - RK2, 4 - RK3)
