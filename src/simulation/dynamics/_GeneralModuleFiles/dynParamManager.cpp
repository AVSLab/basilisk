...
void
StateVector::setStates(const StateVector& operand)
{
...
void
StateVector::addStates(const StateVector& operand)
{
    for (auto [it1, it2] = std::tuple{ std::begin(this->stateMap), std::begin(operand.stateMap) };
         it1 != std::end(this->stateMap) && it2 != std::end(operand.stateMap);
         ++it1, ++it2) {
...
void
StateVector::scaleStates(double scaleFactor)
{
...
void
StateVector::propagateStates(double dt)
{
    for (const auto& [key, value] : stateMap) {
...
void
DynParamManager::updateStateVector(const StateVector& newState)
{
    this->stateContainer.setStates(newState);
}

void
DynParamManager::propagateStateVector(double dt)
{
    this->stateContainer.propagateStates(dt);
}
