...
StateVecStochasticIntegrator::getStateIdToNoiseIndexMaps()
{
    ...
    // sharedNoiseUUID values are generated per DynParamManager, not globally.
    // Namespace them by dynamic object index to avoid cross-object collisions.
    std::map<std::pair<size_t, size_t>, size_t> sharedNoiseIndexToVectorIndex;
        ...
        -> StateIdToIndexMap&
    {
        ...
        std::pair<size_t, size_t> namespacedSharedNoiseId = {dynIndex, sharedNoiseUUID};

        if (sharedNoiseIndexToVectorIndex.count(namespacedSharedNoiseId) == 0)
        {
            ...
            sharedNoiseIndexToVectorIndex[namespacedSharedNoiseId] = stateIdToNoiseIndexMaps.size();
            return stateIdToNoiseIndexMaps.emplace_back();
        }

        // This noise source is shared, and the diffusion should be
        // placed at an map that was already created
        size_t vecIndex = sharedNoiseIndexToVectorIndex.at(namespacedSharedNoiseId);
