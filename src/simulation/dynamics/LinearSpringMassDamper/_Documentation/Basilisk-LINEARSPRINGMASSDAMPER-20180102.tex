...
\newcommand{\ModuleName}{linearSpringMassDamper}
\newcommand{\subject}{Linear Spring Mass Damper Dynamics Model}
\newcommand{\status}{Initial Draft}
\newcommand{\preparer}{C. Allard}
\newcommand{\summary}{The linear spring mass damper class is an instantiation of the state effector abstract class. The integrated test is validating the interaction between the linear spring mass damper module and the rigid body hub that it is attached to. There can be general number of linear spring mass damper particles attached to the spacecraft and can be oriented in any directions with the respect to the rigid body hub. This document outlines the analytical development of the model and describes the efforts completed to validate/verify the model.}
