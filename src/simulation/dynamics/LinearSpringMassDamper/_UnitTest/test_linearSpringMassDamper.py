...
def fuelSloshTest(show_plots, useFlag, testCase):
    ...
    scObject.hub.v_CN_NInit = [[0.1], [0.0 - 5], [0.3]]
    ...
    if testCase == "MassDepletion":
        stateLog = pythonVariableLogger.PythonVariableLogger(
            {
                "mass1": lambda _: scObject.dynManager.getStateObject(
                    "linearSpringMassDamperMass1"
                ).getState(),
                "mass2": lambda _: scObject.dynManager.getStateObject(
                    "linearSpringMassDamperMass2"
                ).getState(),
                "mass3": lambda _: scObject.dynManager.getStateObject(
                    "linearSpringMassDamperMass3"
                ).getState(),
            }
        )
    ...
    if testCase == "MassDepletion":
        ...
        mDotParicle1True = mDotFuel * (10.0 / 85.0)
        mDotParicle2True = mDotFuel * (20.0 / 85.0)
        mDotParicle3True = mDotFuel * (15.0 / 85.0)
        mDotParicle1Data = (mass1Out[2, 1] - mass1Out[1, 1]) / (
            (mass1Out[2, 0] - mass1Out[1, 0]) * 1e-9
        )
        mDotParicle2Data = (mass2Out[2, 1] - mass2Out[1, 1]) / (
            (mass2Out[2, 0] - mass2Out[1, 0]) * 1e-9
        )
        mDotParicle3Data = (mass3Out[2, 1] - mass3Out[1, 1]) / (
            (mass3Out[2, 0] - mass3Out[1, 0]) * 1e-9
        )
    ...
    if testCase != "MassDepletion":
        ...
        for i in range(0, len(initialOrbAngMom_N)):
            ...
            if not unitTestSupport.isArrayEqualRelative(
                ...
                testFailCount += 1
                testMessages.append(
                    "FAILED: Linear Spring Mass Damper unit test failed orbital angular momentum unit test"
                )
        ...
        for i in range(0, len(initialRotAngMom_N)):
            ...
            if not unitTestSupport.isArrayEqualRelative(
                ...
                testFailCount += 1
                testMessages.append(
                    "FAILED: Linear Spring Mass Damper unit test failed rotational angular momentum unit test"
                )
        ...
        if testCase == "Gravity" or testCase == "NoGravity":
            for i in range(0, len(initialRotEnergy)):
                ...
                if not unitTestSupport.isArrayEqualRelative(
                    ...
                    testFailCount += 1
                    testMessages.append(
                        "FAILED: Linear Spring Mass Damper unit test failed rotational energy unit test"
                    )
        ...
        for i in range(0, len(initialOrbEnergy)):
            ...
            if not unitTestSupport.isArrayEqualRelative(
                ...
                testFailCount += 1
                testMessages.append(
                    "FAILED: Linear Spring Mass Damper unit test failed orbital energy unit test"
                )

    if testCase == "MassDepletion":
        accuracy = 1e-4
        if not unitTestSupport.isDoubleEqual(
            mDotParicle1Data, mDotParicle1True, accuracy
        ):
            testFailCount += 1
            testMessages.append(
        ...
        if not unitTestSupport.isDoubleEqual(
            mDotParicle2Data, mDotParicle2True, accuracy
        ):
            testFailCount += 1
            testMessages.append(
        ...
        if not unitTestSupport.isDoubleEqual(
            mDotParicle3Data, mDotParicle3True, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: Linear Spring Mass Damper unit test failed mass 3 dot test"
            )
