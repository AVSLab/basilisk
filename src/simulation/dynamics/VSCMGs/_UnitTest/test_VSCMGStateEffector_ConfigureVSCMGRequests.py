...
def defaultVSCMG(VSCMG):
    ...
    VSCMG.gsHat0_B = [[1.0], [0.0], [0.0]]
    VSCMG.gtHat0_B = [[1.0], [0.0], [0.0]]
    VSCMG.ggHat_B = [[1.0], [0.0], [0.0]]
    VSCMG.w2Hat0_B = [[0.0], [1.0], [0.0]]
    VSCMG.w3Hat0_B = [[0.0], [0.0], [1.0]]
    VSCMG.theta = 0.0
    VSCMG.u_s_current = 0.0
    VSCMG.u_s_max = 0.0
    VSCMG.u_s_min = 0.0
    VSCMG.u_s_f = 0.0
    ...
    VSCMG.massW = 0.0
    VSCMG.massG = 0.0
    VSCMG.wheelLinearFrictionRatio = 0.0
    VSCMG.VSCMGModel = 0
    return
...
@pytest.mark.parametrize(
    "useFlag, testCase",
    [(False, "saturation"), (False, "minimum"), (False, "friction")],
)
...
def unitSimVSCMG(show_plots, useFlag, testCase):
    ...
    if testCase == "basic":
        ...
        VSCMGs[0].u_s_max = 1.0
        VSCMGs[1].u_s_max = 2.0
        VSCMGs[0].u_g_max = 1.0
        VSCMGs[1].u_g_max = 2.0
        u_s_cmd = [-1.2, 1.5]
        u_g_cmd = [-1.2, 1.5]
        writeNewVSCMGCmds(VSCMG, u_s_cmd, u_g_cmd, len(VSCMGs))

        expOut["u_s_current"] = [-1.0, 1.5]

    elif testCase == "minimum":
        VSCMGs[0].u_s_min = 0.1
        VSCMGs[1].u_s_min = 0.0
        VSCMGs[0].u_g_min = 0.1
        VSCMGs[1].u_g_min = 0.0
        u_s_cmd = [-0.09, 0.0001]
        u_g_cmd = [-0.09, 0.0001]
        writeNewVSCMGCmds(VSCMG, u_s_cmd, u_g_cmd, len(VSCMGs))

        expOut["u_s_current"] = [0.0, 0.0001]
        ...
        Omega_max = [100.0, 0.0]
        gammaDot = [-20.0, 0.0]
        gammaDot_max = [100.0, 0.0]
        wheelLinearFrictionRatio = [0.1, 0.0]
        gimbalLinearFrictionRatio = [0.1, 0.0]
        for i in range(0, numVSCMG):
    ...
    VSCMG.ConfigureVSCMGRequests(0.0)
    ...
    if testFail:
        testFailCount += 1
        testMessages.append(
            "FAILED: " + VSCMG.ModelTag + " Module failed " + outputName + " unit test"
        )
