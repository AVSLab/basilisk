...
def defaultVSCMG():
    ...
    VSCMG.IW1 = 100.0 / VSCMG.Omega_max  # 0.159154943092
...
def VSCMGIntegratedTest(show_plots, useFlag, testCase):
    ...
    if testCase == "BalancedWheels":
        truePos = [[-4025537.664298894, 7487128.570444949, 5249339.643828076]]

        trueSigma = [[0.0185829763256, 0.00212563436704, -0.00118728497031]]

    elif testCase == "JitterSimple":
        truePos = [[-4025537.659558947, 7487128.570662447, 5249339.653774626]]
        ...
        truePos = [[0.0970572658434, -0.195562924079, 0.191874379545]]

        trueSigma = [[0.000201909373901, 2.9217809421e-05, 4.00231302121e-06]]
    elif testCase == "JitterFullyCoupledGravity":
        truePos = [[-4020390.68802, 7490532.37502, 5248309.52745]]

        trueSigma = [[0.000201662012765, 2.92123940252e-05, 4.15606551702e-06]]
    ...
    for i in range(0, N):
        plt.subplot(4, 1, i + 1)
        plt.plot(
            speedLog.times() * 1.0e-9,
            wheelSpeeds[:, i] / (2.0 * math.pi) * 60,
            label="RWA" + str(i),
        )
        plt.xlabel("Time (s)")
    ...
    for i in range(0, N):
        plt.subplot(4, 1, i + 1)
        plt.plot(
            speedLog.times() * 1.0e-9, gimbalRates[:, i] * 180 / np.pi, label=str(i)
        )
        plt.xlabel("Time (s)")
    ...
    if testCase != "BalancedWheels" and testCase != "JitterFullyCoupledGravity":
        ...
        wheelSpeeds_data = np.array(frq[peakIdxs]) * 60.0
        wheelSpeeds_true = np.sort(
            abs(np.array([VSCMG.Omega / macros.RPM for VSCMG in VSCMGs]))
        )
        ...
        plt.xlim((0, VSCMGs[0].Omega_max / macros.RPM / 60.0))
    ...
    for i in range(0, len(truePos)):
        ...
        if not unitTestSupport.isArrayEqualRelative(
            dataPos[i], truePos[i], 3, accuracy
        ):
            testFailCount += 1
    ...
    for i in range(0, len(trueSigma)):
        # check a vector values
        if not unitTestSupport.isArrayEqualRelative(
            dataSigma[i], trueSigma[i], 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: VSCMG Integrated Test failed attitude unit test"
            )
    ...
    if testCase == "JitterSimple":
        for i in range(N):
            # check a vector values
            if (
                not abs(wheelSpeeds_data[i] - wheelSpeeds_true[i]) / wheelSpeeds_true[i]
                < 0.09
            ):
                testFailCount += 1
                testMessages.append(
                    "FAILED: VSCMG Integrated Test failed jitter unit test"
                )
    ...
    if testCase == "BalancedWheels" or testCase == "JitterFullyCoupled":
        for i in range(0, len(initialOrbAngMom_N)):
            ...
            if not unitTestSupport.isArrayEqualRelative(
                ...
                testFailCount += 1
                testMessages.append(
                    "FAILED: VSCMG Integrated Test failed orbital angular momentum unit test"
                )
        ...
        for i in range(0, len(initialRotAngMom_N)):
            ...
            if not unitTestSupport.isArrayEqualRelative(
                ...
                testFailCount += 1
                testMessages.append(
                    "FAILED: VSCMG Integrated Test failed rotational angular momentum unit test"
                )
        ...
        for i in range(0, len(initialOrbEnergy)):
            ...
            if not unitTestSupport.isArrayEqualRelative(
                ...
                testFailCount += 1
                testMessages.append(
                    "FAILED: VSCMG Integrated Test failed orbital energy unit test"
                )
        ...
        for i in range(0, len(initialRotEnergy)):
            ...
            if not unitTestSupport.isArrayEqualRelative(
                ...
                testFailCount += 1
                testMessages.append(
                    "FAILED: VSCMG Integrated Test failed rot energy unit test"
                )
