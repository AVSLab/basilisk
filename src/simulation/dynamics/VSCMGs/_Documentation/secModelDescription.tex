...
The key equations used to model the dynamics of VSCMGs are produced in a form that uses the Backsubstitution Method, a computationally efficient way to solve rigid body dynamics. The corresponding equations are highlighted below. A more thorough explanation and derivation of the equations of motion can be found in \href{http://hanspeterschaub.info/Papers/grads/JohnAlcorn.pdf}{Alcorn 2017}\cite{alcorn2017}.
...
The goal of Backsubstitution is to manipulate the rotational and translational equations of motion to conform to the following form,
\begin{equation}
\begin{bmatrix}
[A] & [B]\\
[C] & [D]
...
\subsubsection{Modified EOM for Backsubstitution}
To make use of Backsubstitution and define the Backsubstitution contribution matrices for a balanced VSCMG, the EOM must be arranged into the following form:
\begin{multline}
...
\subsubsection{Backsubstitution Contribution Matrices}
The balanced VSCMG Backsubstitution contribution matrices are given by,
\begin{gather}
...
This aforementioned translation equation of motion may be rewritten to conform with Backsubstitution requirements in the following way:
\begin{equation}
...
\subsubsection{Backsubstitution Matrices for Imbalanced VSCMG}
