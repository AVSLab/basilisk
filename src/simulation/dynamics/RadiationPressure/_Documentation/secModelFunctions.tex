...
\begin{itemize}
	...
	\item \textbf{Solar Eclipse}: The code takes solar eclipses into account via a "illumination factor". This illumination factor is output from the Basilisk solar eclipse module and can include the effects of multiple planets. It is applied to the force/torque outputs.
	...
	\item \textbf{Interface: Solar Eclipse}: The code receives solar eclipse (illumination factor) information via the Basilisk messaging system.
...
\begin{itemize}
	...
	\item \textbf{Eclipse}: The illumination factor applies a simple scaling factor to the output forces and torques. This assumes that all portions of the spacecraft are affected equally by the eclipse. This should, in most circumstances, be highly accurate. For exceptionally large $A_{\odot}$ spacecraft which also need highly accurate state calculations, this assumption could fail.
