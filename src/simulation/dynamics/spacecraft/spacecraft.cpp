 ...
 */
void
Spacecraft::Reset(uint64_t CurrentSimNanos)
{
...
void
Spacecraft::addStateEffector(StateEffector* newStateEffector)
{
...
void
Spacecraft::addDynamicEffector(DynamicEffector* newDynamicEffector)
{
...
void
Spacecraft::writeOutputStateMessages(uint64_t clockTime)
{
...
void
Spacecraft::UpdateState(uint64_t CurrentSimNanos)
{
...
void
Spacecraft::linkInStates(DynParamManager& statesIn)
{
 ...
 */
void
Spacecraft::updateSCMassProps(double time)
{
...
void
Spacecraft::equationsOfMotion(double integTimeSeconds, double timeStep)
{
    ...
    for (it = this->states.begin(); it != this->states.end(); it++) {
        ...
        (*it)->updateContributions(integTimeSeconds,
                                   this->backSubContributions,
                                   this->hubSigma->getState(),
                                   this->hubOmega_BN_B->getState(),
                                   *this->g_N);
    ...
    this->hub.hubBackSubMatrices.vecTrans +=
      -2.0 * (*this->m_SC)(0, 0) * omegaLocalBN_B.cross(cPrimeLocal_B) -
    ...
    this->hub.computeDerivatives(integTimeSeconds,
                                 this->hubV_N->getStateDeriv(),
                                 this->hubOmega_BN_B->getStateDeriv(),
                                 this->hubSigma->getState());
 ...
 */
void
Spacecraft::preIntegration(uint64_t integrateToThisTimeNanos)
{
 ...
 */
void
Spacecraft::postIntegration(uint64_t integrateToThisTimeNanos)
{
    ...
    this->dvAccum_CN_B += newDcm_NB.transpose() * (newV_CN_N - this->BcGravVelocity->getState());

    // - Find the accumulated DV of the body frame in the body frame
    this->dvAccum_BN_B += newDcm_NB.transpose() * (newV_BN_N - this->hubGravVelocity->getState());
    ...
    this->nonConservativeAccelpntB_B =
      (newDcm_NB.transpose() * (newV_BN_N - this->hubGravVelocity->getState())) / this->timeStep;
    ...
    if (fabs(this->timeStep) > 1e-10) {
        this->omegaDot_BN_B =
          (newOmega_BN_B - this->oldOmega_BN_B) / this->timeStep; // angular acceleration of B wrt N in the Body frame
    } else {
...
void
Spacecraft::computeEnergyMomentum(double time)
{
    ...
    this->hub.updateEnergyMomContributions(
      time, this->rotAngMomPntCContr_B, this->rotEnergyContr, this->hubOmega_BN_B->getState());
...
void
Spacecraft::calcForceTorqueFromStateEffectors(double time, Eigen::Vector3d omega_BN_B)
{
    // - Loop over stateEffectors to get their contributions to energy and momentum
    std::vector<StateEffector*>::iterator it;
    for (it = this->states.begin(); it != this->states.end(); it++) {
