...
def SCTranslation(show_plots):
    ...
    if not unitTestSupport.isArrayEqual(
        scObject.scStateOutMsg.read().r_BN_N,
        ...
        testFailCount += 1
        testMessages.append(
            "FAILED: SCHub Translation test failed init pos msg unit test"
        )
    if not unitTestSupport.isArrayEqual(
        scObject.scStateOutMsg.read().v_BN_N,
    ...
    unitTestSupport.writeFigureLaTeX(
        "scPlusChangeInOrbitalAngularMomentumTranslationOnly",
        "Change in Orbital Angular Momentum Translation Only",
        plt,
    ...
    plt.ylabel("Relative Difference")
    unitTestSupport.writeFigureLaTeX(
        "scPlusChangeInOrbitalEnergyTranslationOnly",
        "Change in Orbital Energy Translation Only",
        plt,
        r"width=0.8\textwidth",
        path,
    )
    ...
    truePos = [[-4072255.7737936215, 7456050.4649078, 5258610.029627514]]

    initialOrbAngMom_N = [[orbAngMom_N[0, 1], orbAngMom_N[0, 2], orbAngMom_N[0, 3]]]

    finalOrbAngMom = [[orbAngMom_N[-1, 1], orbAngMom_N[-1, 2], orbAngMom_N[-1, 3]]]

    initialOrbEnergy = [[orbEnergy[0, 1]]]

    finalOrbEnergy = [[orbEnergy[-1, 1]]]

    accuracy = 1e-10
    for i in range(0, len(truePos)):
        # check a vector values
        if not unitTestSupport.isArrayEqualRelative(
            moduleOutput[-1, :], truePos[i], 3, accuracy
        ):
            testFailCount += 1
    ...
    for i in range(0, len(initialOrbAngMom_N)):
        ...
        if not unitTestSupport.isArrayEqualRelative(
            ...
            testFailCount += 1
            testMessages.append(
                "FAILED: SCHub Translation test failed orbital angular momentum unit test"
            )
    ...
    for i in range(0, len(initialOrbEnergy)):
        ...
        if not unitTestSupport.isArrayEqualRelative(
            ...
            testFailCount += 1
            testMessages.append(
                "FAILED: SCHub Translation test failed orbital energy unit test"
            )
...
def SCTransAndRotation(show_plots):
    ...
    trueSigma = [[3.73034285e-01, -2.39564413e-03, 2.08570797e-01]]
    ...
    finalRotAngMom = [[rotAngMom_N[-1, 1], rotAngMom_N[-1, 2], rotAngMom_N[-1, 3]]]
    ...
    plt.ylabel("Relative Difference")
    unitTestSupport.writeFigureLaTeX(
        "scPlusChangeInOrbitalAngularMomentumTranslationAndRotation",
        "Change in Orbital Angular Momentum Translation And Rotation",
        plt,
        r"width=0.8\textwidth",
        path,
    )
    ...
    plt.ylabel("Relative Difference")
    unitTestSupport.writeFigureLaTeX(
        "scPlusChangeInOrbitalEnergyTranslationAndRotation",
        "Change in Orbital Energy Translation And Rotation",
        plt,
        r"width=0.8\textwidth",
        path,
    )
    ...
    unitTestSupport.writeFigureLaTeX(
        ...
        "Change in Rotational Angular Momentum Translation And Rotation",
        plt,
        r"width=0.8\textwidth",
        path,
    )
    ...
    plt.ylabel("Relative Difference")
    unitTestSupport.writeFigureLaTeX(
        "scPlusChangeInRotationalEnergyTranslationAndRotation",
        "Change in Rotational Energy Translation And Rotation",
        plt,
        r"width=0.8\textwidth",
        path,
    )
    ...
    for i in range(0, len(truePos)):
        # check a vector values
        if not unitTestSupport.isArrayEqualRelative(
            r_BN_NOutput[-1, :], truePos[i], 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Translation and Rotation Integrated test failed pos unit test"
            )

    for i in range(0, len(trueSigma)):
        # check a vector values
        if not unitTestSupport.isArrayEqualRelative(
            sigma_BNOutput[-1, :], trueSigma[i], 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Translation and Rotation Integrated test failed attitude unit test"
            )
    ...
    for i in range(0, len(initialOrbAngMom_N)):
        ...
        if not unitTestSupport.isArrayEqualRelative(
            ...
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Translation and Rotation Integrated test failed orbital angular momentum unit test"
            )
    ...
    for i in range(0, len(initialRotAngMom_N)):
        ...
        if not unitTestSupport.isArrayEqualRelative(
            ...
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Translation and Rotation Integrated test failed rotational angular momentum unit test"
            )
    ...
    for i in range(0, len(initialRotEnergy)):
        ...
        if not unitTestSupport.isArrayEqualRelative(
            ...
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Translation and Rotation Integrated test failed rotational energy unit test"
            )
    ...
    for i in range(0, len(initialOrbEnergy)):
        ...
        if not unitTestSupport.isArrayEqualRelative(
            ...
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Translation and Rotation Integrated test failed orbital energy unit test"
            )
...
def SCRotation(show_plots):
    ...
    h = numpy.dot(
        numpy.asarray(scObject.hub.IHubPntBc_B),
        numpy.asarray(scObject.hub.omega_BN_BInit).flatten(),
    )
    ...
    trueSigma = [[5.72693314e-01, 5.10734375e-01, -3.07377611e-01]]

    initialRotAngMom_N = [[numpy.linalg.norm(numpy.asarray(rotAngMom_N[0, 1:4]))]]

    finalRotAngMom = [[numpy.linalg.norm(numpy.asarray(rotAngMom_N[-1, 1:4]))]]
    ...
    sigmaAfterAnalytical = -sigmaGhost / numpy.dot(
        numpy.linalg.norm(numpy.asarray(sigmaGhost)),
        numpy.linalg.norm(numpy.asarray(sigmaGhost)),
    )
    ...
    for i in range(0, 5):
        ...
        omegaAnalyticalArray[0, i] = (
            -H
            / (1 + sigmaNorm**2) ** 2
            * (8 * sigma1 * sigma3 - 4 * sigma2 * (1 - sigmaNorm**2))
            / scObject.hub.IHubPntBc_B[0][0]
        )
        omegaAnalyticalArray[1, i] = (
            -H
            / (1 + sigmaNorm**2) ** 2
            * (8 * sigma2 * sigma3 + 4 * sigma1 * (1 - sigmaNorm**2))
            / scObject.hub.IHubPntBc_B[1][1]
        )
        omegaAnalyticalArray[2, i] = (
            -H
            / (1 + sigmaNorm**2) ** 2
            * (4 * (-(sigma1**2) - sigma2**2 + sigma3**2) + (1 - sigmaNorm**2) ** 2)
            / scObject.hub.IHubPntBc_B[2][2]
        )
    ...
    unitTestSupport.writeFigureLaTeX(
        "scPlusMRPs",
        "Attitude of Spacecraft in MRPs",
        plt,
        r"width=0.8\textwidth",
        path,
    )
    ...
    plt.clf()
    plt.plot(
        moduleOutput[index - 3 : index + 3, 0] * 1e-9,
        moduleOutput[index - 3 : index + 3, 1],
        "b",
    )
    plt.plot(
        moduleOutput[index - 1, 0] * 1e-9,
        moduleOutput[index - 1, 1],
        "bo",
        label="Basilisk " + r"$\sigma_{1,t-1}$",
    )
    plt.plot(
        moduleOutput[index, 0] * 1e-9,
        moduleOutput[index, 1],
        "ro",
        label="Basilisk " + r"$\sigma_{1,t}$",
    )
    plt.plot(
        moduleOutput[index, 0] * 1e-9,
        sigmaGhost[0],
        "ko",
        label="Basilisk " + r"$\sigma_{1,0}$",
    )
    plt.plot(
        [moduleOutput[index - 1, 0] * 1e-9, moduleOutput[index, 0] * 1e-9],
        [moduleOutput[index - 1, 1], sigmaGhost[0]],
        "--k",
    )
    axes = plt.gca()
    ...
    plt.ylabel("MRPs")
    unitTestSupport.writeFigureLaTeX(
        "scPlusMRPSwitching", "MRP Switching", plt, r"width=0.8\textwidth", path
    )
    plt.figure()
    plt.clf()
    plt.plot(
        rotAngMom_N[:, 0] * 1e-9, (rotAngMomMag - rotAngMomMag[0]) / rotAngMomMag[0]
    )
    plt.xlabel("Time (s)")
    plt.ylabel("Relative Difference")
    unitTestSupport.writeFigureLaTeX(
        "scPlusChangeInRotationalAngularMomentumRotationOnly",
        "Change in Rotational Angular Momentum Rotation Only",
        plt,
        r"width=0.8\textwidth",
        path,
    )
    ...
    unitTestSupport.writeFigureLaTeX(
        ...
        "Change in Rotational Energy Rotation Only",
        plt,
        r"width=0.8\textwidth",
        path,
    )
    ...
    plt.clf()
    plt.plot(
        omega_BNOutput[:, 0] * 1e-9,
        omega_BNOutput[:, 1],
        label=r"$\omega_1$" + " Basilisk",
    )
    plt.plot(
        omega_BNOutput[:, 0] * 1e-9,
        omega_BNOutput[:, 2],
        label=r"$\omega_2$" + " Basilisk",
    )
    plt.plot(
        omega_BNOutput[:, 0] * 1e-9,
        omega_BNOutput[:, 3],
        label=r"$\omega_3$" + " Basilisk",
    )
    plt.plot(
        timeArray * 1e-9, omegaAnalyticalArray[0, :], "bo", label=r"$\omega_1$" + " BOE"
    )
    plt.plot(
        timeArray * 1e-9, omegaAnalyticalArray[1, :], "go", label=r"$\omega_2$" + " BOE"
    )
    plt.plot(
        timeArray * 1e-9, omegaAnalyticalArray[2, :], "ro", label=r"$\omega_3$" + " BOE"
    )
    plt.xlabel("Time (s)")
    plt.ylabel("Angular Velocity (rad/s)")
    plt.legend(loc="lower right", numpoints=1, prop={"size": 6.5})
    unitTestSupport.writeFigureLaTeX(
        "scPlusBasiliskVsBOECalcForRotation",
        "Basilisk Vs BOE Calc For Rotation",
        plt,
        r"width=0.8\textwidth",
        path,
    )
    ...
    for i in range(0, len(trueSigma)):
        # check a vector values
        if not unitTestSupport.isArrayEqualRelative(
            moduleOutput[-1], trueSigma[i], 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Translation and Rotation Integrated test failed attitude unit test"
    ...
    for i in range(0, len(initialRotAngMom_N)):
        # check a vector values
        if not unitTestSupport.isArrayEqualRelative(
            finalRotAngMom[i], initialRotAngMom_N[i], 1, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Translation and Rotation Integrated test failed rotational angular momentum unit test"
    ...
    for i in range(0, len(initialRotEnergy)):
        ...
        if not unitTestSupport.isArrayEqualRelative(
            ...
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Rotation Integrated test failed rotational energy unit test"
            )
    ...
    for i in range(0, len(omegaAnalyticalArray)):
        # check a vector values
        if not unitTestSupport.isArrayEqualRelative(
            omegaArray[i], omegaAnalyticalArray[i], 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Rotation Integrated test Rotational BOE unit test"
            )
    ...
    if not unitTestSupport.isArrayEqualRelative(
        ...
        sigmaAfterAnalytical,
        1,
        accuracy,
    ):
        testFailCount += 1
        testMessages.append(
            "FAILED: Spacecraft Rotation Integrated test failed MRP Switching unit test"
        )
...
def SCTransBOE(show_plots):
    ...
    basiliskV = [
        v_BN_NOutput[int(t1 / timeStep), 1],
        v_BN_NOutput[int(t2 / timeStep), 1],
        v_BN_NOutput[int(t3 / timeStep), 1],
    ]
    basiliskX = [
        r_BN_NOutput[int(t1 / timeStep), 1],
        r_BN_NOutput[int(t2 / timeStep), 1],
        r_BN_NOutput[int(t3 / timeStep), 1],
    ]
    ...
    for i in range(0, 3):
        ...
        if abs((truthX[i] - basiliskX[i]) / truthX[i]) > accuracy:
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Translation BOE Integrated test failed pos unit test"
            )
    ...
    for i in range(0, 3):
        ...
        if abs((truthV[i] - basiliskV[i]) / truthV[i]) > accuracy:
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Translation BOE Integrated test failed velocity unit test"
            )
...
def SCPointBVsPointC(show_plots):
    ...
    plt.clf()
    plt.plot(
        r_CN_NOutput1[:, 0] * 1e-9,
        r_CN_NOutput1[:, 1],
        "k",
        label="Torque About Point C",
        linewidth=3.0,
    )
    plt.plot(
        r_CN_NOutput1[:, 0] * 1e-9,
    ...
    plt.plot(
        r_CN_NOutput2[:, 0] * 1e-9,
        r_CN_NOutput2[:, 1],
        "--c",
        label="Torque About Point B",
    )
    plt.plot(
        r_CN_NOutput2[:, 0] * 1e-9,
        r_CN_NOutput2[:, 2],
    ...
    accuracy = 1e-8
    if not unitTestSupport.isArrayEqualRelative(
        r_CN_NOutput1[-1, 1:4], r_CN_NOutput2[-1, 1:4], 3, accuracy
    ):
        testFailCount += 1
        testMessages.append(
            "FAILED: Spacecraft Point B Vs Point C test failed pos unit test"
        )

    if not unitTestSupport.isArrayEqualRelative(
        sigma_BNOutput1[-1, 1:4], sigma_BNOutput2[-1, 1:4], 3, accuracy
    ):
        testFailCount += 1
        testMessages.append(
            "FAILED: Spacecraft Point B Vs Point C test failed attitude unit test"
        )
...
def scOptionalRef(show_plots, accuracy):
    ...
    scObject.hub.sigma_BNInit = [[0.0], [0.0], [1.0]]
    scObject.hub.omega_BN_BInit = [[0.5], [-0.4], [0.7]]
    ...
    testFailCount, testMessages = unitTestSupport.compareArray(
        trueOmega, omegaOut, accuracy, "omega_BN_B", testFailCount, testMessages
    )
    testFailCount, testMessages = unitTestSupport.compareArray(
        truer_RN_N, r_RN_Out, accuracy, "r_RN_N", testFailCount, testMessages
    )
    testFailCount, testMessages = unitTestSupport.compareArray(
...
def scAccumDV():
    ...
    v_r = numpy.cross(
        numpy.array(scObject.hub.omega_BN_BInit).T, -numpy.array(scObject.hub.r_BcB_B).T
    )[0]
    truth_dataAccumDV_BN_B = numpy.zeros(3)
    for i in range(len(dataLog.times()) - 1):
        if not unitTestSupport.isArrayEqual(
            dataAccumDV_CN_B[i + 1], truth_dataAccumDV_CN_B, 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Point C Accumulated DV test failed pos unit test"
            )

        truth_dataAccumDV_BN_B += numpy.matmul(
            RigidBodyKinematics.MRP2C(dataLog.sigma_BN[i + 1]),
        ...
        if not unitTestSupport.isArrayEqual(
            dataAccumDV_BN_B[i + 1], truth_dataAccumDV_BN_B, 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Point B Accumulated DV test failed pos unit test"
            )

        if not unitTestSupport.isArrayEqual(
            dataAccumDV_CN_N[i + 1], truth_dataAccumDV_CN_N, 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Point C Accumulated DV in inertial frame test failed pos unit test"
            )
...
def scAccumDVExtForce():
    ...
    for i in range(len(dataLog.times())):
        ...
        if not unitTestSupport.isArrayEqual(
            dataAccumDV_CN_N[i], truth_dataAccumDV_CN_N, 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: Spacecraft Point C Accumulated DV with external force test failed unit test"
            )
