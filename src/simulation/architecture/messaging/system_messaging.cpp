...
void SystemMessaging::selectMessageBuffer(uint64_t bufferUse)
{
    ...
    {
        BSK_PRINT_NOWHERE(MSG_ERROR,"ERROR: You've attempted to access a message buffer that does not exist. Yikes.\n");
...
void SystemMessaging::SetNumMessages(uint64_t MessageCount)
{
    ...
    {
        BSK_PRINT_NOWHERE(MSG_ERROR,"ERROR: Received a request to set num messages for a NULL buffer.\n");
        return;
    }
    ...
    int64_t moduleID)
{
    ...
    {
        BSK_PRINT_NOWHERE(MSG_INFORMATION,"INFO: The message %s was created more than once.\n", MessageName.c_str());
        if(moduleID >= 0)
        {
    ...
    if(MessageName == "")
    {
        BSK_PRINT_NOWHERE(MSG_ERROR,"ERROR: Module ID: %lld tried to create a message without a name.  Please try again.\n", moduleID);
        CreateFails++;
        return(-1);
    }
    if(NumMessageBuffers <= 0)
    {
        BSK_PRINT_NOWHERE(MSG_ERROR,"ERROR: I can't create a message with zero buffers.  I refuse.\n");
        CreateFails++;
        return(-1);
    }
    if(NumMessageBuffers == 1)
    {
        BSK_PRINT_NOWHERE(MSG_WARNING,"WARNING: You created a message with only one buffer. This might compromise the message integrity. Watch out.\n");
    }
    ...
    {
        BSK_PRINT_NOWHERE(MSG_ERROR,"ERROR: Your name length for: %s. is too long, truncating name\n", MessageName.c_str());
        CreateFails++;
    ...
    if(NameLength > MAX_MESSAGE_SIZE)
    {
        BSK_PRINT_NOWHERE(MSG_ERROR,"ERROR: Your struct name length for: %s. is too long, truncating name\n", messageStruct.c_str());
        CreateFails++;
        NameLength = MAX_MESSAGE_SIZE;
    }
                                   ...
                                   uint64_t MsgSize, void *MsgPayload, int64_t moduleID)
{
    ...
    {
        BSK_PRINT_NOWHERE(MSG_ERROR, "ERROR: Received a write request for invalid message ID: %lld \n", MessageID);
        WriteFails++;
        return(false);
    }
    ...
    if(MsgHdr->previousPublisher != moduleID)
    {
        ...
        {
            BSK_PRINT_NOWHERE(MSG_ERROR, "ERROR: Received a write request from a module that doesn't publish for %s . You get nothing.\n",
    ...
    {
        BSK_PRINT_NOWHERE(MSG_ERROR, "ERROR: Received a write request that was incorrect size for: %s . You get nothing.\n",
                  MsgHdr->MessageName);
        WriteFails++;
        return(false);
    }
                                  ...
                                  *DataHeader, uint64_t MaxBytes, void *MsgPayload, int64_t moduleID, uint64_t CurrentOffset)
{
    if(MessageID >= GetMessageCount())
    {
        BSK_PRINT_NOWHERE(MSG_ERROR, "ERROR: Received a read request for invalid message ID: %lld \n", MessageID);
        ReadFails++;
        return(false);
    }
    MessageHeaderData* MsgHdr = FindMsgHeader(MessageID);
    ...
    if(accIt->accessList.find(moduleID) == accIt->accessList.end()
        && moduleID != -1)
    {
        BSK_PRINT_NOWHERE(MSG_WARNING, "WARNING: Message %s was read by module ID %lld who is not on access list.\n", MsgHdr->MessageName, moduleID);
    }
...
{
    uint64_t TotalMessageCount = GetMessageCount();
    BSK_PRINT_NOWHERE(MSG_INFORMATION, "INFORMATION: Number of Messages: %lld \n", TotalMessageCount);
    for(uint64_t i=0; i<TotalMessageCount; i++)
    {
        PrintMessageStats(i);
    }
...
void SystemMessaging::PrintMessageStats(uint64_t MessageID)
{
    ...
    if(MsgHdr == NULL)
    {
        BSK_PRINT_NOWHERE(MSG_ERROR, "ERROR: Received a print request for ID: %lld That ID is not valid.\n", MessageID);
        return;
    }
