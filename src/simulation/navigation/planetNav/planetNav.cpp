...
*/
void
PlanetNav::Reset(uint64_t CurrentSimNanos)
{
    ...
    if (this->PMatrix.size() != numStates * numStates) {
        bskLogger.bskLog(
          BSK_ERROR, "Your process noise matrix (PMatrix) is not 12*12. Size is %ld.  Quitting", this->PMatrix.size());
        return;
    }
 ...
 */
void
PlanetNav::writeOutputMessages(uint64_t CurrentSimNanos)
{
 ...
 */
void
PlanetNav::computeErrors(uint64_t CurrentSimNanos)
{
    double timeStep;
...
*/
void
PlanetNav::UpdateState(uint64_t CurrentSimNanos)
{
