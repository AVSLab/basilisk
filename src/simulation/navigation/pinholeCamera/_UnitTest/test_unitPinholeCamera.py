...
def test_visibility():
    ...
    FOVx = (
        38.17124015837933 * np.pi / 180
    )  # this is the horizontal FOV, check camera.FOVx
    FOVy = (
        29.094758030219015 * np.pi / 180
    )  # this is the vertical FOV, check camera.FOVy
    n_lmk = 10
    pos_lmk = np.array(
        [
            [-16 * 1e3, 0, 0],  # Image center
            [
                -16 * 1e3,
                18 * 1e3 * np.tan(FOVy / 2),
                0,
            ],  # Image corner (18 km = horizontal distance sc-lmk)
