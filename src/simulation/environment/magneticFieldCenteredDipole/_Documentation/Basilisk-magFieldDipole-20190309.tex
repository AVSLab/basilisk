...
\newcommand{\subject}{Magnetic Field Environment}
\newcommand{\status}{Released}
\newcommand{\preparer}{H. Schaub}
\newcommand{\summary}{The {\tt MagneticFieldCenteredDipole} class is used to calculate the magnetic field vector above a body using the centered dipole model.  This class is used to hold relevant planetary magnetic field properties to compute answers for a given set of spacecraft locations relative to a specified planet. Planetary parameters, including position and input message, are settable by the user. In a given simulation, each planet of interest should have only one magnetic field  model associated with it linked to the spacecraft in orbit about that body. }
