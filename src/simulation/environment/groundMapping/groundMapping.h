...
class GroundMapping : public SysModel
{
  ...
  public:
    double minimumElevation; //!< [rad] (optional) minimum elevation above the local horizon needed to see a spacecraft;
                             //!< defaults to 10 degrees equivalent.
    double maximumRange; //!< [m] (optional) Maximum slant range to compute access for; defaults to -1, which represents
                         //!< no maximum range.
  ...
  private:
    ...
    Eigen::Vector3d
      r_LP_P; //!< [m] Ground Location position vector relative to to planet origin vector in planet frame coordinates.
    Eigen::Vector3d
