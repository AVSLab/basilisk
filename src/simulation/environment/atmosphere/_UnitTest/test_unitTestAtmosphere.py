...
def run(show_plots, useDefault, useMinReach, useMaxReach, usePlanetEphemeris):
    ...
    testModule = atmosphere.Atmosphere()
    testModule.ModelTag = "AtmosphereModel"

    if useDefault:
        refBaseDens = 1.217;
        refScaleHeight = 8500.0
        refPlanetRadius = 6378.1366*1000
    else:
        simSetPlanetEnvironment.exponentialAtmosphere(testModule, "earth")
        refPlanetRadius = testModule.planetRadius
        refBaseDens = testModule.exponentialParams.baseDensity
        refScaleHeight = testModule.exponentialParams.scaleHeight
