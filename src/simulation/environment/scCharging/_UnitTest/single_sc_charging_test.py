...
def run_two_sc_case(scale_e=1.0, scale_i=1.0, show_plots=False, label="baseline"):
    ...
    sc0_state.r_BN_N = r_target
    sc0_msg = messaging.SCStatesMsg().write(sc0_state)

    # Servicer state
    sc1_state = messaging.SCStatesMsgPayload()
    sc1_state.r_BN_N = r_servicer
    sc1_msg = messaging.SCStatesMsg().write(sc1_state)

    module.addSpacecraft(sc0_msg)  # "target"
    module.addSpacecraft(sc1_msg)  # "servicer" with e-gun in C++
    ...
    v_rec0 = module.voltOutMsgs[0].recorder()  # target
    v_rec1 = module.voltOutMsgs[1].recorder()  # servicer
    ...
    sim.ConfigureStopTime(macros.sec2nano(5.0))
    sim.ExecuteSimulation()

    t = v_rec0.times() * macros.NANO2SEC
    v_target = np.copy(v_rec0.voltage)
    v_servicer = np.copy(v_rec1.voltage)
...
def test_spacecraft_charging_single_sc(show_plots=True):
    ...
    # 1) Baseline
    # phi_t_base, phi_s_base = run_two_sc_case(1.0, 1.0, show_plots=show_plots, label="baseline")
    phi_t_base, phi_s_base = run_two_sc_case( -100.0, 100.0, show_plots=True,
                                         label="baseline")
