...
Kps = ["0o", "4-", "5+"]
LTs = [0.00, 14.73]
z_offsets = [0.0, 3500e3]
r_EN_Ns = np.array([[0.0, 0.0, 0.0], [400e3, 300e3, -200e3]])


@pytest.mark.parametrize("accuracy", [1e2])
@pytest.mark.parametrize(
    "param1_Kp, param2_LT, param3_z, param4_r_EN",
    [
...
def test_dentonFluxModel(
    show_plots, param1_Kp, param2_LT, param3_z, param4_r_EN, accuracy
):
    r"""
    ...
    **Description of Variables Being Tested**
    ...
    """
    dentonFluxModelTestFunction(
        show_plots, param1_Kp, param2_LT, param3_z, param4_r_EN, accuracy
    )


def dentonFluxModelTestFunction(
    show_plots, param1_Kp, param2_LT, param3_z, param4_r_EN, accuracy
):
    """Test method"""
    ...
    LT = param2_LT
    angle = LT * 360.0 / 24.0 * np.pi / 180 - np.pi
    orbitRadius = 42000 * 1e3  # GEO orbit

    r_BE_N = np.array(
        [orbitRadius * math.cos(angle), orbitRadius * math.sin(angle), param3_z]
    )
    r_SE_N = np.array([149000000000.0, 0.0, -2.73 * param3_z])
    r_EN_N = param4_r_EN
    ...
    filename = (
        "FluxData_" + str(kpIndexCounter + 1) + "_" + str("%.2f" % param2_LT) + ".txt"
    )
    ...
    with open(filepath, "r") as file:
        ...
        trueElectronFluxData[0 : module.numOutputEnergies] = 10.0 ** (rows[1]) * 1e4
        trueIonFluxData[0 : module.numOutputEnergies] = 10.0 ** (rows[2]) * 1e4

    # make sure module output data is correct
    paramsString = " for Kp-Index={}, LT={}, accuracy={}".format(
        str(param1_Kp), str(param2_LT), str(accuracy)
    )
    ...
    ax = fig.gca()
    plt.semilogy(
        energyData[0 : module.numOutputEnergies],
        electronFluxData[0 : module.numOutputEnergies],
    )
