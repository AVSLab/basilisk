...
def test_range(show_plots):
    ...
    groundTarget.planetRadius = orbitalMotion.REQ_EARTH * 1000.0
    groundTarget.maximumRange = 100e3  # meters
    groundTarget.minimumElevation = np.radians(80.0)
    groundTarget.specifyLocation(np.radians(0.0), np.radians(0.0), 0.0)
    ...
    sc1_message.r_BN_N = [
        orbitalMotion.REQ_EARTH * 1e3 + 100e3,
        0,
        0,
    ]  # SC1 is in range
    ...
    sc3_message.r_BN_N = rbk.euler3(np.radians(11.0)).dot(
        np.array([100e3, 0, 0])
    ) + np.array([orbitalMotion.REQ_EARTH * 1e3, 0, 0])
    ...
    ref_elevation = [np.radians(90.0), np.radians(90.0), np.radians(79.0)]
    ref_access = [1, 0, 0]
...
def test_rotation(show_plots):
    ...
    groundTarget.minimumElevation = np.radians(10.0)
    groundTarget.specifyLocation(np.radians(0.0), np.radians(10.0), 0.0)
    scSim.AddModelToTask(simTaskName, groundTarget)

    #   Write out mock planet rotation, spacecraft position messages
    sc1_message = messaging.SCStatesMsgPayload()
    sc1_message.r_BN_N = np.array(
        [orbitalMotion.REQ_EARTH * 1e3 + 90e3, 0, 0]
    )  # SC1 is in range
    ...
    planet_message.J20002Pfix = rbk.euler3(np.radians(-10.0)).tolist()
...
def test_AzElR_rates():
    ...
    groundStation.minimumElevation = np.radians(60.0)
...
def plot_geometry(groundLocation, scLocations, minimumElevation):
    ...
    # draw a point0
    ax.scatter(
        groundLocation[0], groundLocation[1], groundLocation[2], color="r", s=100
    )
