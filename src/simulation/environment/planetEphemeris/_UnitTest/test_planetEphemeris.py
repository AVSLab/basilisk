...
def test_module(show_plots, setRAN, setDEC, setLST, setRate):
    ...
    expect_error = any([setRAN, setDEC, setLST, setRate]) and not all(
        [setRAN, setDEC, setLST, setRate]
    )
    with pytest.raises(BasiliskError) if expect_error else nullcontext():
        [testResults, testMessage] = planetEphemerisTest(
            show_plots, setRAN, setDEC, setLST, setRate
        )
...
def planetEphemerisTest(show_plots, setRAN, setDEC, setLST, setRate):
    ...
    mu = orbitalMotion.MU_SUN * 1000.0 * 1000.0 * 1000  # m^3/s^2
    ...
    if setRAN:
        ...
        RANlist = [0.0 * macros.D2R, 272.76 * macros.D2R]
    ...
    if setDEC:
        ...
        DEClist = [90.0 * macros.D2R, 67.16 * macros.D2R]
    ...
    if setLST:
        ...
        lstList = [10.0 * macros.D2R, 30.0 * macros.D2R]
    ...
    for c in range(0, len(planetNames)):
        ...
        if planet != FinalPlanetMessage.PlanetName:
            testFailCount += 1
            testMessages.append(
                "FAILED: planetEphemeris() didn't set the desired plane name " + planet
            )
        ...
        testFailCount, testMessages = unitTestSupport.compareArrayND(
            dcmTrue, J20002Pfix, accuracy, "DCM", 9, testFailCount, testMessages
        )
        testFailCount, testMessages = unitTestSupport.compareArrayND(
            dcmRateTrue,
            J20002Pfix_dot,
