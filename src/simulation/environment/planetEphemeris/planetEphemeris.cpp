...
void PlanetEphemeris::SelfInit()
{
    ...
    for(it = this->planetNames.begin(), c=0; it != this->planetNames.end(); it++, c++)
    {
        ...
        {
            bskPrint.printMessage(MSG_ERROR, "Warning, your planet name %s is too long.", (*it).c_str());
            continue;
        }
...
void PlanetEphemeris::Reset(uint64_t CurrenSimNanos)
{
    ...
    if (this->planetElements.size() != this->planetNames.size()) {
        bskPrint.printMessage(MSG_ERROR, "Only %lu planet element sets provided, but %lu plane names are present.",
    ...
    if (this->lst0.size() != this->planetNames.size()) {
        bskPrint.printMessage(MSG_ERROR, "Only %lu planet initial principal rotation angles provided, but %lu plane names are present.",
    ...
    if (this->rightAscension.size() != this->planetNames.size()) {
        bskPrint.printMessage(MSG_ERROR, "Only %lu planet right ascension angles provided, but %lu plane names are present.",
    ...
    if (this->declination.size() != this->planetNames.size()) {
        bskPrint.printMessage(MSG_ERROR, "Only %lu planet declination angles provided, but %lu plane names are present.",
                  this->rotRate.size(), this->planetNames.size());
        this->computeAttitudeFlag = 0;
    }
    ...
    if (this->rotRate.size() != this->planetNames.size()) {
        bskPrint.printMessage(MSG_ERROR, "Only %lu planet rotation rates provided, but %lu plane names are present.",
                  this->rotRate.size(), this->planetNames.size());
        this->computeAttitudeFlag = 0;
    }
