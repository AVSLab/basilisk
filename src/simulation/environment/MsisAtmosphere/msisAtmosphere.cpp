...
bool Atmosphere::ReadInputs()
{
    ...
    if(this->envType.compare(MODEL_MSISE)==0){
        ...
        for(int ind=0; ind < 23; ind++){
            if(this->swDataInMsgIds[ind] >= 0) {
                good_read = SystemMessaging::GetInstance()->ReadMessage(this->swDataInMsgIds[ind], &localHeader,
                        sizeof(SwDataSimMsg),
    ...
    return(planetRead && scRead && swRead);
}
    ...
    for(it = scStates.begin(); it != scStates.end(); it++, atmoMsgIt++){
        ...
        if(tmpAltitude > this->envMinReach &&
           (tmpAltitude < this->envMaxReach || this->envMaxReach < 0))
           {
            ...
            else if (this->envType.compare(MODEL_MSISE)==0){
                //! Update the SW indices and other input parameters for MSIS.
                runNrlmsise00Model(currentTime, &(*atmoMsgIt));
            }
        ...
        } // Check that the model si within an altitude bound
    }
...
void Atmosphere::runNrlmsise00Model(double currentTime, AtmoPropsSimMsg *msg)
{
