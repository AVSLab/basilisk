...
def test_unitEclipse(show_plots, eclipseCondition, planet):
    ...
    One of the two user-defined inputs is the eclipse identifier, appropriately named eclipseCondition. This specifies the eclipse type and can be set, via string inputs, to either full, partial, annular, or none for the full, partial, annular, and no eclipse cases. This input changes the spacecraft state parameters to position the spacecraft in the desired eclipse.

    Planet Input:
    The second user input is planet, which allows the user to test either Earth or Mars as the occulting body. Only these two planets are tested since there is no appreciable difference in the eclipse algorithm when varying the planet. This input changes the zero base to reference the chosen planet and the spacecraft state parameters to position the spacecraft in the desired eclipse.
    ...
    Planet Names:
    The eclipse module accepts and will analyze multiple planets at a time, so to indicate which planets are of interest, the names must be input. These are input as Spice objects of type string and can either be venus, earth, or mars barycenter. Spice uses this information and the time input to provide state data, which is then used to determine the closest planet. The closest planet is the only one further evaluated for potential eclipse conditions.

    Time:
    In order to specify the states of the planets and sun, time is a neccessary input. Only one time is used for every test, fixing the sun and planet states and varying the states of the spacecraft. For this test, the input "2021 MAY 04 07:47:49.965 (UTC)" is used, where the time is represented in UTC form.

    Spice Kernels:
    Spice information is gathered from a collection of kernels, specfically SPK, LSK, and PCK files. The binary SPK used is de430.bsp, which provides ephemeris data. The LSK file is naif0012.tls, which offers leapsecond information. For reference frame orientation, two PCK files are given. These are de-403-masses.tpc and pck00010.tpc.
