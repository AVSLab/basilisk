...
void Albedo::addInstrumentConfig(std::string instInMsgName, instConfig_t configMsg) {
    ...
    if (configMsg.fov < 0.0) {
        ...
        BSK_PRINT(MSG_WARNING, "Albedo Module (addInstrumentConfig): For the instrument (%s)'s half field of view angle (fov), the default value is used.", instInMsgName.c_str());
    } 
    else {
    ...
    if (!configMsg.nHat_B.isZero()) {
        ...
        BSK_PRINT(MSG_WARNING, "Albedo Module (addInstrumentConfig): For the instrument (%s)'s unit normal vector (nHat_B), the default vector is used.", instInMsgName.c_str());
    }
    return;
}
...
void Albedo::addInstrumentConfig(std::string instInMsgName, double fov, Eigen::Vector3d nHat_B, Eigen::Vector3d r_IB_B) {
    ...
    if (fov < 0.0) {
        this->fovs.push_back(this->fov_default);
        BSK_PRINT(MSG_WARNING, "Albedo Module (addInstrumentConfig): Instrument (%s)'s half field of view angle (fov) cannot be negative, the default value is used instead.", instInMsgName.c_str());
    }
    else {
    ...
    if (!nHat_B.isZero()) {
        ...
        this->nHat_Bs.push_back(nHat_B_default);
        BSK_PRINT(MSG_WARNING, "Albedo Module (addInstrumentConfig): Instrument (%s)'s unit normal vector (nHat_B) cannot be composed of all zeros, the default vector is used instead.", instInMsgName.c_str());
    }      
    return;
}
...
void Albedo::addPlanetandAlbedoAverageModel(std::string planetSpiceName)
{
    if (std::binary_search(this->planetInMsgNames.begin(), this->planetInMsgNames.end(), planetSpiceName + "_planet_data")) {
        BSK_PRINT(MSG_ERROR, "Albedo Module (addPlanetandAlbedoAverageModel): Same planet cannot be added more than once!");
        return;
    }
    else {
...
{
    if (std::binary_search(this->planetInMsgNames.begin(), this->planetInMsgNames.end(), planetSpiceName + "_planet_data")) {
        BSK_PRINT(MSG_ERROR, "Albedo Module (addPlanetandAlbedoDataModel): Same planet cannot be added more than once!");
        return;
    }
    else {
...
void Albedo::CrossInit()
{
    ...
    if (this->planetInMsgNames.empty()) {
        BSK_PRINT(MSG_ERROR, "Albedo Module (CrossInit): Planet message name vector (planetInMsgNames) is empty.");
    }
    else {
        int id = 0;
    ...
    if (this->sunPositionInMsgName != "") {
        ...
        BSK_PRINT(MSG_ERROR, "Albedo Module (CrossInit): Sun message name (sunPositionInMsgName) is empty.");
    }
    ...
    if (this->spacecraftStateInMsgName != "") {
        ...
        BSK_PRINT(MSG_ERROR, "Albedo Module (CrossInit): Spacecraft message name (spacecraftStateInMsgName) is empty.");
    }
    return;
}
...
void Albedo::Reset(uint64_t CurrentSimNanos)
{
    ...
    if (this->modelNames.empty()) {
        BSK_PRINT(MSG_ERROR, "Albedo Module (Reset): Albedo model was not set.");
        return;
    }
    int idx = 0;
    this->ALB.clear();
...
void Albedo::getPlanetRadius(std::string planetSpiceName)
{
    if (planetSpiceName == "mercury") {
        ...
        BSK_PRINT(MSG_ERROR, "Albedo Module (getPlanetRadius): The planet's radius cannot be obtained. The planet (%s) is not found.", planetSpiceName.c_str());
    }
    return;
}
...
double Albedo::getAlbedoAverage(std::string planetSpiceName)
{
    ...
    if (planetSpiceName == "mercury") {
        ...
        BSK_PRINT(MSG_ERROR, "Albedo Module (getAlbedoAverage): The average albedo value is not defined for the specified planet (%s).", planetSpiceName.c_str()); return 0.;
    }
}
...
void Albedo::evaluateAlbedoModel(int idx)
{
    ...
    if (modelName == "ALBEDO_AVG") {
        ...
        if (fileName == "") {
            BSK_PRINT(MSG_ERROR, "Albedo Module (evaluateAlbedoModel): Albedo fileName was not set.");
        }
        else {
            this->readFile = true;
        }
        ...
        BSK_PRINT(MSG_ERROR, "Albedo Module (evaluateAlbedoModel): Check the model name (%s).", modelName.c_str());
    }
    //! - Read the albedo coefficient file if the model requires
    if (this->readFile) {
        //! - Check that required module variables are set
        if (dataPath == "") {
            BSK_PRINT(MSG_ERROR, "Albedo Module (evaluateAlbedoModel): Albedo data path was not set.");
        }
        std::ifstream input(fileName);
        if (!input) {
            BSK_PRINT(MSG_ERROR, "Albedo Module (evaluateAlbedoModel): Albedo module is unable to load file %s", fileName.c_str());
        }
        ...
        if (!numLat || !numLon) {
            BSK_PRINT(MSG_ERROR, "Albedo Module (evaluateAlbedoModel): There has been an error in reading albedo data from (%s).", fileName.c_str());
        }
...
void Albedo::computeAlbedo(int idx, int instIdx, SpicePlanetStateSimMsg planetMsg, bool albArray, double outData[]) {
    ...
    if (this->altitudeRateLimit >=0 && alti_I / RA_planet > this->altitudeRateLimit) {
        BSK_PRINT(MSG_WARNING, "Albedo Module (computeAlbedo): The rate (altitude to planet's radii) limit is exceeded for the planet (%s) and albedo set to zero.", planetMsg.PlanetName);
        outData[0] = 0.0;
        outData[1] = 0.0;
        outData[2] = 0.0;
        outData[3] = 0.0;
