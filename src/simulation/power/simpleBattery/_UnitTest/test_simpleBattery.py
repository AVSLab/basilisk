...
def storage_limits(show_plots):
    ...
    **Validation Test Description**
    ...
    test_battery.storedCharge_Init = 5.0
    test_battery.storageCapacity = 10.0  # 10 W-s capacity.
    ...
    for ind in range(0, len(netPowerLog)):
        ...
        if currentPower != 10.0:
            testFailCount += 1
            testMessages.append(
                "FAILED: SimpleBattery did not correctly log the net power."
            )

    if not unitTestSupport.isDoubleEqualRelative((10.0), storedChargeLog[-1], 1e-8):
        testFailCount += 1
        testMessages.append(
            "FAILED: SimpleBattery did not track integrated power. Returned "
            + str(storedChargeLog[-1, 1])
            + ", expected "
            + str((10.0))
        )
    ...
    for ind in range(0, len(storedChargeLog)):
        if storedChargeLog[ind] > capacityLog[ind]:
            testFailCount += 1
            testMessages.append(
                "FAILED: SimpleBattery's stored charge exceeded its capacity."
            )

        if storedChargeLog[ind] < 0.0:
            testFailCount += 1
