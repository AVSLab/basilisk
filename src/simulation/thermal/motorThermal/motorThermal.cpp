...
MotorThermal::MotorThermal()
{
    ...
    this->ambientTemperature = 0.0;         //!< ambient temperature defaults to 0
...
void MotorThermal::Reset(uint64_t CurrentSimNanos)
{
    ...
    {
        bskLogger.bskLog(BSK_WARNING, "motorThermal.rwStateInMsg is not linked.");
    }
    ...
    {
        bskLogger.bskLog(BSK_WARNING, "motorThermal: current temperature is at or below absolute zero.");
    }
    ...
    {
        bskLogger.bskLog(BSK_WARNING, "motorThermal: current ambient thermal resistance is at or below zero, must be a positive number. The value has either not been set, or has not been set properly.");
    }

    //!< if the motor heat capacity is at or below zero, it means either the value wasn't set properly
    if (this->motorHeatCapacity <= 0)
    {
        bskLogger.bskLog(BSK_WARNING, "Current ambient heat capacity is at or below zero, must be a positive number. The value has either not been set, or has not been set properly.");
    }
