...
class Camera : public SysModel
{
  ...
  public:
    ...
    int ppMaxBlurSize{}; //!< Convolution kernel size of the bokeh filter, which determines the maximum radius of bokeh.
                         //!< It also affects the performance (the larger the kernel is, the longer the GPU time is
                         //!< required). Depth textures Value of 1 for Small, 2 for Medium, 3 for Large, 4 for Extra
