...
public:
    ...
    void hueShift(const cv::Mat, cv::Mat &mDst, std::vector<double>);
    void BGRAdjustPercent(const cv::Mat, cv::Mat &mDst, std::vector<int>);
...
public:
    ...
    std::vector<double> hsv;    //!< (int) HSV color correction, H (-180/180) hue shift, S and V are percent multipliers
    std::vector<int> bgrPercent;
