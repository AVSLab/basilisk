...
@pytest.mark.parametrize(
    "show_plots,   testCase,       stopTime,       procRate,   gyroLSBIn,      accelLSBIn,     senRotMaxIn,    senTransMaxIn,  senRotNoiseStd,     senTransNoiseStd,   errorBoundsGyroIn,  errorBoundsAccelIn, senRotBiasIn,   senTransBiasIn, accuracy",
    [
        (
            False,
            "clean",
...
def test_unitSimIMU(
    ...
    [testResults, testMessage] = unitSimIMU(
        show_plots,
        testCase,
        stopTime,
        procRate,
...
def unitSimIMU(
    ...
    # center of mass
    rDotDot_CN_N = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # acceleration of center of mass wrt inertial frame
    rDotDot_CB_N = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # acceleration of center of mass wrt body frame
    rDot_CN_N = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # velocity of center of mass wrt inertial frame
    r_CN_N = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # position of center of mass wrt to inertial frame
    # body frame
    rDotDot_BN_N = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # acceleration of body frame wrt to inertial
    rDot_BN_N = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # velocity of body frame wrt to inertial
    r_BN_N = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # position of body frame wrt to inertial
    omegaDot_BN_N = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # angular acceleration of body frame wrt to inertial
    omega_BN_N = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # angular rate of body frame wrt to inertial
    sigmaDot_BN = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # MRP derivative, body wrt to inertial
    sigma_BN = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # MRP body wrt to inertial
    # sensor
    rDotDot_SN_N = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # sensor sensed acceleration
    rDot_SN_N = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # sensor accumulated DV
    r_SN_N = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # sensor position in body frame coordinates
    ...
    for i in range(
        0, dataRows
    ):  # loops through acceleration vectors so that each element can be set individually (or, hopefully, as a function)
        rDotDot_BN_N[i][0] = 1.0
        rDotDot_BN_N[i][1] = 1.0
        rDotDot_BN_N[i][2] = 1.0
    ...
    r_BN_N[0][:] = [10000.0, 10000.0, 10000.0]  # leaves r_CN_N[0][i] with some offset
    # Body Rotation
    ...
    r_SB_B = np.array(
        [1.0, 1.0, 2.0]
    )  # constant. sensor position wrt to body frame origin
    ...
    cPrimePrime_N = (
        cDotDot_N
        - np.dot(2, np.cross(omega_BN_N[0][:], cPrime_N))
        - np.cross(omegaDot_BN_N[0][:], c_N)
        - np.cross(omega_BN_N[0][:], np.cross(omega_BN_N[0][:], c_N))
    )
    dcm_BN = rbk.MRP2C(sigma_BN[0][:])
    dcm_NB = np.transpose(dcm_BN)
    sigmaDot_BN[0][:] = findSigmaDot(
        sigma_BN[0][:], np.dot(dcm_BN, omega_BN_N[0][:])
    )  # sigmaDot_BN
    ...
    rDotDot_SN_N[0][:] = (
        ...
        - cPrimePrime_N
        - np.dot(2, np.cross(omega_BN_N[0][:], cPrime_N))
        + np.cross(omegaDot_BN_N[0][:], r_SC_N)
        + np.cross(omega_BN_N[0][:], np.cross(omega_BN_N[0][:], r_SC_N))
    )
    ...
    ImuSensor.sensorPos_B = np.array(
        r_SB_B
    )  # must be set by user - no default. check if this works by giving an array - SJKC
    ...
    setRandomWalk(
        ImuSensor, senRotNoiseStd, senTransNoiseStd, errorBoundsGyro, errorBoundsAccel
    )
    ...
    accelScale = [2.0, 2.0, 2.0]
    gyroScale = [1.0, 1.0, 1.0]
    ImuSensor.accelScale = np.array(accelScale)
    ImuSensor.gyroScale = np.array(gyroScale)

    accel_SN_P_disc = np.array([0.0, 0.0, 0.0])
    omega_SN_P_disc = np.array([0.0, 0.0, 0.0])

    # Set-up the sensor output truth vectors
    rDotDot_SN_P = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # sensor sensed acceleration in sensor platform frame coordinates
    rDotDot_SN_P[0][:] = np.dot(dcm_PN, rDotDot_SN_N[0][:])
    DVAccum_SN_P = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # sensor accumulated delta V ouput in the platform frame
    stepPRV_PN = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # principal rotatation vector from time i-1 to time i in platform frame coordinates
    omega_PN_P = np.resize(
        np.array([0.0, 0.0, 0.0]), (int(stopTime / unitProcRate_s + 1), 3)
    )  # angular rate omega_BN_P = omega_PN_P
    ...
    StateCurrent.TotalAccumDV_BN_B = np.array([0.0, 0.0, 0.0])
    ...
    for i in range(1, int(stopTime / dt) + 1):
        ...
        omega_BN_N[i][:] = (
            omega_BN_N[i - 1][:]
            + ((omegaDot_BN_N[i - 1][:] + omegaDot_BN_N[i][:]) / 2) * dt
        )
        ...
        for j in range(0, 10):  # Seems to converge after a few iterations
            sigma_BN[i][:] = (
                sigma_BN[i - 1][:]
                + ((sigmaDot_BN[i - 1][:] + sigmaDot_BN[i][:]) / 2) * dt
            )
            dcm_BN_2 = rbk.MRP2C(sigma_BN[i][:])
            sigmaDot_BN[i][:] = findSigmaDot(
                sigma_BN[i][:], np.dot(dcm_BN_2, omega_BN_N[i][:])
            )
        sigma_BN[i][:] = (
            sigma_BN[i - 1][:] + ((sigmaDot_BN[i - 1][:] + sigmaDot_BN[i][:]) / 2) * dt
        )
        dcm_BN_2 = rbk.MRP2C(sigma_BN[i][:])
        ...
        rDot_CN_N[i][:] = (
            rDot_CN_N[i - 1][:]
            + ((rDotDot_CN_N[i - 1][:] + rDotDot_CN_N[i][:]) / 2) * dt
        )
        r_CN_N[i][:] = (
            r_CN_N[i - 1][:] + ((rDot_CN_N[i - 1][:] + rDot_CN_N[i][:]) / 2) * dt
        )

        rDot_BN_N[i][:] = (
            rDot_BN_N[i - 1][:]
            + ((rDotDot_BN_N[i - 1][:] + rDotDot_BN_N[i][:]) / 2) * dt
        )
        r_BN_N[i][:] = (
            r_BN_N[i - 1][:] + ((rDot_BN_N[i - 1][:] + rDot_BN_N[i][:]) / 2) * dt
        )
        ...
        cPrimePrime_N = (
            cDotDot_N
            - np.dot(2, np.cross(omega_BN_N[i][:], cPrime_N))
            - np.cross(omegaDot_BN_N[i][:], c_N)
            - np.cross(omega_BN_N[i][:], np.cross(omega_BN_N[i][:], c_N))
        )
        ...
        rDotDot_SN_N[i][:] = (
            ...
            - cPrimePrime_N
            - np.dot(2, np.cross(omega_BN_N[i][:], cPrime_N))
            + np.cross(omegaDot_BN_N[i][:], r_SC_N)
            + np.cross(omega_BN_N[i][:], np.cross(omega_BN_N[i][:], r_SC_N))
        )
        rDot_SN_N[i][:] = (
            rDot_CN_N[i][:] - cPrime_N + np.cross(omega_BN_N[i][:], r_SC_N)
        )
        ...
        rDotDot_SN_P[i][:] = np.multiply(
            np.dot(dcm_PN_2, rDotDot_SN_N[i][:]) + senTransBiasIn, accelScale
        )  # This should match trueValues.AccelPlatform
        # accumulated delta v (non-conservative) in platform frame
        DVAccum_SN_P[i][:] = np.multiply(
            np.dot(dcm_PN_2, rDot_SN_N[i][:] - rDot_SN_N[i - 1][:])
            + senTransBiasIn * dt,
            accelScale,
        )
        ...
        stepPRV_PN[i][:] = np.multiply(
            rbk.MRP2PRV(rbk.C2MRP(dcm_PN_21)) + senRotBiasIn * dt, gyroScale
        )

        # angular rate in platform frame
        omega_PN_P[i][:] = np.multiply(
            np.dot(dcm_PN_2, omega_BN_N[i][:]) + senRotBiasIn, gyroScale
        )
        #
        # #discretization
        if accelLSBIn > 0.0:
            for k in [0, 1, 2]:
                accel_SN_P_disc[k] = (
                    np.floor(np.abs(rDotDot_SN_P[i][k] / accelLSBIn))
                    * accelLSBIn
                    * np.sign(rDotDot_SN_P[i][k])
                )
        ...
        if gyroLSBIn > 0.0:
            for k in [0, 1, 2]:
                omega_SN_P_disc[k] = (
                    np.floor(np.abs(omega_PN_P[i][k] / gyroLSBIn))
                    * gyroLSBIn
                    * np.sign(omega_PN_P[i][k])
                )
        ...
        StateCurrent.TotalAccumDV_BN_B = np.dot(
            dcm_BN_2, rDot_BN_N[i][:] - rDot_BN_N[0][:]
        )
    ...
    unitTestSupport.writeFigureLaTeX(
        testCase + "PRVcomparison",
        "Plot Comparing Time Step PRV Truth and Output for test: "
        + testCase
        + ". Note that 1, 2, and 3 indicate the components of the principal rotation vector.",
        plt,
    ...
    myLegend.get_frame().set_facecolor("#909090")
    unitTestSupport.writeFigureLaTeX(
        testCase + "omegaComparison",
        "Plot Comparing Angular Rate Truth and Output for test: "
        + testCase
        + ". Note that 1, 2, and 3 indicate the components of the angular rate.",
        plt,
        "height=0.7\\textwidth, keepaspectratio",
        path,
    )
    ...
    myLegend.get_frame().set_facecolor("#909090")
    unitTestSupport.writeFigureLaTeX(
        testCase + "accelComparison",
        "Plot Comparing Sensor Linear Accelertaion Truth and Output for test: "
        + testCase
        + ". Note that 1, 2, and 3 indicate the components of the acceleration.",
        plt,
        "height=0.7\\textwidth, keepaspectratio",
        path,
    )
    ...
    unitTestSupport.writeFigureLaTeX(
        ...
        "Plot Comparing Time Step DV Truth and Output for test: "
        + testCase
        + ". Note that 1, 2, and 3 indicate the components of the velocity delta.",
        plt,
        "height=0.7\\textwidth, keepaspectratio",
        path,
    )
    ...
    if testCase != "noise":
        ...
        for i, (actual, expected, name) in enumerate(
            [
                ...
                (
                    np.std(DVoutNoise[:, 0]),
                    senTransNoiseStd * dt * accelScale[0],
                    "DVnoise1",
                ),
                (
                    np.std(DVoutNoise[:, 1]),
                    senTransNoiseStd * dt * accelScale[1],
                    "DVnoise2",
                ),
                (
                    np.std(DVoutNoise[:, 2]),
                    senTransNoiseStd * dt * accelScale[2],
                    "DVnoise3",
                ),
                (
                    np.std(rDotDotOutNoise[:, 0]),
                    senTransNoiseStd * accelScale[0],
                    "AccelNoise1",
                ),
                (
                    np.std(rDotDotOutNoise[:, 1]),
                    senTransNoiseStd * accelScale[1],
                    "AccelNoise2",
                ),
                (
                    np.std(rDotDotOutNoise[:, 2]),
                    senTransNoiseStd * accelScale[2],
                    "AccelNoise3",
                ),
