...
@pytest.mark.parametrize(
    "useFlag, testCase", [(False, "basic"), (False, "noise"), (False, "walk bounds")]
)
...
def unitSimStarTracker(show_plots, useFlag, testCase):
    ...
    if testCase == "basic":
        ...
        trueVector["qInrtl2Case"] = listStack(
            rbk.MRP2EP(sigma), simStopTime, unitProcRate
        )
        trueVector["timeTag"] = np.arange(
            0, 0 + simStopTime * 1e9, unitProcRate_s * 1e9
        )
        ...
        dt = unitProcRate_s
        adjustedNoiseStd = (
            noiseStd  # Remove sqrt(dt) scaling since it's handled internally
        )
    ...
    if testCase == "noise":
        for i in range(0, 3):
            ...
            if (
                ...
                or np.abs(std_val - expected_std) / expected_std > std_threshold
            ):
                testFail = True
                testMessages.append(
                    f"Axis {i} failed: mean={mean_val:.6f}, std={std_val:.6f}"
                )
                break
        ...
        for i in range(0, 3):
            ...
            if (
                np.max(np.abs(np.asarray(moduleOutput2[i])))
        ...
        for i in range(0, len(trueVector["qInrtl2Case"])):
            if not unitTestSupport.isArrayEqual(
                moduleOutput[i], trueVector["qInrtl2Case"][i], 3, accuracy
            ):
                testFail = True
                break
    ...
    if testFail:
        testFailCount += 1
        testMessages.append(
            "FAILED: " + StarTracker.ModelTag + " Module failed unit test"
        )
