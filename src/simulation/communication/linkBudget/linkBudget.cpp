...
LinkBudget::LinkBudget()
{
    ...
    this->P_Rx1             = {};       // [W]  Received power at antenna 1
    ...
    this->oxygenLookupFilePath = {};    // [-]  Optional oxygen lookup table JSON file path override
    this->waterVaporLookupFilePath = {}; // [-] Optional water-vapor lookup table JSON file path override
...
void LinkBudget::initialization()
{
    ...
    if (this->atmosAtt && this->gndAntPnt != nullptr) {
        ...
        double h0Raw = sqrt(this->gndAntPnt->r_AP_N[0]*this->gndAntPnt->r_AP_N[0] +
                            this->gndAntPnt->r_AP_N[1]*this->gndAntPnt->r_AP_N[1] +
                            this->gndAntPnt->r_AP_N[2]*this->gndAntPnt->r_AP_N[2]) - REQ_EARTH *1e3;
        // Guard against tiny negative altitude due to Earth radius constant mismatch and floating-point roundoff.
        this->h_0 = std::max(0.0, h0Raw);
...
void LinkBudget::setOxygenLookupFilePath(const std::string& filePath)
{
    this->oxygenLookupFilePath = filePath;
}

void LinkBudget::setWaterVaporLookupFilePath(const std::string& filePath)
{
    this->waterVaporLookupFilePath = filePath;
}
...
void LinkBudget::calculateAtmosphericLoss()
{
    ...
    cosSlant_i              = std::clamp(cosSlant_i, -1.0, 1.0);
...
/* Slant angle calculation at ground antenna location (helper function for atmospheric attenuation calculation)
*/
...
Eigen::Vector2d LinkBudget::getPointingError(Eigen::Vector3d r_A1N_N, Eigen::Vector3d r_A2N_N, Eigen::MRPd sigma_AN1)
{
    ...
    Eigen::Vector3d r_A2A1_N  = r_A2N_N - r_A1N_N;
    double deltaNorm          = r_A2A1_N.norm();
    if (deltaNorm <= std::numeric_limits<double>::epsilon()) {
    ...
    double elevationError     = std::atan2(n_A2A1_A1[1], boresightComponent);  // [rad] Elevation pointing error
...
{
    // [Hz] lower frequency (take the highest lower frequency)
    double f_low     = std::max(this->antennaIn_1.frequency - this->antennaIn_1.B/2.0,
                                this->antennaIn_2.frequency - this->antennaIn_2.B/2.0);
    // [Hz] upper frequency (take the lowest upper frequency)
    double f_high    =  std::min(this->antennaIn_1.frequency + this->antennaIn_1.B/2.0,
                                 this->antennaIn_2.frequency + this->antennaIn_2.B/2.0);
    this->B_overlap  = f_high - f_low;          // [Hz] Overlapping bandwidth
    this->centerfreq = (f_high + f_low) / 2.0; // [Hz] Update center frequency of the overlapping bandwidth
    ...
    if (this->B_overlap < smaller_bandwidth && this->B_overlap > 0.0) {
        ...
        this->L_freq = 10.0 * std::log10(smaller_bandwidth / this->B_overlap); // [dB] Frequency offset loss
        this->linkValid = true;
        return;
...
void LinkBudget::generateLookupTable(LinkBudgetTypes::GasType gasType, LookupTable* lookupTable)
{
    ...
    if (gasType == LinkBudgetTypes::GasType::OXYGEN) {
        filename = this->oxygenLookupFilePath.empty() ? "supportData/AtmRadioFreqDampData/oxygen.json" : this->oxygenLookupFilePath;
    ...
    if (!std::filesystem::exists(filePath) && filePath.is_relative()) {
        std::filesystem::path repoRoot = std::filesystem::path(__FILE__).parent_path();
        for (int i = 0; i < 4; ++i) {
...
{
    // Clear all cached layer data so repeated Reset() calls rebuild a consistent table.
    attenuationLookup->layer_i.clear();
    attenuationLookup->gamma_i.clear();
    attenuationLookup->delta_h_layer_i.clear();
    attenuationLookup->n_i.clear();
    ...
    while (h_i < this->h_Toa) {
        ...
        m_i     = (std::exp(2.0/100.0) - std::exp(1.0/100.0)) /                                // (ITU-R P.676-13 eq 16c) Weighting factor for layer i
                  (std::exp(static_cast<double>(i)/100.0) - std::exp(static_cast<double>(i-1)/100.0));
...
void LinkBudget::calculateCNR()
{
    ...
    // Calculate the received power P_Rx for the receiver
    // Check which antenna is the receiver
    if (ant1_rx && ant2_tx) {
        // Calculate the noise power for antenna 1
        // P_Rx_dBW = EIRP_dBW + G_Rx_dB - L_FSPL - L_atm - L_point - L_freq
