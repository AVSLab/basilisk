...
def test_default_noise_is_white():
    ...
    assert abs(np.std(errors, ddof=1) / NOISE_STD - 1.0) < 0.07
    assert abs(np.corrcoef(errors[:-1], errors[1:])[0, 1]) < 0.07
...
def test_explicit_random_walk_is_bounded():
    ...
    sensor, errors = run_sensor(1000, seed=1234, propagationFactor=1.0, walkBound=bound)
