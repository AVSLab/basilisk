...
class PrescribedLinearTranslation : public SysModel
{
  public:
    ...
    void setSmoothingDuration(
      const double smoothingDuration); //!< Setter method for the duration the acceleration is smoothed until reaching
                                       //!< the given maximum acceleration value
    ...
    double getSmoothingDuration() const; //!< Getter method for the duration the acceleration is smoothed until reaching
                                         //!< the given maximum acceleration value
    ...
    ReadFunctor<LinearTranslationRigidBodyMsgPayload>
      linearTranslationRigidBodyInMsg; //!< Input msg for the translational reference position and velocity
    Message<LinearTranslationRigidBodyMsgPayload>
      linearTranslationRigidBodyOutMsg; //!< Output msg for the translational reference position and velocity
    ...
    PrescribedTranslationMsg_C
      prescribedTranslationOutMsgC = {}; //!< C-wrapped Output msg for the translational body prescribed states

    BSKLogger* bskLogger; //!< BSK Logging

  private:
    /* Methods for computing the required translation parameters */
    void computeTranslationParameters(); //!< Intermediate method to group the calculation of translation parameters
                                         //!< into a single method
    void computeBangBangParametersNoSmoothing(); //!< Method for computing the required parameters for the non-smoothed
                                                 //!< bang-bang profiler option
    void computeBangCoastBangParametersNoSmoothing(); //!< Method for computing the required parameters for the
                                                      //!< non-smoothed bang-coast-bang profiler option
    void computeSmoothedBangBangParameters(); //!< Method for computing the required parameters for the translation with
                                              //!< a smoothed bang-bang acceleration profile
    void computeSmoothedBangCoastBangParameters(); //!< Method for computing the required parameters for the smoothed
                                                   //!< bang-coast-bang option

    /* Methods for computing the current translational states */
    void computeCurrentState(double time); //!< Intermediate method used to group the calculation of the current
                                           //!< translational states into a single method
    bool isInFirstBangSegment(
      double time) const; //!< Method for determining if the current time is within the first bang segment
    bool isInSecondBangSegment(
      double time) const; //!< Method for determining if the current time is within the second bang segment
    bool isInFirstSmoothedSegment(double time) const;  //!< Method for determining if the current time is within the
                                                       //!< first smoothing segment for the smoothed profiler options
    bool isInSecondSmoothedSegment(double time) const; //!< Method for determining if the current time is within the
                                                       //!< second smoothing segment for the smoothed profiler options
    bool isInThirdSmoothedSegment(double time) const;  //!< Method for determining if the current time is within the
                                                       //!< third smoothing segment for the smoothed profiler options
    bool isInFourthSmoothedSegment(
      double time) const; //!< Method for determining if the current time is within the fourth smoothing segment for the
                          //!< smoothed bang-coast-bang option
    bool isInCoastSegment(
      double time) const; //!< Method for determining if the current time is within the coast segment
    void computeFirstBangSegment(
      double time); //!< Method for computing the first bang segment scalar translational states
    void computeSecondBangSegment(
      double time); //!< Method for computing the second bang segment scalar translational states
    void computeFirstSmoothedSegment(double time);  //!< Method for computing the first smoothing segment scalar
                                                    //!< translational states for the smoothed profiler options
    void computeSecondSmoothedSegment(double time); //!< Method for computing the second smoothing segment scalar
                                                    //!< translational states for the smoothed profiler options
    void computeThirdSmoothedSegment(double time);  //!< Method for computing the third smoothing segment scalar
                                                    //!< translational states for the smoothed profiler options
    void computeFourthSmoothedSegment(double time); //!< Method for computing the fourth smoothing segment scalar
                                                    //!< translational states for the smoothed bang-coast-bang option
    void computeCoastSegment(double time); //!< Method for computing the coast segment scalar translational states
    void computeTranslationComplete(); //!< Method for computing the scalar translational states when the translation is
                                       //!< complete

    void writeOutputMessages(uint64_t CurrentSimNanos); //!< Method for writing the module output messages and computing
                                                        //!< the output message data
