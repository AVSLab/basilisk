...
class moduleGenerator:
    ...
    def __init__(self):
        ...
        self.modulePathRelSrc = (
            None  # path to the new module folder relative to basilisk/src
        )
        ...
        self.cleanBuild = (
            False  # flag if any prior directories should be deleted automatically
        )
        self.verbose = True  # flag if the status messages should be printed

        # private class variables
        self._absPath = (
            None  # absolute path to the folder which will contain the module folder
        )
        self._newModuleLocation = (
            None  # absolute path to the auto-generated Basilisk module folder
        )
    ...
    def createNewModuleFolder(self):
        ...
        if os.path.isdir(self._newModuleLocation):
            ...
            if not self.cleanBuild:
                ans = input(
                    "Do you want to delete this folder and recreate? (y or n): "
                )
                if ans != "y":
    ...
    def createRstFile(self):
        ...
        self.log(
            f"{statusColor}Creating RST Documentation File {rstFileName}:{endColor}",
            end=" ",
        )
    ...
    def createTestFile(self, type):
        ...
        self.log(
            f"{statusColor}Creating Python Init Test File {testFileName}:{endColor}",
            end=" ",
        )
        testFile = ""
        ...
        testFile += "\n"
        testFile += (
            f"def test_{self.moduleName}(show_plots, param1, param2, accuracy):\n"
        )
        testFile += '    r"""\n'
        ...
        if type == "C++":
            ...
            self.log(
                f"{failColor}ERROR: {endColor}Wrong module type provided to test file method."
            )
            exit(0)
        ...
        for msg in self.outMsgList:
            ...
            testFile += (
                f"    unitTestSim.AddModelToTask(unitTaskName, {msg['var']}Rec)\n"
            )
        testFile += "\n"
    ...
    def createCppModule(self):
        ...
        self.log(
            f"{statusColor}Creating Header File {headerFileName}:{endColor}", end=" "
        )
        headerFile = licenseC
        headerFile += "\n"
        ...
        for msg in outMsgList:
            headerFile += (
                f"    Message<{msg['type']}Payload> {msg['var']};  //!< {msg['desc']}\n"
            )
        headerFile += "\n"
        ...
        if len(variableList):
            for msg in variableList:
                ...
                headerFile += (
                    f"    void set{varName[:1].upper() + varName[1:]}({msg['type']});\n"
                )
                headerFile += f"    /** getter for `{varName}` property */\n"
                headerFile += (
                    f"    {msg['type']} get{varName[:1].upper() + varName[1:]}() "
        ...
        self.log(
            statusColor + "Creating Definition File " + defFileName + ":" + endColor,
            end=" ",
        )
        defFile = licenseC
        defFile += "\n"
    ...
    def createCModule(self):
        ...
        for msg in inMsgList + outMsgList:
            ...
            if msg["type"] not in includedMsgs:
                ...
                if msg["wrap"] == "C++":
                    self.log(
                        f"{failColor}Error: {endColor}You can't include C++ messages in a C module."
                    )
                    exit()
                includedMsgs.append(msg["type"])
        headerFile += '#include "architecture/utilities/bskLogging.h"\n'
        ...
        headerFile += "#endif\n"
        headerFile += (
            f"    void SelfInit_{name}({name}Config *configData, int64_t moduleID);\n"
        )
        ...
        self.log(
            f"{statusColor}Creating Definition File {defFileName}:{endColor}", end=" "
        )
        defFile = licenseC
        defFile += "\n"
        defFile += f'#include "{modulePath}/{name}/{name}.h"\n'
        ...
        for msg in inMsgList:
            defFile += (
                f"    if (!{msg['type']}_C_isLinked(&configData->{msg['var']})) {{\n"
            )
            defFile += (
                f'        _bskLog(configData->bskLogger, BSK_ERROR, "Error: {name}.{msg["var"]}'
                + ' was not connected.");\n'
            )
            defFile += "    }\n"
        ...
        self.log(
            f"{statusColor}Creating Swig Interface File {swigFileName}:{endColor}",
            end=" ",
        )
        swigFile = licenseC
        swigFile += f"%module {name}\n"
        ...
        for msg in inMsgList + outMsgList:
            ...
            if msg["type"] not in includedMsgs:
                ...
                if msg["wrap"] == "C++":
                    self.log(
                        f"{failColor}ERROR: {endColor}you cannot swig a C++ message in a C module."
                    )
                includedMsgs.append(msg["type"])
        swigFile += "\n"
        swigFile += "%pythoncode %{\n"
...
def fillCppInfo(module):
    ...
    module.briefDescription = (
        "This is an auto-created sample C++ module.  The description is included with "
        "the module class definition"
    )
    module.copyrightHolder = (
        "Autonomous Vehicle Systems Lab, University of Colorado Boulder"
    )
    ...
    outMsgList.append(
        {
            "type": "DataStorageStatusMsg",
            "var": "anotherCppOutMsg",
            "desc": "output msg description",
            "wrap": "C++",
        }
    )
    ...
    variableList = list()
    variableList.append(
        {"type": "double", "var": "varDouble", "desc": "[units] variable description"}
    )
    variableList.append(
        {"type": "int", "var": "varInt", "desc": "[units] variable description"}
    )
