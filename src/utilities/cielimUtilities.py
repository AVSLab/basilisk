...
def addLocation(viz, **kwargs):
    if not cielimFound:
        print('cielimFound is false. Skipping this method.')
        return

    vizElement = cielimInterface.LocationPbMsg()
    ...
    viz.locations = cielimInterface.LocationConfig(locationList)
...
def createPointLine(viz, **kwargs):
    ...
    vizElement = cielimInterface.PointLine()
    ...
    viz.settings.pointLineList = cielimInterface.PointLineConfig(pointLineList)
    return
...
def updateTargetLineList(viz):
    ...
    viz.liveSettings.targetLineList = cielimInterface.PointLineConfig(targetLineList)
    return
...
def createCustomModel(viz, **kwargs):
    if not cielimFound:
        ...
        return
    global firstSpacecraftName
    vizElement = cielimInterface.CustomModel()
    ...
    if 'simBodiesToModify' in kwargs:
        ...
        for item in simBodiesList:
            if not isinstance(item, str):
                ...
                exit(1)
        vizElement.simBodiesToModify = cielimInterface.StringVector(simBodiesList)
    else:
        vizElement.simBodiesToModify = cielimInterface.StringVector([firstSpacecraftName])
    ...
    if 'color' in kwargs:
        ...
        if len(colorVariable) != 4:
            ...
            exit(1)
        vizElement.color = cielimInterface.IntVector(colorVariable)

    customModelList.append(vizElement)
    del viz.settings.customModelList[:]  # clear settings list to replace it with updated list
    viz.settings.customModelList = cielimInterface.CustomModelConfig(customModelList)
    return
...
def setActuatorGuiSetting(viz, **kwargs):
    ...
    :param viz: copy of the cielimInterface module
    ...
    global firstSpacecraftName
    vizElement = cielimInterface.ActuatorGuiSettings()
    ...
    viz.settings.actuatorGuiSettingsList = cielimInterface.ActuatorGuiSettingsConfig(actuatorGuiSettingList)
    return
...
def setInstrumentGuiSetting(viz, **kwargs):
    ...
    global firstSpacecraftName
    vizElement = cielimInterface.InstrumentGuiSettings()
    ...
    viz.settings.instrumentGuiSettingsList = cielimInterface.InstrumentGuiSettingsConfig(instrumentGuiSettingList)
    return
...
def createConeInOut(viz, **kwargs):
    if not cielimFound:
        ...
        return
    global firstSpacecraftName
    vizElement = cielimInterface.KeepOutInCone()
    ...
    viz.settings.coneList = cielimInterface.KeepOutInConeConfig(coneInOutList)
    return
...
def createStandardCamera(viz, **kwargs):
    ...
    if not cielimFound:
        print('cielimFound is false. Skipping this method.')
        return
    cam = cielimInterface.StdCameraSettings()
    ...
    viz.settings.stdCameraList = cielimInterface.StdCameraConfig(stdCameraList)
    return
...
def enableCielimVisualization(scSim, scList, **kwargs):
    """
    This method creates an instance of the cielimInterface() modules and setups up associated Vizard
    ...
    Returns
    -------
    :ref:`cielimInterface` object
        the cielimInterface instance

    """
    if not cielimFound:
        print(
            'Could not find cielimInterface when import attempted.  Be sure to build BSK with cielimInterface support.')
        return
    ...
    planetInfoList = []
    spiceMsgList = []
    cielimMessenger.scData.clear()
    c = 0
    spacecraftParentName = ""

    for sc in scList:
        # create spacecraft information container
        scData = cielimInterface.VizSpacecraftData()
        ...
        if isinstance(sc, type(spacecraft.Spacecraft())):
            ...
            for gravBody in sc.gravField.gravBodies:
                ...
                if gravBody.planetName not in planetNameList:
                    ...
                    planetInfo = cielimInterface.GravBodyInfo()
        ...
        if thrEffectorScList:
            ...
            scData.thrInfo = cielimInterface.ThrClusterVector(thrInfo)
        ...
        if gsScList:
            ...
            if gsScList[c] is not None:  # generic sensor(s) have been added to this spacecraft
                for gs in gsScList[c]:
                    gsList.append(gs)
                scData.genericSensorList = cielimInterface.GenericSensorVector(gsList)
        ...
        if elScList:
            ...
            if elScList[c] is not None:  # generic sensor(s) have been added to this spacecraft
                for el in elScList[c]:
                    elList.append(el)
                scData.ellipsoidList = cielimInterface.EllipsoidVector(elList)
        ...
        if liScList:
            ...
            if liScList[c] is not None:  # light objects(s) have been added to this spacecraft
                for li in liScList[c]:
                    liList.append(li)
                scData.lightList = cielimInterface.LightVector(liList)
                ...
                for gsd in gsdScList[c]:
                    if len(gsd.color) > 1:
                        ...
                        if len(gsd.thresholds) > 0:
                            ...
                            exit(1)
                    gsdList.append(gsd)
                scData.genericStorageList = cielimInterface.GenericStorageVector(gsdList)
        ...
        if tcScList:
            ...
            if tcScList[c] is not None:  # transceiver(s) have been added to this spacecraft
                for tc in tcScList[c]:
                    tcList.append(tc)
                scData.transceiverList = cielimInterface.TransceiverVector(tcList)
        ...
        if oscOrbitColorList:
            if oscOrbitColorList[c] is not None:
                scData.oscOrbitLineColor = cielimInterface.IntVector(oscOrbitColorList[c])

        if trueOrbitColorList:
            if trueOrbitColorList[c] is not None:
                scData.trueTrajectoryLineColor = cielimInterface.IntVector(trueOrbitColorList[c])
        ...
        cielimMessenger.scData.push_back(scData)

        c += 1

    cielimMessenger.gravBodyInformation = cielimInterface.GravBodyInfoVector(planetInfoList)
    cielimMessenger.spiceInMsgs = messaging.SpicePlanetStateMsgInMsgsVector(spiceMsgList)
    ...
    cielimMessenger.saveFile = False
    if 'saveFile' in kwargs:
        ...
        cielimMessenger.saveFile = True
        cielimMessenger.protoFilename = vizFileNamePath
    ...
    if 'liveStream' in kwargs:
        ...
        if not isinstance(val, bool):
            ...
            exit(1)
        cielimMessenger.liveStream = val
    ...
    cielimMessenger.setOpNavMode(0)
    if 'opNavMode' in kwargs:
        ...
        if val < 0 or val > 2:
            print('ERROR: vizSupport: opNavMode must be 0 (off), 1 (regular opNav) or 2 (high performance opNav)')
            exit(1)
        cielimMessenger.setOpNavMode(val)
        if val > 0:
            cielimMessenger.opnavImageOutMsgName = "opnav_circles"

    return cielimMessenger
