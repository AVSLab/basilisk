...
class UniformDispersion(SingleVariableDispersion):
    ...
    def generate(self, sim):
        ...
        mid = (self.bounds[1] + self.bounds[0]) / 2.0
    ...
    def generate(self, sim):
        ...
        if self.stdDeviation != 0:
            self.magnitude.append(
                str(round((dispValue - self.mean) / self.stdDeviation, 2)) + " sigma"
            )
        return dispValue
...
class VectorVariableDispersion(object):
    ...
    def perturbVectorByAngle(self, vector, angle):
        ...
        self.magnitude.append(
            str(
                round(
                    np.arccos(
                        np.dot(rndVec, vector)
                        / np.linalg.norm(rndVec)
    ...
    def perturbCartesianVectorUniform(self, vector):
        ...
        for i in range(len(vector)):
            ...
            scale = self.bounds[1] - mid
            self.magnitude.append(
                str(round((dispValues[i] - mid) / scale * 100, 2)) + " %"
            )
        return dispValues
    ...
    def perturbCartesianVectorNormal(self, vector):
        ...
        for i in range(len(vector)):
            ...
            if self.stdDeviation != 0:
                self.magnitude.append(
                    str(round((dispValues[i] - self.mean) / self.stdDeviation, 2))
                    + r" $\sigma$"
                )
        return dispValues
...
class NormalVectorDispersion(VectorVariableDispersion):
    ...
    def generate(self, sim):
        vector = getattr(sim, self.varName)
        dispValue = self.perturbCartesianVectorNormal(
            vector, self.mean, self.stdDeviation
        )
        return dispValue
...
class UniformVectorAngleDispersion(VectorVariableDispersion):
    ...
    def generate(self, sim=None):
        ...
        self.phiBounds = [
            meanPhi + self.phiBoundsOffNom[0],
            meanPhi + self.phiBoundsOffNom[1],
        ]
        self.thetaBounds = [
            meanTheta + self.thetaBoundsOffNom[0],
            meanTheta + self.thetaBoundsOffNom[1],
        ]

        phiRnd = np.random.uniform(
            meanPhi + self.phiBounds[0], meanPhi + self.phiBounds[1]
        )
        thetaRnd = np.random.uniform(
            meanTheta + self.thetaBounds[0], meanTheta + self.thetaBounds[1]
        )
        ...
        dispVec = newVec / np.linalg.norm(
            newVec
        )  # Shouldn't technically need the normalization but doing it for completeness

        midPhi = (self.phiBounds[1] + self.phiBounds[0]) / 2.0
        scalePhi = self.phiBounds[1] - midPhi
        midTheta = (self.thetaBounds[1] + self.thetaBounds[0]) / 2.0
        scaleTheta = self.thetaBounds[1] - midTheta
        self.magnitude.append(str(round((phiRnd - midPhi) / scalePhi * 100, 2)) + " %")
        self.magnitude.append(
            str(round((thetaRnd - midTheta) / scaleTheta * 100, 2)) + " %"
        )
...
class NormalVectorAngleDispersion(VectorVariableDispersion):
    def __init__(
        ...
        thetaStd=np.pi / 3.0,
        phiStd=np.pi / 3.0,
        thetaBoundsOffNom=None,
        phiBoundsOffNom=None,
    ):
    ...
    def generate(self, sim=None):
        ...
        self.magnitude.append(
            str(round((phiRnd - meanPhi) / self.phiStd, 2)) + r" $\sigma$"
        )
        self.magnitude.append(
            str(round((thetaRnd - meanTheta) / self.thetaStd, 2)) + r" $\sigma$"
        )
...
class UniformEulerAngleMRPDispersion(VectorVariableDispersion):
    ...
    def generate(self, sim=None):
        ...
        for i in range(3):
            ...
            ) * np.random.random() + self.bounds[0]
        dispMRP = rbk.euler3232MRP(rndAngles)
        dispMRP = dispMRP.reshape(3)
        for i in range(3):
            self.magnitude.append(
                str(round((dispMRP[i] - np.pi) / np.pi * 100, 2)) + " %"
            )
        return dispMRP
...
class NormalThrusterUnitDirectionVectorDispersion(VectorVariableDispersion):
    def __init__(self, varName, thrusterIndex=0, phiStd=0.1745, bounds=None):
        ...
        """
        super(NormalThrusterUnitDirectionVectorDispersion, self).__init__(
            varName, bounds
        )
    ...
    def generate(self, sim=None):
        if sim is None:
            print(
                (
                    "No simulation object parameter set in '"
                    + self.generate.__name__
                    + "()' dispersions will not be set for variable "
                    + self.varName
                )
            )
            return
            ...
            angleDisp = np.arccos(
                np.dot(dirVec, dispVec)
                / np.linalg.norm(dirVec)
                / np.linalg.norm(dispVec)
            )
        ...
        for i in range(3):
            if isinstance(self.stdDeviation, collections.abc.Sequence):
                ...
                if self.stdDeviation[i] != 0:
                    self.magnitude.append(
                        str(round((rnd - self.mean[i]) / self.stdDeviation[i], 2))
                        + " sigma"
                    )
            else:
                ...
                if self.stdDeviation != 0:
                    self.magnitude.append(
                        str(round((rnd - self.mean) / self.stdDeviation, 2)) + " sigma"
                    )
            if self.bounds is not None:
                rnd = self.checkBounds(rnd, self.bounds)
            dispVec.append(rnd)
    ...
    def generate(self, sim=None):
        if sim is None:
            ...
            for i in range(3):
                if self.stdAngle != 0:
                    self.magnitude.append(
                        str(round(angles[i] / self.stdAngle, 2)) + " sigma"
                    )
...
class MRPDispersionPerAxis(VectorVariableDispersion):
    ...
    def generate(self, sim=None):
        ...
        for i in range(3):
            ...
            ) * np.random.random() + self.bounds[i][0]
