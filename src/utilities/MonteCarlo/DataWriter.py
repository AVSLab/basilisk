    ...
    def run(self):
        ...
        while self._endToken is None:
            ...
            for (
                dictName,
                dictData,
            ) in data.items():  # Loops through Messages, Variables, Custom dictionaries in the retention policy
                for (
                    itemName,
                    itemData,
                ) in dictData.items():  # Loop through all items and their data
                    if (
                        itemName == "OrbitalElements.Omega"
                    ):  # Protects from OS that aren't case sensitive.
                    ...
                    if variLen == 0:
                        innerLabel.append(
                            0
                        )  # May not be necessary, might be able to leave blank and get a None
                    labels = pd.MultiIndex.from_product(
                        [outerLabel, innerLabel], names=["runNum", "varIdx"]
                    )
        ...
        for filePath in self._dataFiles:
            ...
            newMultInd = pd.MultiIndex.from_product(
                [
                    list(range(allData.columns.min()[0], allData.columns.max()[0] + 1)),
