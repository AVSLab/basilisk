...
def RV2OE(mu, r_IJK, v_IJK):
    ...
    e_vec = 1.0 / mu * ((vm * vm - mu / rm) * r - np.dot(r, v) * v)
...
def ephemeridesMeeus(JDE, celestialBody):
    ...
    if celestialBody == "EARTH":
        ...
        Pi = (
            336.060234 + 0.4438898 * T + (-0.00017321) * T * T + 0.000000300 * T * T * T
        )
        ...
        Omega = (
            100.464441 + 0.1766828 * T + 0.00090387 * T * T + (-0.000007032) * T * T * T
        )
...
def computeCOE(L, a, e, i, Omega, Pi):
    ...
    M = L - Pi
    C_cen = (
        (2 * e - 1 // 4 * np.power(e, 3) + 5 // 96 * np.power(e, 5)) * np.sin(M)
...
def B_params_1(r, v, mu):
    ...
    e_vec = 1.0 / mu * ((vm * vm - mu / rm) * r - np.dot(r, v) * v)
    e = np.linalg.norm(e_vec)
    Betta = np.arccos(1.0 / e)
    a = 1.0 / (2 / rm - vm * vm / mu)
    b = np.abs(a) * np.sqrt(e * e - 1)

    k_hat = np.array([0, 0, 1])
    S_hat = np.cos(Betta) / e * e_vec + np.sin(Betta) * normalize(
        np.cross(h_hat, e_vec)
    )
...
def B_params_2(v_in, v_out, mu):
    ...
    rp = mu / (vm_in * vm_in) * (1.0 / np.cos(0.5 * (astroConstants.MPI - psi)) - 1)
...
def EarthResonantOrbit(v_inf_in, V1, R1, v_inf_out, V2, N):
    ...
    rm_1 = la.norm(R1)
    vm_sc_h = np.sqrt(
        astroConstants.MU_SUN * (2.0 / rm_1 - 1.0 / a)
    )  # s/c heliocentric velocity

    # compute theta
    vm_inf_in = la.norm(v_inf_in)
    vm_P = la.norm(V1)
    cosTheta = (vm_inf_in * vm_inf_in + vm_P * vm_P - vm_sc_h * vm_sc_h) / (
        2 * vm_inf_in * vm_P
    )
    ...
    phi_vec = np.linspace(0.0, 2 * astroConstants.MPI, p)
    for phi in phi_vec:
...
def testResonance():
    ...
    R2 = np.array([31194146713.798, 143988869052.709, 2352102.45497072])
    V1 = np.array([-29599.9908905588, 6189.32095554111, 0.0729005972949417])
    V2 = np.array([-29598.5134550237, 6195.92283490717, 0.0568758149973483])

    v_inf_in = np.array([-0.320955575422136, -8.79065711515436, 1.70894261814904])
    v_inf_out = np.array([-8.95399388012787, -0.346120703226863, -0.350867778719334])
...
def plotPlanetOrbit(JD, planet, color, ax):
    ...
    max_range = np.array(
        [rx.max() - rx.min(), ry.max() - ry.min(), rz.max() - rz.min()]
    ).max()
...
def TisserandPlot(R_P, v_inf, color):
    RP = np.array([])
    RA = np.array([])
    R_P_vec = np.array([0.0, -R_P, 0.0])
    V_P = V_circular(R_P, astroConstants.MU_SUN)
    V_P_vec = np.array([V_P, 0.0, 0.0])
    points = 360
    alpha_vec = np.linspace(0.0, astroConstants.MPI, points)
    for alpha in alpha_vec:
        M = rotationMatrix(alpha)
        V_P_hat = normalize(V_P_vec)
        v_inf_vec = v_inf * np.dot(M, V_P_hat)
        V_inf_vec = V_P_vec + v_inf_vec
        (a, e, i, Omega, omega, nu) = RV2OE(astroConstants.MU_SUN, R_P_vec, V_inf_vec)
        rp = a * (1.0 - e)
        ra = a * (1.0 + e)
...
def Tisserand_P1_2_P2(r_planet_vec, v_inf_P1_vec, v_inf_P2_vec):
    for r_planet in r_planet_vec:
        plt.loglog(r_planet / astroConstants.AU, r_planet / astroConstants.AU, "ko")
    ...
    plt.title(
        r"$v_{\infty}^{Earth} $ = " + str(v_inf_P1_vec) + r" km/s \n"
