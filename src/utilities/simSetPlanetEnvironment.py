...
def centeredDipoleMagField(magFieldModule, name):
    ...
    if name == "earth":
        ...
        convertToIgrfDipoleCoefficients(
            0.142 / 10000,  # [T]   dipole strength
            46.9 * macros.D2R,  # [rad] Dipole tilt to rotational axis
            288.0 * macros.D2R,  # [rad] Longitude of tilt
            magFieldModule,
        )
        ...
        print(
            "ERROR: "
            + name
            + " not setup for centered dipole magnetic field model. Options include mercury, earth, jupiter, saturn, uranus and neptune. \n"
        )

    return


def convertToIgrfDipoleCoefficients(
    nominalField, tilt, longitudeOfTilt, magFieldModule
):
    """
