...
\section{Unit Test}
The unit test for this module \verb|test_MRP_feedback| tests a single set of gains $K,K_i,P$ on a rigid body with 4 reaction wheels with no external torques, and with a time-varying reference attitude. The simulation is run for one second, cleared, and then run for two. The output torques from each are compared to a precomputed set to validate the calculation.
...
\item Gains $k,k_i,P$ and integrator windup limit $integralLimit$ in \verb|moduleConfig|
\end{itemize}
