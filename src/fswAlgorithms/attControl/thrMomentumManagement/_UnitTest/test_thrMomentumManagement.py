...
def test_thrMomentumManagement(show_plots, hsMinCheck):
    ...
    [testResults, testMessage] = thrMomentumManagementTestFunction(
        show_plots, hsMinCheck
    )
...
def thrMomentumManagementTestFunction(show_plots, hsMinCheck):
    ...
    if hsMinCheck:
        module.hs_min = 1000.0 / 6000.0 * 100.0  # Nms
    else:
        module.hs_min = 100.0 / 6000.0 * 100.0  # Nms

    # wheelSpeeds Message
    rwSpeedMessage = messaging.RWSpeedMsgPayload()
    rwSpeedMessage.wheelSpeeds = [10.0, -25.0, 50.0, 100.0]
    ...
    if hsMinCheck == 1:
        trueVector = [[0.0, 0.0, 0.0]] * 2
    else:
        trueVector = [[-5.914369484146579, -2.858300248464629, -9.407020039211664]] * 2
