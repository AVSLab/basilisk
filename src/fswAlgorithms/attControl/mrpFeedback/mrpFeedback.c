...
*/
void
SelfInit_mrpFeedback(mrpFeedbackConfig* configData, int64_t moduleID)
{
...
*/
void
Reset_mrpFeedback(mrpFeedbackConfig* configData, uint64_t callTime, int64_t moduleID)
{
...
*/
void
Update_mrpFeedback(mrpFeedbackConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    if (configData->rwConfigParams.numRW > 0) {
        ...
        wheelsAvailability =
          RWAvailabilityMsg_C_zeroMsgPayload(); /* wheelAvailability set to 0 (AVAILABLE) by default */
    ...
    if (configData->Ki > 0) { /* check if integral feedback is turned on  */
        ...
        m33MultV3(
          RECAST3X3 configData->ISCPntB_B, guidCmd.omega_BR_B, v3_2); /* -[v3Tilde(omega_r+Ki*z)]([I]omega + [Gs]h_s) */
    ...
    v3Scale(configData->P, guidCmd.omega_BR_B, v3_3); /* +P delta_omega */
    v3Add(v3_3, Lr, Lr);
