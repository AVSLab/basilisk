...
def test_module(show_plots, numAttNav, numTransNav):
    ...
    [testResults, testMessage] = navAggregateTestFunction(
        show_plots, numAttNav, numTransNav
    )
...
def navAggregateTestFunction(show_plots, numAttNav, numTransNav):
    ...
    navAtt1Msg.omega_BN_B = [1.0, 1.0, -1.0]
    ...
    navAtt2Msg.omega_BN_B = [2.0, 2.0, -2.0]
    ...
    navTrans1Msg.v_BN_N = [1.0, 1.0, -1.0]
    ...
    navTrans2Msg.v_BN_N = [2.0, 2.0, -2.0]
    ...
    unitTestSupport.writeTeXSnippet(
        "navAtt1Msg.sigma_BN", str(navAtt1Msg.sigma_BN), path
    )
    unitTestSupport.writeTeXSnippet(
        "navAtt1Msg.omega_BN_B", str(navAtt1Msg.omega_BN_B), path
    )
    unitTestSupport.writeTeXSnippet(
        "navAtt1Msg.vehSunPntBdy", str(navAtt1Msg.vehSunPntBdy), path
    )
    unitTestSupport.writeTeXSnippet("navAtt2Msg.timeTag", str(navAtt2Msg.timeTag), path)
    unitTestSupport.writeTeXSnippet(
        "navAtt2Msg.sigma_BN", str(navAtt2Msg.sigma_BN), path
    )
    unitTestSupport.writeTeXSnippet(
        "navAtt2Msg.omega_BN_B", str(navAtt2Msg.omega_BN_B), path
    )
    unitTestSupport.writeTeXSnippet(
        "navAtt2Msg.vehSunPntBdy", str(navAtt2Msg.vehSunPntBdy), path
    )
    unitTestSupport.writeTeXSnippet(
        "navTrans1Msg.timeTag", str(navTrans1Msg.timeTag), path
    )
    unitTestSupport.writeTeXSnippet(
        "navTrans1Msg.r_BN_N", str(navTrans1Msg.r_BN_N), path
    )
    unitTestSupport.writeTeXSnippet(
        "navTrans1Msg.v_BN_N", str(navTrans1Msg.v_BN_N), path
    )
    unitTestSupport.writeTeXSnippet(
        "navTrans1Msg.vehAccumDV", str(navTrans1Msg.vehAccumDV), path
    )
    unitTestSupport.writeTeXSnippet(
        "navTrans2Msg.timeTag", str(navTrans2Msg.timeTag), path
    )
    unitTestSupport.writeTeXSnippet(
        "navTrans2Msg.r_BN_N", str(navTrans2Msg.r_BN_N), path
    )
    unitTestSupport.writeTeXSnippet(
        "navTrans2Msg.v_BN_N", str(navTrans2Msg.v_BN_N), path
    )
    unitTestSupport.writeTeXSnippet(
        "navTrans2Msg.vehAccumDV", str(navTrans2Msg.vehAccumDV), path
    )
    ...
    if numAttNav == 0 or numAttNav == 3:
        ...
        trueAttSigma = [[0.0, 0.0, 0.0]] * 3
        trueAttOmega = [[0.0, 0.0, 0.0]] * 3
        trueAttSunVector = [[0.0, 0.0, 0.0]] * 3
    ...
    testFailCount, testMessages = unitTestSupport.compareArray(
        trueAttOmega, attOmega, accuracy, "omega_BN_B", testFailCount, testMessages
    )
    testFailCount, testMessages = unitTestSupport.compareArray(
        trueAttSunVector,
        attSunVector,
    ...
    testFailCount, testMessages = unitTestSupport.compareArray(
        trueTransVel, transVel, accuracy, "omega_BN_B", testFailCount, testMessages
    )
    testFailCount, testMessages = unitTestSupport.compareArray(
        trueTransAccum,
        transAccum,
