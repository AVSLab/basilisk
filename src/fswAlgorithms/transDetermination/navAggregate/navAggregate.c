 ...
 */
void
SelfInit_aggregateNav(NavAggregateData* configData, int64_t moduleID)
{
 ...
 */
void
Reset_aggregateNav(NavAggregateData* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    if (configData->attMsgCount > MAX_AGG_NAV_MSG) {
        ...
        snprintf(info,
                 MAX_LOGGING_LENGTH,
                 "The attitude message count %d is larger than allowed (%d). Setting count to max value.",
                 configData->attMsgCount,
                 MAX_AGG_NAV_MSG);
    ...
    for (uint32_t i = 0; i < configData->attMsgCount; i = i + 1) {
        if (!NavAttMsg_C_isLinked(&configData->attMsgs[i].navAttInMsg)) {
            _bskLog(configData->bskLogger,
                    BSK_ERROR,
                    "An attitude input message name was not linked.  Be sure that attMsgCount is set properly.");
        }
    }
    ...
    for (uint32_t i = 0; i < configData->transMsgCount; i = i + 1) {
        if (!NavTransMsg_C_isLinked(&configData->transMsgs[i].navTransInMsg)) {
            _bskLog(configData->bskLogger,
                    BSK_ERROR,
                    "A translation input message name was not specified.  Be sure that transMsgCount is set properly.");
        }
    }
 ...
 */
void
Update_aggregateNav(NavAggregateData* configData, uint64_t callTime, int64_t moduleID)
{
    uint32_t i;
