...
\newcommand{\ModuleName}{oeStateEphem}
\newcommand{\subject}{Orbit Element State Ephemeris Generation Module}
\newcommand{\status}{Released}
\newcommand{\preparer}{H. Schaub}
\newcommand{\summary}{This module takes the TDB time, current object time and computes trajectory position and velocity states using the time corrected by TDB and the stored Chebyshev orbit element coefficients.  If the time provided is outside the specified range for which the stored Chebyshev coefficients are valid then the position vectors rail high/low appropriately.}
...
\hline
1.0 & First Documentation Release & H. Schaub & 2019-04-26\\
1.1 & updated documentation on anomaly angle types & H. Schaub & 2019-04-29\\
\hline
