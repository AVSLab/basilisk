...
@pytest.mark.parametrize(
    "validChebyCurveTime, anomFlag", [(True, 0), (True, 1), (True, -1), (False, -1)]
)
def test_chebyPosFitAllTest(show_plots, validChebyCurveTime, anomFlag):
    """Module Unit Test"""
    [testResults, testMessage] = chebyPosFitAllTest(
        show_plots, validChebyCurveTime, anomFlag
    )
...
def chebyPosFitAllTest(show_plots, validChebyCurveTime, anomFlag):
    ...
    for timeVal in timeHistory:
        ...
        omegaArray.append(orbEl.omega)
        if anomFlag == 1:
            currentAnom = orbitalMotion.E2M(
                orbitalMotion.f2E(orbEl.f, orbEl.e), orbEl.e
            )
        else:
            currentAnom = orbEl.f
    ...
    chebOmegaCoeff = numpy.polynomial.chebyshev.chebfit(
        fitTimes, OmegaArray, degChebCoeff
    )
    chebomegaCoeff = numpy.polynomial.chebyshev.chebfit(
        fitTimes, omegaArray, degChebCoeff
    )
    chebAnomCoeff = numpy.polynomial.chebyshev.chebfit(
        fitTimes, anomArray, degChebCoeff
    )
    ...
    clockCorrData.ephemerisTime = (
        oeStateModel.ephArray[0].ephemTimeMid - oeStateModel.ephArray[0].ephemTimeRad
    )
    ...
    if not validChebyCurveTime:
        ...
        if not numpy.array_equal(secondLastPos, lastPos):
            testFailCount += 1
            testMessages.append(
                "FAILED: Expected Chebychev position to rail high or low "
                + str(secondLastPos)
                + " != "
                + str(lastPos)
            )
        ...
        if not numpy.array_equal(secondLastVel, lastVel):
            testFailCount += 1
            testMessages.append(
                "FAILED: Expected Chebychev velocity to rail high or low "
                + str(secondLastVel)
                + " != "
                + str(lastVel)
            )
        ...
        maxErrVec = [
            abs(max(posChebData[:, 0] - tdrssPosList[:, 0])),
            abs(max(posChebData[:, 1] - tdrssPosList[:, 1])),
            abs(max(posChebData[:, 2] - tdrssPosList[:, 2])),
        ]
        maxVelErrVec = [
            abs(max(velChebData[:, 0] - tdrssVelList[:, 0])),
        ...
        if max(maxErrVec) >= orbitPosAccuracy:
            testFailCount += 1
            testMessages.append(
                "FAILED: maxErrVec >= orbitPosAccuracy, TDRSS Orbit Accuracy: "
                + str(max(maxErrVec))
            )
        if max(maxVelErrVec) >= orbitVelAccuracy:
            testFailCount += 1
            testMessages.append(
                "FAILED: maxVelErrVec >= orbitVelAccuracy, TDRSS Velocity Accuracy: "
