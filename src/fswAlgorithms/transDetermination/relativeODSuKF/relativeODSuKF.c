...
#include "transDetermination/relativeODSuKF/relativeODSuKF.h"
 ...
 */
void SelfInit_relativeODSuKF(RelativeODSuKFConfig *ConfigData, uint64_t moduleID)
{
    ...
    ConfigData->filtDataOutMsgId = CreateNewMessage(ConfigData->filtDataOutMsgName,
        sizeof(RelativeODFilterFswMsg), "RelativeODFilterFswMsg", moduleID);
 ...
 */
void CrossInit_relativeODSuKF(RelativeODSuKFConfig *ConfigData, uint64_t moduleID)
{
    /*! Begin method steps */
...
/*! This method resets the relativeOD attitude filter to an initial state and
 initializes the internal estimation matrices.
 @return void
 @param ConfigData The configuration data associated with the CSS estimator
 @param callTime The clock time at which the function was called (nanoseconds)
 */
void Reset_relativeODSuKF(RelativeODSuKFConfig *ConfigData, uint64_t callTime,
                      uint64_t moduleID)
{
    ...
    memset(&(ConfigData->outputRelativeOD), 0x0, sizeof(NavAttIntMsg));
...
/*! This method takes the parsed CSS sensor data and outputs an estimate of the
 ...
 @param callTime The clock time at which the function was called (nanoseconds)
 */
void Update_relativeODSuKF(RelativeODSuKFConfig *ConfigData, uint64_t callTime,
    uint64_t moduleID)
{
    double newTimeTag;
    ...
    uint64_t ClockTime;
    uint32_t ReadSize;
    RelativeODFilterFswMsg relativeODDataOutBuffer;
    ...
    {
        relativeODSuKFSwitch(ConfigData->bVec_B, ConfigData->state, ConfigData->covar);
    }
    ...
    {
        relativeODSuKFTimeUpdate(ConfigData, newTimeTag);
        relativeODSuKFMeasUpdate(ConfigData, newTimeTag);
    }
    ...
    relativeODSuKFMeasModel(ConfigData);
    ...
    /*! - Write the relativeOD estimate into the copy of the navigation message structure*/
	v3Copy(ConfigData->state, ConfigData->outputRelativeOD.vehSunPntBdy);
    v3Normalize(ConfigData->outputRelativeOD.vehSunPntBdy,
        ConfigData->outputRelativeOD.vehSunPntBdy);
    ConfigData->outputRelativeOD.timeTag = ConfigData->timeTag;
	WriteMessage(ConfigData->navStateOutMsgId, callTime, sizeof(NavAttIntMsg),
		&(ConfigData->outputRelativeOD), moduleID);
    ...
    relativeODDataOutBuffer.timeTag = ConfigData->timeTag;
    relativeODDataOutBuffer.numObs = ConfigData->numObs;
    memmove(relativeODDataOutBuffer.covar, ConfigData->covar,
            SKF_N_STATES_SWITCH*SKF_N_STATES_SWITCH*sizeof(double));
    memmove(relativeODDataOutBuffer.state, states_BN, SKF_N_STATES_SWITCH*sizeof(double));
    memmove(relativeODDataOutBuffer.postFitRes, ConfigData->postFits, MAX_N_CSS_MEAS*sizeof(double));
    WriteMessage(ConfigData->filtDataOutMsgId, callTime, sizeof(RelativeODFilterFswMsg),
                 &relativeODDataOutBuffer, moduleID);
    
    return;
}

/*! This method propagates a relativeOD state vector forward in time.  Note 
    that the calling parameter is updated in place to save on data copies.
	@return void
	@param stateInOut The state that is propagated
*/
void relativeODStateProp(double *stateInOut, double *b_Vec, double dt)
{
    ...
    relativeODSuKFComputeDCM_BS(stateInOut, b_Vec, &dcm_BS[0][0]);
...
/*! This method performs the time update for the relativeOD kalman filter.
...
*/
void relativeODSuKFTimeUpdate(RelativeODSuKFConfig *ConfigData, double updateTime)
{
	int i, Index;
	...
	relativeODStateProp(&(ConfigData->SP[0 * ConfigData->numStates + 0]), ConfigData->bVec_B, ConfigData->dt);
	...
	for (i = 0; i<ConfigData->countHalfSPs; i++)
	{
		...
		relativeODStateProp(spPtr, ConfigData->bVec_B, ConfigData->dt);
 ...
 */
void relativeODSuKFMeasModel(RelativeODSuKFConfig *ConfigData)
{
...
/*! This method performs the measurement update for the relativeOD kalman filter.
 ...
 @param updateTime The time that we need to fix the filter to (seconds)
 */
void relativeODSuKFMeasUpdate(RelativeODSuKFConfig *ConfigData, double updateTime)
{
    uint32_t i;
...
void relativeODSuKFSwitch(double *bVec_B, double *states, double *covar)
{
    ...
    relativeODSuKFComputeDCM_BS(sun_heading_norm, bVec_B, &dcm_BSold[0][0]);
    
    if (v3IsEqual(bVec_B, b1, 1e-10))
    {
        relativeODSuKFComputeDCM_BS(sun_heading_norm, b2, &dcm_BSnew_T[0][0]);
        v3Copy(b2, bVec_B);
    }
    else
    {
        relativeODSuKFComputeDCM_BS(sun_heading_norm, b1, &dcm_BSnew_T[0][0]);
        v3Copy(b1, bVec_B);
    }
...
void relativeODSuKFComputeDCM_BS(double sunheading[SKF_N_STATES_HALF], double bVec[SKF_N_STATES_HALF], double *dcm){
