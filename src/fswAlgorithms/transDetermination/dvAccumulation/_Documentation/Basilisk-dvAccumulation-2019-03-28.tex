...
\newcommand{\ModuleName}{dvAccumulation}
\newcommand{\subject}{Net $\Delta v$ Evaluation Module Integrating Accelerometer Data}
\newcommand{\status}{First Release}
\newcommand{\preparer}{H. Schaub}
\newcommand{\summary}{This module reads in a message with an array of accelerometer measurements and integrates them to determine an accumulated $\Delta\bm v$ value.  On reset the net $\Delta\bm v$ is set to zero.  The output navigation message contains the latest measurements time tag and the total $\Delta\bm v$.}
