...
class IzzoSolve:
    def __init__(self, r1, r2, tof, mu, max_multiRev=10, max_iter=12):
        ...
        h_hat = np.cross(self.r1_hat, self.r2_hat) / np.linalg.norm(
            np.cross(self.r1_hat, self.r2_hat)
        )
        assert h_hat[2] != 0, print("r1 and r2 don't define a plane")

        self.lambda2 = 1.0 - c_norm / self.s
        ...
        self.lambda3 = self.lambd**3.0
        self.T = np.sqrt(2.0 * self.mu / self.s**3) * self.tof
    ...
    def solve(self):
        ...
        self.x = x_list
        gamma = np.sqrt(self.mu * self.s / 2.0)
        rho = (np.linalg.norm(self.r1) - np.linalg.norm(self.r2)) / np.linalg.norm(
            self.c
        )
        ...
        for i in range(len(x_list)):
            y = np.sqrt(1.0 - self.lambda2 + self.lambda2 * x_list[i] ** 2)
            vr1 = (
                gamma
                * ((self.lambd * y - x_list[i]) - rho * (self.lambd * y + x_list[i]))
                / np.linalg.norm(self.r1)
            )
            vr2 = (
                -gamma
                * ((self.lambd * y - x_list[i]) + rho * (self.lambd * y + x_list[i]))
                / np.linalg.norm(self.r2)
            )
    ...
    def findxy(self, lambd, T):
        ...
        T_00 = np.arccos(lambd) + lambd * np.sqrt(1.0 - lambd**2)
        ...
        if T < T_0 and M_max > 0:
            ...
            for it in range(12):
                ...
                if DT != 0.0:
                    x_new = x_old - 2.0 * DT * D2T / (2.0 * D2T**2 - DT * D3T)
        ...
        if T >= T_00:
            ...
            self.x0 = 5.0 / 2.0 * T_1 * (T_1 - T) / (T * (1 - self.lambd**5)) + 1
        else:
            self.x0 = (T_00 / T) ** (np.log(T_1 / T_00) / np.log(2)) - 1.0
    ...
    def dTdx(self, x, T):
        umx2 = 1.0 - x**2
        y = np.sqrt(1.0 - self.lambda2 * umx2)

        DT = 1.0 / umx2 * (3.0 * T * x - 2.0 + 2.0 * self.lambda3 * x / y)
        D2T = (
            1.0
            / umx2
            * (
                3.0 * T
                + 5.0 * x * DT
                + 2.0 * (1.0 - self.lambda2) * self.lambda3 / y**3
            )
        )
        D3T = (
            1.0
            ...
            * (
                7.0 * x * D2T
                + 8.0 * DT
                - 6.0 * (1.0 - self.lambda2) * self.lambda2 * self.lambda3 * x / y**5
            )
        )
    ...
    def x2tof2(self, x, N):
        ...
        if a > 0:  # Eclipse case
            ...
            tof = (
                a
                * np.sqrt(a)
                * ((alfa - np.sin(alfa)) - (beta - np.sin(beta)) + 2.0 * np.pi * N)
            ) / 2.0
        else:
            ...
            tof = (
                -a
                * np.sqrt(-a)
                * ((beta - np.sinh(beta)) - (alfa - np.sinh(alfa)))
                / 2.0
            )
        return tof
