 ...
 */
void
SelfInit_pixelLineBiasUKF(PixelLineBiasUKFConfig* configData, int64_t moduleId)
{
 ...
 */
void
Reset_pixelLineBiasUKF(PixelLineBiasUKFConfig* configData, uint64_t callTime, int64_t moduleId)
{
 ...
 */
void
Update_pixelLineBiasUKF(PixelLineBiasUKFConfig* configData, uint64_t callTime, int64_t moduleId)
{
    ...
    memmove(opNavOutBuffer.covar, configData->covar, PIXLINE_N_STATES * PIXLINE_N_STATES * sizeof(double));
 ...
 */
void
relODStateProp(PixelLineBiasUKFConfig* configData, double* stateInOut, double dt)
{
 ...
 */
void
pixelLineBiasUKFTwoBodyDyn(double state[PIXLINE_DYN_STATES], double muPlanet, double* stateDeriv)
{
    double rNorm;
    double dvdt[3];
 ...
 */
int
pixelLineBiasUKFTimeUpdate(PixelLineBiasUKFConfig* configData, double updateTime)
{
    uint64_t i, Index;
    double sBarT[PIXLINE_N_STATES * PIXLINE_N_STATES]; // Sbar transpose (chol decomp of covar)
    double xComp[PIXLINE_N_STATES],
      AT[(2 * PIXLINE_N_STATES + PIXLINE_N_STATES) * PIXLINE_N_STATES]; // Intermediate state, process noise chol decomp
 ...
 */
void
pixelLineBiasUKFMeasModel(PixelLineBiasUKFConfig* configData)
{
    size_t i, j;
    ...
    v3Set(configData->circlesInBuffer.circlesCenters[0],
          configData->circlesInBuffer.circlesCenters[1],
          configData->circlesInBuffer.circlesRadii[0],
          configData->obs);
    ...
    pX = 2. * tan(configData->cameraSpecs.fieldOfView * configData->cameraSpecs.resolution[0] /
                  configData->cameraSpecs.resolution[1] / 2.0);
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (i < 2) {
                configData->yMeas[i * (configData->countHalfSPs * 2 + 1) + j] =
                  centers[i] - configData->SP[j * configData->numStates + PIXLINE_DYN_STATES + i];
            }
            if (i == 2) {
                configData->yMeas[i * (configData->countHalfSPs * 2 + 1) + j] =
                  radius - configData->SP[j * configData->numStates + PIXLINE_DYN_STATES + i];
            }
            configData->yMeas[(i + PIXLINE_N_MEAS / 2) * (configData->countHalfSPs * 2 + 1) + j] =
              configData->SP[j * configData->numStates + PIXLINE_DYN_STATES + i];
        }
    }
    /*! - yMeas matrix was set backwards deliberately so we need to transpose it through*/
    mTranspose(configData->yMeas, PIXLINE_N_MEAS, configData->countHalfSPs * 2 + 1, configData->yMeas);
}
 ...
 */
int
pixelLineBiasUKFMeasUpdate(PixelLineBiasUKFConfig* configData)
{
    uint32_t i;
    ...
    mSetZero(configData->measNoise, PIXLINE_N_MEAS, PIXLINE_N_MEAS);
    mSetSubMatrix(
      configData->circlesInBuffer.uncertainty, 3, 3, configData->measNoise, PIXLINE_N_MEAS, PIXLINE_N_MEAS, 3, 3);
    mSetSubMatrix(
      configData->circlesInBuffer.uncertainty, 3, 3, configData->measNoise, PIXLINE_N_MEAS, PIXLINE_N_MEAS, 0, 0);
 ...
 */
void
pixelLineBiasUKFCleanUpdate(PixelLineBiasUKFConfig* configData)
{
    size_t i;
