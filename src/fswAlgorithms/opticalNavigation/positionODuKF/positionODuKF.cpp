...
PositionODuKF::PositionODuKF() = default;
...
void PositionODuKF::Reset(uint64_t CurrentSimNanos)
{
    ...
    this->sBar = PositionODuKF::choleskyDecomposition(this->covar);
    this->cholProcessNoise = PositionODuKF::choleskyDecomposition(this->processNoise * 1E-6);
}
...
@return void
 */
void PositionODuKF::computePostFitResiudals()
{
...
void PositionODuKF::measurementUpdate()
{
    ...
    sy = PositionODuKF::qrDecompositionJustR(A);

    /*! - Cholesky update block for vectors.*/
    Eigen::VectorXd yError = this->yMeas.col(0) - yBar;
    sy = PositionODuKF::choleskyUpDownDate(sy, yError, this->wC(0));
    ...
    STkMatT = PositionODuKF::forwardSubstitution(sy, pXY.transpose());
    kMat = PositionODuKF::backSubstitution(sy.transpose(), STkMatT).transpose();
