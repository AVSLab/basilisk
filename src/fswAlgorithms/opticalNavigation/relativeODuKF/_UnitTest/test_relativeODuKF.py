...
def relOD_method_test(show_plots):
    ...
    if (
        np.linalg.norm(np.array(measurements) - np.transpose(SPexp[0:3, :]).flatten())
        > 1.0e-15
    ):
        testFailCount += 1
...
def StateUpdateRelOD(show_plots):
    ...
    mu = 42828.314 * 1e9
    energy[0] = -mu / (
        2 * orbitalMotion.rv2elem(mu, expected[0, 1:4], expected[0, 4:]).a
    )

    kick = np.array([0.0, 0.0, 0.0, -0.01, 0.01, 0.02]) * 10 * 1e3

    expected[0:t1, :] = rk4(twoBodyGrav, time[0:t1], module.stateInit)
    expected[t1 : multT1 * t1 + 1, :] = rk4(
        twoBodyGrav, time[t1 : len(time)], expected[t1 - 1, 1:] + kick
    )
    for i in range(1, len(time)):
        energy[i] = -mu / (
            2 * orbitalMotion.rv2elem(mu, expected[i, 1:4], expected[i, 4:]).a
        )
    ...
    FilterPlots.PostFitResiduals(
        postFitLog, np.sqrt(5 * 1e-2 * 1e6), "Update", show_plots
    )
