...
@pytest.mark.parametrize(
    "r_c1, r_c2, valid1, valid2, faultMode",
    [
...
def test_faultdetection(show_plots, r_c1, r_c2, valid1, valid2, faultMode):
    ...
    **Description of Variables Being Tested**
    ...
    """
    [testResults, testMessage] = faultdetection(
        show_plots, r_c1, r_c2, valid1, valid2, faultMode
    )
...
def faultdetection(show_plots, r_c1, r_c2, valid1, valid2, faultMode):
    ...
    inputCamera.fieldOfView = 2.0 * np.arctan(
        10 * 1e-3 / 2.0 / (1.0 * 1e-3)
    )  # 2*arctan(s/2 / f)
    inputCamera.resolution = [512, 512]
    inputCamera.sigma_CB = [1.0, 0.3, 0.1]
    ...
    inputPrimary.covar_C = [0.5, 0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 1.0]
    inputPrimary.covar_N = (
        np.dot(
            np.dot(
                NC,
                np.array([0.5, 0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 1.0]).reshape([3, 3]),
            ),
            NC.T,
        )
        .flatten()
    ...
    inputSecondary.covar_C = [0.5, 0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 1.0]
    inputSecondary.covar_N = (
        np.dot(
            np.dot(
                NC,
                np.array([0.5, 0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 1.0]).reshape([3, 3]),
    ...
    if valid1 == 0 and valid2 == 1:
        ...
        if faultMode == 0:
            r_Cexp = r_c2
            covar_Cexp = [0.5, 0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 1.0]
    ...
    if valid1 == 1 and valid2 == 1:
        ...
        covarz = np.array([0.5, 0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 1.0]).reshape([3, 3])
        z1 = covarz[:, 2]
        z2 = np.copy(z1)
        if faultNorm > faults.sigmaFault * (np.linalg.norm(z1) + np.linalg.norm(z2)):
            ...
            covar_Cexp = (
                np.linalg.inv(np.linalg.inv(covarz) + np.linalg.inv(covarz))
                .flatten()
                .tolist()
            )
            r_Cexp = np.dot(
                np.linalg.inv(np.linalg.inv(covarz) + np.linalg.inv(covarz)),
                np.dot(np.linalg.inv(covarz), r1) + np.dot(np.linalg.inv(covarz), r2),
            ).tolist()
    ...
    for i in range(len(outputCovar[-1, 0:])):
        if np.abs((covar_Cexp[i] - outputCovar[-1, i])) > 1e-10 or np.isnan(
            outputTime.any()
        ):
            testFailCount += 1
