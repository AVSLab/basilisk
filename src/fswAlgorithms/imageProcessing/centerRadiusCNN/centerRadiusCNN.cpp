 ...
 */
void
CenterRadiusCNN::Reset(uint64_t CurrentSimNanos)
{
 ...
 */
void
CenterRadiusCNN::UpdateState(uint64_t CurrentSimNanos)
{
    ...
    cv::Mat img_blob =
      cv::dnn::blobFromImage(imageCV, 1.0 / 255.0, cv::Size(imageCV.rows, imageCV.cols), cv::Scalar(0, 0, 0), true);
