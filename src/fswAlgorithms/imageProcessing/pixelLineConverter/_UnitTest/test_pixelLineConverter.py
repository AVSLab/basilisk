...
def mapState(state, planet, camera):
    ...
    pX = 2.0 * np.tan(
        camera.fieldOfView * camera.resolution[0] / camera.resolution[1] / 2.0
    )
    pY = 2.0 * np.tan(camera.fieldOfView / 2.0)
    ...
    vec = np.array([state[0] * d_x, state[1] * d_y, 1.0])
...
def mapCovar(CovarXYR, rho, planet, camera):
    ...
    rho_map = (
        ...
        * D
        * (
            -np.sqrt(1 + rho**2 * d_x**2) / (rho**2 * d_x)
            + d_x / (np.sqrt(1 + rho**2 * d_x**2))
        )
    )
    x_map = 0.5 * D / np.sin(0.5 * A) * (d_x)
    y_map = 0.5 * D / np.sin(0.5 * A) * (d_y)
    CovarMap = np.array([[x_map, 0.0, 0.0], [0.0, y_map, 0.0], [0.0, 0.0, rho_map]])
...
def pixelLineConverterTestFunction():
    ...
    inputCircles.uncertainty = [0.5, 0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 1.0]
    ...
    state = [
        inputCircles.circlesCenters[0],
        inputCircles.circlesCenters[1],
        inputCircles.circlesRadii[0],
    ]
    ...
    for i in range(len(outputCovar[-1, 0:])):
        if np.abs((covar_Nexp[i] - outputCovar[-1, i])) > 1e-10 and np.isnan(
            outputTime.any()
        ):
            testFailCount += 1
            testMessages.append("FAILED: Covar Check in pixelLine")

    if np.abs((timTagExp - outputTime[-1]) / timTagExp) > 1e-10 and np.isnan(
        outputTime.any()
    ):
        testFailCount += 1
