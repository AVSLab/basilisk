...
def test_PrescribedRot2DOFTestFunction(
    show_plots,
    thetaInit,
    thetaRef1a,
    thetaRef2a,
    ...
    **Description of Variables Being Tested**
    ...
    [testResults, testMessage] = PrescribedRot2DOFTestFunction(
        show_plots,
        thetaInit,
        thetaRef1a,
        thetaRef2a,
...
def PrescribedRot2DOFTestFunction(
    show_plots,
    thetaInit,
    thetaRef1a,
    thetaRef2a,
    ...
    rotAxis1_M = np.array(
        [0.0, 1.0, 0.0]
    )  # Rotation axis for the first reference rotation angle, thetaRef1a
    rotAxis2_P1 = np.array(
        [0.0, 0.0, 1.0]
    )  # Rotation axis for the second reference rotation angle, thetaRef2a
    ...
    prescribedRot2DOFObj.omega_PM_P = np.array(
        [0.0, 0.0, 0.0]
    )  # [rad/s] Angular velocity of frame P relative to frame M in P frame components
    prescribedRot2DOFObj.omegaPrime_PM_P = np.array(
        [0.0, 0.0, 0.0]
    )  # [rad/s^2] B frame time derivative of omega_FB_F in P frame components
    prescribedRot2DOFObj.sigma_PM = np.array(
        [0.0, 0.0, 0.0]
    ...
    prv_P0M_a = (
        thetaRef1a * rotAxis1_M[0],
        thetaRef1a * rotAxis1_M[1],
        thetaRef1a * rotAxis1_M[2],
    )
    prv_P1P0_a = (
        thetaRef2a * rotAxis2_P1[0],
        thetaRef2a * rotAxis2_P1[1],
        thetaRef2a * rotAxis2_P1[2],
    )

    # Calculate a single reference PRV for the first rotation and the associated MRP attitude
    if (
        thetaRef1a == 0 and thetaRef2a == 0
    ):  # Prevent a (0,0,0) error using rbk.addPRV()
    ...
    prv_P2M_b = (
        thetaRef1b * rotAxis1_M[0],
        thetaRef1b * rotAxis1_M[1],
        thetaRef1b * rotAxis1_M[2],
    )
    prv_P3P2_b = (
        thetaRef2b * rotAxis2_P1[0],
        thetaRef2b * rotAxis2_P1[1],
        thetaRef2b * rotAxis2_P1[2],
    )

    # Calculate a single reference PRV (prv_P3M_b) for the second rotation beginning from the M frame
    if (
        thetaRef1b == 0 and thetaRef2b == 0
    ):  # Prevent a (0,0,0) error using rbk.addPRV()
    ...
    hingedRigidBodyMessageData2.thetaDot = thetaDot_Ref
    HingedRigidBodyMessage1 = messaging.HingedRigidBodyMsg().write(
        hingedRigidBodyMessageData1, macros.sec2nano(simTime1)
    )
    HingedRigidBodyMessage2 = messaging.HingedRigidBodyMsg().write(
        hingedRigidBodyMessageData2, macros.sec2nano(simTime1)
    )
    prescribedRot2DOFObj.spinningBodyRef1InMsg.subscribeTo(HingedRigidBodyMessage1)
    prescribedRot2DOFObj.spinningBodyRef2InMsg.subscribeTo(HingedRigidBodyMessage2)
    ...
    plt.title(
        r"Prescribed Angular Velocity ${}^\mathcal{P} \omega_{\mathcal{P}/\mathcal{M}}$"
    )
    plt.xlabel("Time (s)")
    plt.ylabel("(rad/s)")
    ...
    plt.plot(
        timespan * macros.NANO2SEC, thetaRef1_plotting, "--", label=r"$\Phi_{1_{Ref}}$"
    )
    plt.plot(
        timespan * macros.NANO2SEC, thetaRef2_plotting, "--", label=r"$\Phi_{2_{Ref}}$"
    )
    ...
    if not unitTestSupport.isArrayEqual(sigma_PM_Final1, sigma_PM_Ref1, 3, accuracy):
        ...
        testMessages.append(
            "FAILED: "
            + prescribedRot2DOFObj.ModelTag
            + " MRPs sigma_PM_Final1 and sigma_PM_Ref1 do not match"
        )
    ...
    if not unitTestSupport.isArrayEqual(sigma_PM_Final2, sigma_PM_Ref2, 3, accuracy):
        ...
        testMessages.append(
            "FAILED: "
            + prescribedRot2DOFObj.ModelTag
            + " MRPs sigma_PM_Final2 and sigma_PM_Ref2 do not match"
        )
