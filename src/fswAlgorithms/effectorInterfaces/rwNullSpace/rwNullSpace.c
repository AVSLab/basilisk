 ...
 */
void
SelfInit_rwNullSpace(rwNullSpaceConfig* configData, int64_t moduleID)
{
 ...
 */
void
Reset_rwNullSpace(rwNullSpaceConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    mMultM(GsMatrix, 3, configData->numWheels, GsTranspose, configData->numWheels, 3, GsInvHalf); /* find [Gs].[Gs]^T */
    m33Inverse(RECAST3X3 GsInvHalf, RECAST3X3 GsInvHalf);                         /* find ([Gs].[Gs]^T)^-1 */
    mMultM(GsInvHalf, 3, 3, GsMatrix, 3, configData->numWheels, configData->tau); /* find ([Gs].[Gs]^T)^-1.[Gs] */
    mMultM(GsTranspose,
           configData->numWheels,
           3,
           configData->tau,
           3,
           configData->numWheels,
           GsTemp); /* find [Gs]^T.([Gs].[Gs]^T)^-1.[Gs] */
    mSetIdentity(identMatrix, configData->numWheels, configData->numWheels);
    mSubtract(identMatrix,
              configData->numWheels,
              configData->numWheels, /* find ([I] - [Gs]^T.([Gs].[Gs]^T)^-1.[Gs]) */
              GsTemp,
              configData->tau);
}
 ...
 */
void
Update_rwNullSpace(rwNullSpaceConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    mMultV(configData->tau, configData->numWheels, configData->numWheels, dVector, finalControl.motorTorque);

    /* add the null motion RW torque solution to the RW feedback control torque solution */
    vAdd(finalControl.motorTorque, configData->numWheels, cntrRequest.motorTorque, finalControl.motorTorque);
