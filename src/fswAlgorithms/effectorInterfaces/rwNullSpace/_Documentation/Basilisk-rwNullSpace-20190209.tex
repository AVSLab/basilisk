...
\newcommand{\ModuleName}{rwNullSpace}
\newcommand{\subject}{Reaction Wheel Wheel Speed Reduction using the RW Null Space}
\newcommand{\status}{first release module Documentation}
\newcommand{\preparer}{H. Schaub}
\newcommand{\summary}{The module reads in the reaction wheel (RW) control motor torques, and super imposes an additional torque that drives the wheel speeds to desired values.  The default RW desired speeds are 0 rpm.  The RW speed control torque array is projected through the RW null space to ensure that the net torque created is always zero.  Thus, the addition of this RW null motion torque has no impact on the earlier control torque performance.}
...
1.1 & Desired RW Speeds & H. Schaub & 2021-06-23\\
\hline
