...
class Results_thrForceMapping:
    def __init__(
        ...
        self.angErrThresh = (
            angErrThresh  # Determines when the thrusters are considered saturated
        )
        ...
        self.C = np.array(
            COrig
        )  # Control "Frame" (could be 1, 2, or 3 axii controllable)
    ...
    def results_thrForceMapping(self):
        ...
        if self.thrForceSign < 0 or self.use2ndLoop:
            ...
            for i in range(0, len(F)):
                if t[i]:
                    DNew = np.append(
                        DNew, np.cross((self.rData[i, :] - self.COM), self.gData[i])
                    )
            DNew = np.reshape(DNew, (3, (len(DNew) // 3)), order="F")
            FNew = self.mapToForce(DNew, Lr_Bar)
            if self.thrForceSign > 0:
                FNew = self.subtractMin(
                    FNew, len(DNew[0])
                )  # Produced negative forces when doing 2nd loop, dropped thruster, and COM offset
        ...
        if angle > self.angErrThresh:
            ...
            for i in range(0, self.numThrusters):
                if (
                    self.thrForceMag[i] > 0
                    and abs(F[i]) / self.thrForceMag[i] > maxFractUse
                ):
