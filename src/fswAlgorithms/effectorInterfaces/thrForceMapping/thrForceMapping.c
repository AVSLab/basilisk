...
void Reset_thrForceMapping(thrForceMappingConfig *configData, uint64_t callTime, int64_t moduleID)
{
    ...
    if (configData->numControlAxes==0) {
        _printMessage(configData->bskPrint, MSG_ERROR,"thrForceMapping() is not setup to control any axes!");
    }
    if (configData->thrForceSign==0) {
        _printMessage(configData->bskPrint, MSG_ERROR,"thrForceMapping() must have thrForceSign set to either +1 or -1");
    }
    ...
    for(i=0; i<configData->numThrusters; i=i+1)
    {
        ...
        if(localThrusterData.thrusters[i].maxThrust <= 0.0){
            _printMessage(configData->bskPrint, MSG_ERROR, "A configured thruster has a non-sensible saturation limit of <= 0 N!");
        } else {
