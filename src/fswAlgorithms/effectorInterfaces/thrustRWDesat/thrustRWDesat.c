 ...
 */
void
SelfInit_thrustRWDesat(thrustRWDesatConfig* configData, int64_t moduleID)
{
    THRArrayOnTimeCmdMsg_C_init(&configData->thrCmdOutMsg);
}

void
Reset_thrustRWDesat(thrustRWDesatConfig* configData, uint64_t callTime, int64_t moduleID)
{
 ...
 */
void
Update_thrustRWDesat(thrustRWDesatConfig* configData, uint64_t callTime, int64_t moduleID)
{
    int32_t i;
    ...
    if ((callTime - configData->previousFiring) * 1.0E-9 < configData->thrFiringPeriod) {
        return;
    }
    ...
    for (i = 0; i < configData->numRWAs; i++) {
        v3Scale(rwSpeeds.wheelSpeeds[i], &(configData->rwAlignMap[i * 3]), singleSpeedVec);
    ...
    if (v3Dot(configData->currDMDir, observedSpeedVec) <= configData->DMThresh) {
        for (i = 0; i < configData->numThrusters; i++) {
            ...
            fireValue = v3Dot(observedSpeedVec, &(configData->thrTorqueMap[i * 3]));
            currentMatch = v3Dot(configData->accumulatedImp, &(configData->thrAlignMap[i * 3]));
        ...
        if (selectedThruster >= 0) {
            v3Normalize(&configData->thrTorqueMap[selectedThruster * 3], configData->currDMDir);
        }
    }
    ...
    */
    ...
    for (i = 0; i < configData->numThrusters; i++) {
        ...
        fireValue = v3Dot(configData->currDMDir, &(configData->thrTorqueMap[i * 3]));
        currentMatch = v3Dot(configData->accumulatedImp, &(configData->thrAlignMap[i * 3]));
        if (fireValue - currentMatch > bestMatch && fireValue > 0.0) {
            selectedThruster = i;
            bestMatch = fireValue - currentMatch;
    ...
    if (bestMatch > 0.0) {
        outputData.OnTimeRequest[selectedThruster] =
          v3Dot(configData->currDMDir, &(configData->thrTorqueMap[selectedThruster * 3]));
        outputData.OnTimeRequest[selectedThruster] = outputData.OnTimeRequest[selectedThruster] > configData->maxFiring
                                                       ? configData->maxFiring
                                                       : outputData.OnTimeRequest[selectedThruster];
        configData->previousFiring = callTime;
        configData->totalAccumFiring += outputData.OnTimeRequest[selectedThruster];
        v3Scale(
          outputData.OnTimeRequest[selectedThruster], &(configData->thrAlignMap[selectedThruster * 3]), singleSpeedVec);
        v3Add(configData->accumulatedImp, singleSpeedVec, configData->accumulatedImp);
    }
