 ...
 */
void
SelfInit_solarArrayReference(solarArrayReferenceConfig* configData, int64_t moduleID)
{
...
*/
void
Reset_solarArrayReference(solarArrayReferenceConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    if (configData->pointingMode == momentumManagement) {
        if (!RWSpeedMsg_C_isLinked(&configData->rwSpeedsInMsg)) {
            ...
            _bskLog(configData->bskLogger,
                    BSK_WARNING,
                    "solarArrayReference.rwSpeedsInMsg wasn't connected. "
        ...
        if (!RWArrayConfigMsg_C_isLinked(&configData->rwConfigDataInMsg)) {
            ...
            _bskLog(configData->bskLogger,
                    BSK_WARNING,
                    "solarArrayReference.rwConfigDataInMsg wasn't connected."
                    "Switching to power generation mode.");
        } else {
...
*/
void
Update_solarArrayReference(solarArrayReferenceConfig* configData, uint64_t callTime, int64_t moduleID)
{
