 ...
 */
void
SelfInit_spacecraftPointing(spacecraftPointingConfig* configData, int64_t moduleID)
{
 ...
 */
void
Reset_spacecraftPointing(spacecraftPointingConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    /* Build a coordinate system around the vector within the body frame that points towards the antenna and write the
     orientation of the B-frame with respect to the A-frame. */
 ...
 */
void
Update_spacecraftPointing(spacecraftPointingConfig* configData, uint64_t callTime, int64_t moduleID)
{
    NavTransMsgPayload chiefTransMsg; /*!< ---  Input message that consists of the position and velocity of the chief */
    NavTransMsgPayload
      deputyTransMsg;    /*!< ---  Input message that consists of the position and velocity of the deputy */
    ...
    /* Due to the fact that sigma_dot_RN is actually the average increase in sigma over the timeperiod between t-1 and
       ...
       eq. 3.163). */
    old_sigma_RN_squared = configData->old_sigma_RN[0] * configData->old_sigma_RN[0] +
                           configData->old_sigma_RN[1] * configData->old_sigma_RN[1] +
                           configData->old_sigma_RN[2] * configData->old_sigma_RN[2];
    sigma_RN_squared = sigma_RN[0] * sigma_RN[0] + sigma_RN[1] * sigma_RN[1] + sigma_RN[2] * sigma_RN[2];

    m33Set(1.0 - old_sigma_RN_squared + 2.0 * configData->old_sigma_RN[0] * configData->old_sigma_RN[0],
           2.0 * (configData->old_sigma_RN[0] * configData->old_sigma_RN[1] - configData->old_sigma_RN[2]),
           2.0 * (configData->old_sigma_RN[0] * configData->old_sigma_RN[2] + configData->old_sigma_RN[1]),
           2.0 * (configData->old_sigma_RN[1] * configData->old_sigma_RN[0] + configData->old_sigma_RN[2]),
           1.0 - old_sigma_RN_squared + 2.0 * configData->old_sigma_RN[1] * configData->old_sigma_RN[1],
           2.0 * (configData->old_sigma_RN[1] * configData->old_sigma_RN[2] - configData->old_sigma_RN[0]),
           2.0 * (configData->old_sigma_RN[2] * configData->old_sigma_RN[0] - configData->old_sigma_RN[1]),
           2.0 * (configData->old_sigma_RN[2] * configData->old_sigma_RN[1] + configData->old_sigma_RN[0]),
           1.0 - old_sigma_RN_squared + 2.0 * configData->old_sigma_RN[2] * configData->old_sigma_RN[2],
           old_B_sigma_RN);
    ...
    /* One of the requirements for this module is that the user should be able to fill in a vector within the B-frame
       that points at the antenna. For this reason it is necessary to add the orientation of the B-frame with respect to
       the A-frame to the R-frame orientation with respect to the N-frame (add sigma_BA to sigma_RN). This results in
       the orientation of the R1-frame with respect to the N-frame (sigma_R1N). In case the spacecraft B-frame points
       towards the R1-frame, the A-frame will point towards the R-frame, which results in an antenna vector that aligns
       with the vector that points to the chief spacecraft. */
