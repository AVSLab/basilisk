...
def etSphericalControlTestFunction(show_plots, accuracy):
    ...
    Ki = 4e-7
    Pi = 1.85 * Ki**0.5
    module.K = [Ki, 0.0, 0.0, 0.0, Ki, 0.0, 0.0, 0.0, Ki]
    module.P = [Pi, 0.0, 0.0, 0.0, Pi, 0.0, 0.0, 0.0, Pi]
    module.L_r = 30.0
    module.theta_r = 0.0
    module.phi_r = 0.0
    module.mu = mu
    ...
    r_DT_N = np.array([2.0, -L0, -3.0])  # relative position between debris and servicer
    r_DN_N = r_TN_N + r_DT_N
    v_DN_N = v_TN_N
    debrisNavTransOutData = (
        messaging.NavTransMsgPayload()
    )  # Create a structure for the input message
    ...
    servicerConfigOutData.massSC = 500.0
    ...
    debrisConfigOutData.massSC = 2000.0
    ...
    q_T = (L * (L * R_T * V_T - R_T * R_D * V_D)) / (kc * (L**2.0 - R_T * R_D))
    q_D = (L * (L * R_D * V_D - R_T * R_D * V_T)) / (kc * (L**2.0 - R_T * R_D))
    Fc = kc * q_T * q_D / L**2.0
    Fc_N = Fc * (
        -r_DT_N / np.linalg.norm(r_DT_N)
    )  # electrostatic force acting on servicer
    ...
    trueInertialVector = [
        [-0.00714223893615245, 0.00267752848271998, 0.000883681113161883]
    ]
    trueBodyVector = [[-0.00541988234898216, 0.00542736415350300, 0.000360862543207430]]

    # compare the module results to the truth values
    for i in range(0, len(trueInertialVector)):
        # check vector values
        if not unitTestSupport.isArrayEqual(
            forceInertialOutput[i], trueInertialVector[i], 3, accuracy
        ):
            testFailCount += 1
            ...
            testMessages.append(
                ...
                + module.ModelTag
                + " Module failed "
                + "Inertial Force Output"
                + " unit test at t="
                + str(forceInertialOutput[i, 0] * macros.NANO2SEC)
                + "sec\n"
            )
        if not unitTestSupport.isArrayEqual(
            forceBodyOutput[i], trueBodyVector[i], 3, accuracy
        ):
            testFailCount += 1
            ...
            testMessages.append(
                ...
                + "Body Force Output"
                + " unit test at t="
                + str(forceBodyOutput[i, 0] * macros.NANO2SEC)
                + "sec\n"
            )
