 ...
 */
void
SelfInit_etSphericalControl(etSphericalControlConfig* configData, int64_t moduleID)
{
...
*/
void
Reset_etSphericalControl(etSphericalControlConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    if (!VehicleConfigMsg_C_isLinked(&configData->servicerVehicleConfigInMsg)) {
        _bskLog(
          configData->bskLogger, BSK_ERROR, "Error: etSphericalControl.servicerVehicleConfigInMsg wasn't connected.");
    }
    if (!VehicleConfigMsg_C_isLinked(&configData->debrisVehicleConfigInMsg)) {
        _bskLog(
          configData->bskLogger, BSK_ERROR, "Error: etSphericalControl.debrisVehicleConfigInMsg wasn't connected.");
    }
    ...
    if (VehicleConfigMsg_C_read(&configData->debrisVehicleConfigInMsg).massSC <= 0.0) {
        _bskLog(configData->bskLogger,
                BSK_ERROR,
                "Error in etSphericalControl: debris mass must be set to a positive value.");
    }
    ...
    if (vMax(EigenValuesK, 3) > 0.0) {
        _bskLog(configData->bskLogger,
                BSK_ERROR,
                "Error in etSphericalControl: K must be a positive definite 3 by 3 matrix.");
    }
    if (vMax(EigenValuesP, 3) > 0.0) {
        _bskLog(configData->bskLogger,
                BSK_ERROR,
                "Error in etSphericalControl: P must be a positive definite 3 by 3 matrix.");
    }
...
*/
void
Update_etSphericalControl(etSphericalControlConfig* configData, uint64_t callTime, int64_t moduleID)
{
    // in
    ...
    calc_RelativeMotionControl(configData,
 ...
 */
void
calc_RelativeMotionControl(etSphericalControlConfig* configData,
                           NavTransMsgPayload servicerTransInMsgBuffer,
                           NavTransMsgPayload debrisTransInMsgBuffer,
                           NavAttMsgPayload servicerAttInMsgBuffer,
                           VehicleConfigMsgPayload servicerVehicleConfigInMsgBuffer,
                           ...
                           CmdForceBodyMsgPayload* forceBodyOutMsgBuffer)
{
    ...
    double TransfMatrix[3][3]; // transformation matrix to map state vector derivative from Hill components to spherical
                               // components
    m33Set(cos(phi) * sin(theta),
           -cos(theta) * cos(phi),
           -sin(phi),
    ...
    double X_r[3];
    v3Set(configData->L_r, configData->theta_r, configData->phi_r, X_r);
