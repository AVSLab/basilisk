...
static void calc_LyapunovFeedback(alMeanOEFeedbackConfig *configData, NavTransMsgPayload chiefTransMsg, 
...
void SelfInit_alMeanOEFeedback(alMeanOEFeedbackConfig *configData, int64_t moduleID) {
 ...
 */
void Reset_alMeanOEFeedback(alMeanOEFeedbackConfig *configData, uint64_t callTime, int64_t moduleID) {
...
/*! Add a description of what this main Update() routine does for this module
 ...
 @param moduleID The Basilisk module identifier
 */
void Update_alMeanOEFeedback(alMeanOEFeedbackConfig *configData, uint64_t callTime, int64_t moduleID) {
    // in
 ...
 */
static void calc_LyapunovFeedback(alMeanOEFeedbackConfig *configData, NavTransMsgPayload chiefTransMsg,NavTransMsgPayload deputyTransMsg, NavAttMsgPayload deputyAttNavMsg, CmdForceInertialMsgPayload *forceMsg) {
    ...
    /*
    When mean longitude L is 0 deg or 180 deg, the eccentricity vector 
    components (P1, P2) are most sensitive. Thrusting in-plane (radial or 
    along-track) at these longitudes efficiently changes eccentricity. 
    Eccentricity can be influenced at other values of L, but the effect 
    couples with semi-major axis changes.

    For inclination vector changes (Q1, Q2), thrusting out-of-plane 
    (normal to the orbital plane) near L = 90 deg and 270 deg is most 
    effective. These locations correspond to the ascending and descending 
    node regions in equinoctial coordinates.

    Thus:
      - L ≈ 0, 180 deg → target eccentricity (P1, P2) with in-plane thrust
      - L ≈ 90, 270 deg → target inclination (Q1, Q2) with out-of-plane thrust
    */


    double windowRad_nodes = configData->windowDeg_nodes * M_PI / 180.0;  // half-width [rad]
    double windowRad_apses = configData->windowDeg_apses * M_PI / 180.0;  // half-width [rad]
    
    double Ld = wrapPi(oe_eq_mean_d.L);  // mean longitude
    
    /* decide if we are inside ANY allowed window */
    int inWindow = 0;
    
    if (configData->enableApsesWindows) {
        if ( nearAngle(Ld, 0.0,  windowRad_nodes) ||    // perigee-like lobe for eccentricity changes
        nearAngle(Ld, M_PI, windowRad_nodes) ) {   // apogee-like lobe for eccentricity changes
            inWindow = 1;
        }
    }   
    
    if (configData->enableNodeWindows) {
        if ( nearAngle(Ld, M_PI_2,  windowRad_apses) ||    // ascending node-like lobe for inclination changes
        nearAngle(Ld, 3.0*M_PI_2, windowRad_apses) ) {   // descending node-like lobe for inclination changes
            inWindow = 1;
        }
    }
