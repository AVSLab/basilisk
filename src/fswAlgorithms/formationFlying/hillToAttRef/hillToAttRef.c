...
void
SelfInit_hillToAttRef(HillToAttRefConfig* configData, int64_t moduleID)
{
 ...
 */
void
Reset_hillToAttRef(HillToAttRefConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    if (AttRefMsg_C_isLinked(&configData->attRefInMsg) && NavAttMsg_C_isLinked(&configData->attNavInMsg)) {
        _bskLog(configData->bskLogger,
                BSK_ERROR,
                "Error: hillToAttRef can't have both attRefInMsg and attNavInMsg connected.");
    }
...
AttRefMsgPayload
RelativeToInertialMRP(HillToAttRefConfig* configData, double relativeAtt[3], double sigma_XN[3])
{
    AttRefMsgPayload attRefOut;
    //  Check to see if the relative attitude components exceed specified bounds (by default these are non-physical and
 ...
 */
void
Update_hillToAttRef(HillToAttRefConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    // std::cout<<"Current relative state: "<<hillState[0]<<" "<<hillState[1]<<" "<<hillState[2]<<" "<<hillState[3]<<"
    // "<<hillState[4]<<" "<<hillState[5]<<std::endl; std::cout<<"Printing current gain matrix:"<<std::endl;
    // std::cout<<gainMat[0][0]<<" "<<gainMat[0][1]<<" "<<gainMat[0][2]<<" "<<gainMat[0][3]<<" "<<gainMat[0][4]<<"
    // "<<gainMat[0][5]<<std::endl; std::cout<<gainMat[1][0]<<" "<<gainMat[1][1]<<" "<<gainMat[1][2]<<"
    // "<<gainMat[1][3]<<" "<<gainMat[1][4]<<" "<<gainMat[1][5]<<std::endl; std::cout<<gainMat[2][0]<<"
    // "<<gainMat[2][1]<<" "<<gainMat[2][2]<<" "<<gainMat[2][3]<<" "<<gainMat[2][4]<<" "<<gainMat[2][5]<<std::endl;

    //  Apply the gainMat to the relative state to produce a chief-relative attitude
    mMultV(&configData->gainMatrix, 3, 6, hillState, relativeAtt);
