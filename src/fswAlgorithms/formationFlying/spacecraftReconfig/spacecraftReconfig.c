...
void SelfInit_spacecraftReconfig(spacecraftReconfigConfig *configData, int64_t moduleID)
{
    ...
    ReconfigBurnArrayInfoMsg_C_init(&configData->burnInfoOutMsgs);
}
...
void Reset_spacecraftReconfig(spacecraftReconfigConfig *configData, uint64_t callTime, int64_t moduleID)
{
    ...
    configData->burnInfoOutMsgsBuffer = ReconfigBurnArrayInfoMsg_C_zeroMsgPayload();
...
void Update_spacecraftReconfig(spacecraftReconfigConfig *configData, uint64_t callTime, int64_t moduleID)
{
    ...
    ReconfigBurnArrayInfoMsg_C_write(&configData->burnInfoOutMsgsBuffer, &configData->burnInfoOutMsgs, moduleID, callTime);
                     ...
                     uint64_t callTime, int64_t moduleID)
{
    ...
    if(configData->burnInfoOutMsgsBuffer.burnArray[0].flag == 0){
        ...
        qsort(configData->burnInfoOutMsgsBuffer.burnArray, sizeof(configData->burnInfoOutMsgsBuffer) / sizeof(configData->burnInfoOutMsgsBuffer.burnArray[0]),
              sizeof(ReconfigBurnInfoMsgPayload), CompareTime);
    }

    /* After burn scheduling, the routine below is executed at every time step */
    /* Overall, configData->burnInfoOutMsgsBuffer.burnArray[i].flag is checked sequentially (i=0,1,2)  */
    if(configData->burnInfoOutMsgsBuffer.burnArray[0].flag == 1){
        double t_left = configData->burnInfoOutMsgsBuffer.burnArray[0].t - configData->tCurrent; // remaining time until first burn
        if(t_left > configData->attControlTime && configData->attRefInIsLinked){
            ...
            *attRefOutMsgBuffer = attRefInMsgBuffer;
            ...
            v3Copy(configData->burnInfoOutMsgsBuffer.burnArray[0].sigma_RN, attRefOutMsgBuffer->sigma_RN);
        }
        // middle of thruster burn duration time is located at the expected exact timing of impulsive control
        if(t_left < (int)configData->burnInfoOutMsgsBuffer.burnArray[0].thrustOnTime/(2*thrustConfigMsgBuffer.numThrusters) &&
            configData->burnInfoOutMsgsBuffer.burnArray[0].flag == 1){
            configData->thrustOnFlag     = 1; // thrustOnFlag is ON
            configData->burnInfoOutMsgsBuffer.burnArray[0].flag  = 2; // first burn is regarded as finished by setting this to 2
            int i = 0;
            for(i = 0;i < thrustConfigMsgBuffer.numThrusters;++i){
                thrustOnMsgBuffer->OnTimeRequest[i] = configData->burnInfoOutMsgsBuffer.burnArray[0].thrustOnTime/thrustConfigMsgBuffer.numThrusters;
            }
        }else{
            configData->thrustOnFlag = 0; // thrustOnFlag is OFF
        }
    }else if(configData->burnInfoOutMsgsBuffer.burnArray[1].flag == 1) {
        double t_left = configData->burnInfoOutMsgsBuffer.burnArray[1].t - configData->tCurrent; // remaining time until second burn
        if(configData->burnInfoOutMsgsBuffer.burnArray[0].flag == 2 && 
           configData->tCurrent < (configData->burnInfoOutMsgsBuffer.burnArray[0].t+configData->burnInfoOutMsgsBuffer.burnArray[0].thrustOnTime/(2*thrustConfigMsgBuffer.numThrusters))){
            ...
            *attRefOutMsgBuffer = attRefInMsgBuffer;
            ...
            v3Copy(configData->burnInfoOutMsgsBuffer.burnArray[1].sigma_RN, attRefOutMsgBuffer->sigma_RN);
        }
        if(t_left < (int)configData->burnInfoOutMsgsBuffer.burnArray[1].thrustOnTime/(2*thrustConfigMsgBuffer.numThrusters) &&
           configData->burnInfoOutMsgsBuffer.burnArray[1].flag == 1){
            configData->thrustOnFlag = 1;
            configData->burnInfoOutMsgsBuffer.burnArray[1].flag = 2;
            int i = 0;
            for(i = 0;i < thrustConfigMsgBuffer.numThrusters;++i){
                thrustOnMsgBuffer->OnTimeRequest[i] = configData->burnInfoOutMsgsBuffer.burnArray[1].thrustOnTime/thrustConfigMsgBuffer.numThrusters;
            }
        }else{
            configData->thrustOnFlag = 0;
        }
    }else if(configData->burnInfoOutMsgsBuffer.burnArray[2].flag == 1){
        double t_left = configData->burnInfoOutMsgsBuffer.burnArray[2].t - configData->tCurrent; // remaining time until third burn
        if(configData->burnInfoOutMsgsBuffer.burnArray[1].flag == 2 && 
           configData->tCurrent < (configData->burnInfoOutMsgsBuffer.burnArray[1].t+configData->burnInfoOutMsgsBuffer.burnArray[1].thrustOnTime/(2*thrustConfigMsgBuffer.numThrusters))){
            // in this case, second burn is still executed, so second burn attitude is set as target
            v3Copy(configData->burnInfoOutMsgsBuffer.burnArray[1].sigma_RN, attRefOutMsgBuffer->sigma_RN);
        }else if(configData->burnInfoOutMsgsBuffer.burnArray[0].flag == 2 && 
           configData->tCurrent < (configData->burnInfoOutMsgsBuffer.burnArray[0].t+configData->burnInfoOutMsgsBuffer.burnArray[0].thrustOnTime/(2*thrustConfigMsgBuffer.numThrusters))){
            // in this case, first burn is still executed, so first burn attitude is set as target
            v3Copy(configData->burnInfoOutMsgsBuffer.burnArray[0].sigma_RN, attRefOutMsgBuffer->sigma_RN);
        }else if(t_left > configData->attControlTime && configData->attRefInIsLinked){
            ...
            *attRefOutMsgBuffer = attRefInMsgBuffer;
            ...
            v3Copy(configData->burnInfoOutMsgsBuffer.burnArray[2].sigma_RN, attRefOutMsgBuffer->sigma_RN);
        }
        if(t_left < (int)configData->burnInfoOutMsgsBuffer.burnArray[2].thrustOnTime/(2*thrustConfigMsgBuffer.numThrusters) &&
           configData->burnInfoOutMsgsBuffer.burnArray[2].flag == 1){
            configData->thrustOnFlag = 1;
            configData->burnInfoOutMsgsBuffer.burnArray[2].flag = 2;
            int i = 0;
            for(i = 0;i < thrustConfigMsgBuffer.numThrusters;++i){
                thrustOnMsgBuffer->OnTimeRequest[i] = configData->burnInfoOutMsgsBuffer.burnArray[2].thrustOnTime/thrustConfigMsgBuffer.numThrusters;
            }
        }else{
            configData->thrustOnFlag = 0;
        }
        ...
        if(configData->burnInfoOutMsgsBuffer.burnArray[2].flag == 2){
            if(configData->tCurrent > (configData->burnInfoOutMsgsBuffer.burnArray[2].t+configData->burnInfoOutMsgsBuffer.burnArray[2].thrustOnTime/(2*thrustConfigMsgBuffer.numThrusters)) &&
               configData->attRefInIsLinked){
                *attRefOutMsgBuffer = attRefInMsgBuffer;
            }else{
                v3Copy(configData->burnInfoOutMsgsBuffer.burnArray[2].sigma_RN, attRefOutMsgBuffer->sigma_RN);
            }
        }else if(configData->burnInfoOutMsgsBuffer.burnArray[1].flag == 2){
            if(configData->tCurrent > (configData->burnInfoOutMsgsBuffer.burnArray[1].t+configData->burnInfoOutMsgsBuffer.burnArray[1].thrustOnTime/(2*thrustConfigMsgBuffer.numThrusters)) &&
               configData->attRefInIsLinked){
                *attRefOutMsgBuffer = attRefInMsgBuffer;
            }else{
                v3Copy(configData->burnInfoOutMsgsBuffer.burnArray[1].sigma_RN, attRefOutMsgBuffer->sigma_RN);
            ...
            if(configData->tCurrent > (configData->burnInfoOutMsgsBuffer.burnArray[0].t+configData->burnInfoOutMsgsBuffer.burnArray[0].thrustOnTime/(2*thrustConfigMsgBuffer.numThrusters)) &&
               configData->attRefInIsLinked){
                *attRefOutMsgBuffer = attRefInMsgBuffer;
            }else{
                v3Copy(configData->burnInfoOutMsgsBuffer.burnArray[0].sigma_RN, attRefOutMsgBuffer->sigma_RN);
                          ...
                          classicElements oe_d, THRArrayConfigMsgPayload thrustConfigMsgBuffer, VehicleConfigMsgPayload vehicleConfigMsgBuffer)
{
    ...
    if(M_c_dvrtp > M_c){
        configData->burnInfoOutMsgsBuffer.burnArray[0].t = (M_c_dvrtp - M_c)/n;
    }else{
        configData->burnInfoOutMsgsBuffer.burnArray[0].t = (2*M_PI + M_c_dvrtp - M_c)/n;
    }
    // 2. calculate t_dvrta
    ...
    if(M_c_dvrta > M_c){
        configData->burnInfoOutMsgsBuffer.burnArray[1].t = (M_c_dvrta - M_c)/n;
    }else{
        configData->burnInfoOutMsgsBuffer.burnArray[1].t = (2*M_PI + M_c_dvrta - M_c)/n;
    }
    // 3. calculate t_dvn
    ...
    if(M_c_dvn > M_c){
        configData->burnInfoOutMsgsBuffer.burnArray[2].t = (M_c_dvn - M_c)/n;
    }else{
        configData->burnInfoOutMsgsBuffer.burnArray[2].t = (2*M_PI + M_c_dvn - M_c)/n;
    }
    ...
    if(configData->burnInfoOutMsgsBuffer.burnArray[0].t < configData->burnInfoOutMsgsBuffer.burnArray[1].t){
        dM = dM
           - 3.0/2.0*n/oe_c.a*da*configData->burnInfoOutMsgsBuffer.burnArray[0].t
           - 3.0/2.0*n/oe_c.a*(da+2.0*oe_c.a*oe_c.a/h*p/rp*dvtp_mag)*(configData->burnInfoOutMsgsBuffer.burnArray[1].t - configData->burnInfoOutMsgsBuffer.burnArray[0].t);
    }else{
        dM = dM
           - 3.0/2.0*n/oe_c.a*da*configData->burnInfoOutMsgsBuffer.burnArray[1].t
           - 3.0/2.0*n/oe_c.a*(da+2.0*oe_c.a*oe_c.a/h*p/ra*dvta_mag)*(configData->burnInfoOutMsgsBuffer.burnArray[0].t - configData->burnInfoOutMsgsBuffer.burnArray[1].t);
    }
    ...
    if(configData->burnInfoOutMsgsBuffer.burnArray[2].t - configData->burnInfoOutMsgsBuffer.burnArray[0].t < configData->attControlTime &&
       configData->burnInfoOutMsgsBuffer.burnArray[0].t - configData->burnInfoOutMsgsBuffer.burnArray[2].t < configData->attControlTime){
        configData->burnInfoOutMsgsBuffer.burnArray[0].thrustOnTime = sqrt(dvrp_mag*dvrp_mag+dvtp_mag*dvtp_mag+dvn_mag*dvn_mag)* vehicleConfigMsgBuffer.massSC/thrustConfigMsgBuffer.thrusters[0].maxThrust;
        configData->burnInfoOutMsgsBuffer.burnArray[1].thrustOnTime = sqrt(dvra_mag*dvra_mag+dvta_mag*dvta_mag)* vehicleConfigMsgBuffer.massSC/thrustConfigMsgBuffer.thrusters[0].maxThrust;
        configData->burnInfoOutMsgsBuffer.burnArray[2].thrustOnTime = 0.0;
        configData->burnInfoOutMsgsBuffer.burnArray[2].flag = 3;
    }else if(configData->burnInfoOutMsgsBuffer.burnArray[2].t - configData->burnInfoOutMsgsBuffer.burnArray[1].t < configData->attControlTime &&
             configData->burnInfoOutMsgsBuffer.burnArray[1].t - configData->burnInfoOutMsgsBuffer.burnArray[2].t < configData->attControlTime){
        configData->burnInfoOutMsgsBuffer.burnArray[0].thrustOnTime = sqrt(dvrp_mag*dvrp_mag+dvtp_mag*dvtp_mag)* vehicleConfigMsgBuffer.massSC/thrustConfigMsgBuffer.thrusters[0].maxThrust;
        configData->burnInfoOutMsgsBuffer.burnArray[1].thrustOnTime = sqrt(dvra_mag*dvra_mag+dvta_mag*dvta_mag+dvn_mag*dvn_mag)* vehicleConfigMsgBuffer.massSC/thrustConfigMsgBuffer.thrusters[0].maxThrust;
        configData->burnInfoOutMsgsBuffer.burnArray[2].thrustOnTime = 0.0;
        configData->burnInfoOutMsgsBuffer.burnArray[2].flag = 3;
    }else{
        configData->burnInfoOutMsgsBuffer.burnArray[0].thrustOnTime = sqrt(dvrp_mag*dvrp_mag+dvtp_mag*dvtp_mag)* vehicleConfigMsgBuffer.massSC/thrustConfigMsgBuffer.thrusters[0].maxThrust;
        configData->burnInfoOutMsgsBuffer.burnArray[1].thrustOnTime = sqrt(dvra_mag*dvra_mag+dvta_mag*dvta_mag)* vehicleConfigMsgBuffer.massSC/thrustConfigMsgBuffer.thrusters[0].maxThrust;
        configData->burnInfoOutMsgsBuffer.burnArray[2].thrustOnTime = dvn_mag* vehicleConfigMsgBuffer.massSC/thrustConfigMsgBuffer.thrusters[0].maxThrust;
    }
    ...
    if(configData->burnInfoOutMsgsBuffer.burnArray[0].thrustOnTime/thrustConfigMsgBuffer.numThrusters < 1.0){
        configData->burnInfoOutMsgsBuffer.burnArray[0].flag = 3;
    }
    if(configData->burnInfoOutMsgsBuffer.burnArray[1].thrustOnTime/thrustConfigMsgBuffer.numThrusters < 1.0){
        configData->burnInfoOutMsgsBuffer.burnArray[1].flag = 3;
    }
    if(configData->burnInfoOutMsgsBuffer.burnArray[2].thrustOnTime/thrustConfigMsgBuffer.numThrusters < 1.0){
        configData->burnInfoOutMsgsBuffer.burnArray[2].flag = 3;
    }
    ...
    // calculate dcm_RN
    double M_d_dvrtp = M_d + configData->burnInfoOutMsgsBuffer.burnArray[0].t*n;
    ...
    C2MRP(dcm_TN_dvrtp, configData->burnInfoOutMsgsBuffer.burnArray[0].sigma_RN);
    
    /* calculate sigma_dvrta_RN */
    double M_d_dvrta = M_d + configData->burnInfoOutMsgsBuffer.burnArray[1].t*n;
    ...
    // in case two burns are combined, target attitude also has to be adjusted
    if(configData->burnInfoOutMsgsBuffer.burnArray[2].t - configData->burnInfoOutMsgsBuffer.burnArray[1].t < configData->attControlTime &&
       configData->burnInfoOutMsgsBuffer.burnArray[1].t - configData->burnInfoOutMsgsBuffer.burnArray[2].t < configData->attControlTime){
    ...
    C2MRP(dcm_TN_dvrta, configData->burnInfoOutMsgsBuffer.burnArray[1].sigma_RN);
    
    /* calculate sigma_dvn_RN */
    double M_d_dvn = M_d + configData->burnInfoOutMsgsBuffer.burnArray[2].t*n;
    ...
    if(configData->burnInfoOutMsgsBuffer.burnArray[2].t - configData->burnInfoOutMsgsBuffer.burnArray[0].t < configData->attControlTime &&
       configData->burnInfoOutMsgsBuffer.burnArray[0].t - configData->burnInfoOutMsgsBuffer.burnArray[2].t < configData->attControlTime){
        C2MRP(dcm_TN_dvrtp, configData->burnInfoOutMsgsBuffer.burnArray[2].sigma_RN);
    }else if(configData->burnInfoOutMsgsBuffer.burnArray[2].t - configData->burnInfoOutMsgsBuffer.burnArray[1].t < configData->attControlTime &&
             configData->burnInfoOutMsgsBuffer.burnArray[1].t - configData->burnInfoOutMsgsBuffer.burnArray[2].t < configData->attControlTime){
        C2MRP(dcm_TN_dvrta, configData->burnInfoOutMsgsBuffer.burnArray[2].sigma_RN);
    }else{
        // calculate dcm_TN = dcm_TR * dcm_RN
        double dcm_TN_dvn[3][3];
        m33MultM33(dcm_TR,dcm_RN,dcm_TN_dvn);
        C2MRP(dcm_TN_dvn, configData->burnInfoOutMsgsBuffer.burnArray[2].sigma_RN);
    }
    ...
    if(configData->burnInfoOutMsgsBuffer.burnArray[1].flag == 0){
        configData->burnInfoOutMsgsBuffer.burnArray[1].flag = 1;
    }
    if(configData->burnInfoOutMsgsBuffer.burnArray[2].flag == 0){
        configData->burnInfoOutMsgsBuffer.burnArray[2].flag = 1;
    }
}
