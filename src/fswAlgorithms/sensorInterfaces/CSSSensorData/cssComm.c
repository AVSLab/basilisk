 ...
 */
void
SelfInit_cssProcessTelem(CSSConfigData* configData, int64_t moduleID)
{
 ...
 */
void
Reset_cssProcessTelem(CSSConfigData* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    if (configData->maxSensorValue == 0) {
        _bskLog(configData->bskLogger,
                BSK_WARNING,
                "Max CSS sensor value configured to zero! CSS sensor values will be normalized by zero, inducing faux "
                "saturation!");
    }
 ...
 */
void
Update_cssProcessTelem(CSSConfigData* configData, uint64_t callTime, int64_t moduleID)
{
    uint32_t i, j;
    ...
    for (i = 0; i < configData->numSensors; i++) {
        ...
        ChebyDiffFactor = 0.0;
        ChebyDiffFactor = configData->chebyCount > 0 ? ChebyPrev * configData->kellyCheby[0]
                                                     : ChebyDiffFactor; /* if only first order correction */
        ChebyDiffFactor = configData->chebyCount > 1 ? ChebyNow * configData->kellyCheby[1] + ChebyDiffFactor
