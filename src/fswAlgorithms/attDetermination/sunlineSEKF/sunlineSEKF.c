 ...
 */
void
SelfInit_sunlineSEKF(sunlineSEKFConfig* configData, int64_t moduleID)
{
 ...
 */
void
Reset_sunlineSEKF(sunlineSEKFConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    mScale(configData->qProcVal,
           configData->procNoise,
           configData->numStates - 3,
           configData->numStates - 3,
           configData->procNoise);
 ...
 */
void
Update_sunlineSEKF(sunlineSEKFConfig* configData, uint64_t callTime, int64_t moduleID)
{
    double newTimeTag;
    ...
    memmove(sunlineDataOutBuffer.covar, configData->covar, EKF_N_STATES_SWITCH * EKF_N_STATES_SWITCH * sizeof(double));
...
*/
void
sunlineTimeUpdate(sunlineSEKFConfig* configData, double updateTime)
{
    ...
    double qGammaT[(EKF_N_STATES_SWITCH - 3) * EKF_N_STATES_SWITCH],
      gammaQGammaT[EKF_N_STATES_SWITCH * EKF_N_STATES_SWITCH];
    ...
    sunlineStateSTMProp(
      configData->dynMat, configData->bVec_B, configData->dt, configData->state, configData->stateTransition);
    ...
    mMultM(configData->covar,
           ...
           EKF_N_STATES_SWITCH,
           covPhiT);
    mMultM(configData->stateTransition,
           EKF_N_STATES_SWITCH,
           EKF_N_STATES_SWITCH,
           covPhiT,
           EKF_N_STATES_SWITCH,
 ...
 */
void
sunlineStateSTMProp(double dynMat[EKF_N_STATES_SWITCH * EKF_N_STATES_SWITCH],
...
void
sunlineDynMatrix(double states[EKF_N_STATES_SWITCH], double bVec[SKF_N_STATES], double dt, double* dynMat)
{
    ...
    m33Scale(-1, skewOmega, skewOmega); // bring to omega_SB with negative sign
    v3Tilde(states, skewStates);
    m33Scale(-1, skewStates, skewStates); // bring to omega_SB with negative sign
 ...
 */
void
sunlineMeasUpdate(sunlineSEKFConfig* configData, double updateTime)
{
    /*! - Compute the valid observations and the measurement model for all observations*/
    int numObsInt = (int)configData->numObs;
    sunlineHMatrixYMeas(configData->state,
                        configData->numCSSTotal,
                        configData->cssSensorInBuffer.CosValue,
                        configData->sensorUseThresh,
                        configData->cssNHat_B,
                        configData->obs,
                        configData->yMeas,
                        &(numObsInt),
                        configData->measMat);
    ...
    sunlineKalmanGain(
      configData->covarBar, configData->measMat, configData->qObsVal, configData->numObs, configData->kalmanGain);
    ...
    if (vMaxAbs(configData->covar, EKF_N_STATES_SWITCH * EKF_N_STATES_SWITCH) > configData->eKFSwitch) {
        ...
        sunlineCKFUpdate(configData->xBar,
                         configData->kalmanGain,
                         configData->covarBar,
                         configData->qObsVal,
                         configData->numObs,
                         configData->yMeas,
         ...
         * the filter update */
        sunlineSEKFUpdate(configData->kalmanGain,
                          configData->covarBar,
                          configData->qObsVal,
                          configData->numObs,
                          configData->yMeas,
...
void
sunlineCKFUpdate(double xBar[EKF_N_STATES_SWITCH],
 ...
 */
void
sunlineSEKFUpdate(double kalmanGain[EKF_N_STATES_SWITCH * MAX_N_CSS_MEAS],
                  double covarBar[EKF_N_STATES_SWITCH * EKF_N_STATES_SWITCH],
                  double qObsVal,
                  size_t numObs,
                  double yObs[MAX_N_CSS_MEAS],
...
void
sunlineHMatrixYMeas(double states[EKF_N_STATES_SWITCH],
                    size_t numCSS,
...
void
sunlineKalmanGain(double covarBar[EKF_N_STATES_SWITCH * EKF_N_STATES_SWITCH],
                  double hObs[MAX_N_CSS_MEAS * EKF_N_STATES_SWITCH],
                  double qObsVal,
                  size_t numObs,
                  double* kalmanGain)
...
void
sunlineSEKFSwitch(double* bVec_B, double* states, double* covar)
{
    ...
    mMultM(
      switchMat, EKF_N_STATES_SWITCH, EKF_N_STATES_SWITCH, covar, EKF_N_STATES_SWITCH, EKF_N_STATES_SWITCH, switchMatP);
    mTranspose(switchMat, EKF_N_STATES_SWITCH, EKF_N_STATES_SWITCH, switchMat);
    mMultM(
      switchMatP, EKF_N_STATES_SWITCH, EKF_N_STATES_SWITCH, switchMat, EKF_N_STATES_SWITCH, EKF_N_STATES_SWITCH, covar);
    return;
}
 ...
 */
void
sunlineSEKFComputeDCM_BS(double sunheading[SKF_N_STATES_HALF], double bVec[SKF_N_STATES_HALF], double* dcm)
{
