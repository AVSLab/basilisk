...
def StatesPlot(x, Pflat, show_plots):
    ...
    for i in range(len(Pflat[:, 0])):
        t[i] = x[i, 0] * 1e-9
        P[i, :, :] = Pflat[i, 1 : (numStates * numStates + 1)].reshape(
            [numStates, numStates]
        )
    ...
    unitTestSupport.writeFigureLaTeX(
        "StatesPlot",
        "State error and covariance",
        plt,
        "height=0.9\\textwidth, keepaspectratio",
        path,
    )
    if show_plots:
        plt.show()
...
def StatesPlotCompare(x, x2, Pflat, Pflat2, show_plots):
    ...
    for i in range(len(Pflat[:, 0])):
        ...
        P2[i, :, :] = Pflat2[i, 1 : (numStates * numStates + 1)].reshape(
            [numStates, numStates]
        )
    ...
    unitTestSupport.writeFigureLaTeX(
        "StatesCompare",
        "State error and covariance vs expected Values",
        plt,
        "height=0.9\\textwidth, keepaspectratio",
        path,
    )
...
def StatesVsExpected(stateLog, expectedStateArray, show_plots):
    ...
    unitTestSupport.writeFigureLaTeX(
        "StatesExpected",
        "States vs true states in static case",
        plt,
        "height=0.9\\textwidth, keepaspectratio",
        path,
    )
