 ...
 */
void
SelfInit_sunlineEKF(sunlineEKFConfig* configData, int64_t moduleID)
{
 ...
 */
void
Reset_sunlineEKF(sunlineEKFConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    if (cssConfigInBuffer.nCSS > MAX_N_CSS_MEAS) {
        _bskLog(configData->bskLogger,
                BSK_ERROR,
                "sunlineEKF.cssConfigInMsg.nCSS must not be greater than "
 ...
 */
void
Update_sunlineEKF(sunlineEKFConfig* configData, uint64_t callTime, int64_t moduleID)
{
    double newTimeTag;
    ...
    memmove(sunlineDataOutBuffer.covar, configData->covar, SKF_N_STATES * SKF_N_STATES * sizeof(double));
...
*/
void
sunlineTimeUpdate(sunlineEKFConfig* configData, double updateTime)
{
    ...
    //    double
    //    Gamma[6][3]={{configData->dt*configData->dt/2,0,0},{0,configData->dt*configData->dt/2,0},{0,0,configData->dt*configData->dt/2},{configData->dt,0,0},{0,configData->dt,0},{0,0,configData->dt}};
    ...
    mMultMt(
      configData->procNoise, SKF_N_STATES_HALF, SKF_N_STATES_HALF, Gamma, SKF_N_STATES, SKF_N_STATES_HALF, qGammaT);
 ...
 */
void
sunlineStateSTMProp(double dynMat[SKF_N_STATES * SKF_N_STATES], double dt, double* stateInOut, double* stateTransition)
{
...
void
sunlineDynMatrix(double states[SKF_N_STATES], double dt, double* dynMat)
{
 ...
 */
void
sunlineMeasUpdate(sunlineEKFConfig* configData, double updateTime)
{
    /*! - Compute the valid observations and the measurement model for all observations*/
    sunlineHMatrixYMeas(configData->state,
                        (int)configData->numCSSTotal,
    ...
    if (vMaxAbs(configData->covar, SKF_N_STATES * SKF_N_STATES) > configData->eKFSwitch) {
         ...
         * the filter update */
        sunlineEKFUpdate(configData->kalmanGain,
                         configData->covarBar,
                         configData->qObsVal,
                         configData->numObs,
                         configData->yMeas,
...
void
sunlineCKFUpdate(double xBar[SKF_N_STATES],
 ...
 */
void
sunlineEKFUpdate(double kalmanGain[SKF_N_STATES * MAX_N_CSS_MEAS],
                 double covarBar[SKF_N_STATES * SKF_N_STATES],
                 double qObsVal,
                 int numObs,
                 double yObs[MAX_N_CSS_MEAS],
...
void
sunlineHMatrixYMeas(double states[SKF_N_STATES],
                    int numCSS,
...
void
sunlineKalmanGain(double covarBar[SKF_N_STATES * SKF_N_STATES],
                  double hObs[MAX_N_CSS_MEAS * SKF_N_STATES],
                  double qObsVal,
                  int numObs,
                  double* kalmanGain)
