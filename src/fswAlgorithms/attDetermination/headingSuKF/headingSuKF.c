 ...
 */
void
SelfInit_headingSuKF(HeadingSuKFConfig* configData, int64_t moduleID)
{
 ...
 */
void
Reset_headingSuKF(HeadingSuKFConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    mSetZero(configData->SP, (size_t)configData->countHalfSPs * 2 + 1, (size_t)configData->numStates);
    ...
    /*! - User a cholesky decomposition to obtain the sBar and sQnoise matrices for use in
          filter at runtime*/
    mCopy(configData->covarInit, (size_t)configData->numStates, (size_t)configData->numStates, configData->covar);
    mCopy(configData->covar, (size_t)configData->numStates, (size_t)configData->numStates, configData->sBar);
    ukfCholDecomp(configData->sBar, configData->numStates, configData->numStates, tempMatrix);
    mCopy(tempMatrix, (size_t)configData->numStates, (size_t)configData->numStates, configData->sBar);
    ukfCholDecomp(configData->qNoise, configData->numStates, configData->numStates, configData->sQnoise);
    mTranspose(configData->sQnoise, (size_t)configData->numStates, (size_t)configData->numStates, configData->sQnoise);
 ...
 */
void
Update_headingSuKF(HeadingSuKFConfig* configData, uint64_t callTime, int64_t moduleID)
{
    double newTimeTag;
    ...
    for (i = 0; i < configData->countHalfSPs * 2 + 1; i++) {
        vCopy(&(configData->yMeas[i * OPNAV_MEAS]), OPNAV_MEAS, tempYVec);
...
*/
void
headingStateProp(double* stateInOut, double* b_Vec, double dt)
{
...
*/
void
headingSuKFTimeUpdate(HeadingSuKFConfig* configData, double updateTime)
{
    int i, Index;
    ...
    vCopy(configData->state, (size_t)configData->numStates, &(configData->SP[0 * configData->numStates + 0]));
    ...
    mTranspose(configData->sBar, (size_t)configData->numStates, (size_t)configData->numStates, sBarT);
    ...
    mSetZero(
      AT, (2 * (size_t)configData->countHalfSPs + (size_t)configData->numStates), (size_t)configData->countHalfSPs);
    ...
    for (i = 0; i < 2 * configData->countHalfSPs; i++) {
        ...
        vAdd(aRow, (size_t)configData->numStates, &(configData->SP[(i + 1) * configData->numStates]), aRow);
        vScale(sqrt(configData->wC[i + 1]), aRow, (size_t)configData->numStates, aRow);
        memcpy((void*)&AT[i * configData->numStates], (void*)aRow, (size_t)configData->numStates * sizeof(double));
    }
    ...
    memcpy(&AT[2 * configData->countHalfSPs * configData->numStates],
           configData->sQnoise,
           (size_t)configData->numStates * (size_t)configData->numStates * sizeof(double));
    ...
    mTranspose(sBarT, (size_t)configData->numStates, (size_t)configData->numStates, configData->sBar);
    ...
    ukfCholDownDate(configData->sBar, xErr, configData->wC[0], configData->numStates, sBarUp);
    ...
    mTranspose(configData->sBar, (size_t)configData->numStates, (size_t)configData->numStates, configData->covar);
...
/*! This method computes what the expected measurement vector is for each opnave measurement.  All data is transacted
 from the main data structure for the model because there are many variables that would have to be updated otherwise.

 @param configData The configuration data associated with the heading estimator

 */
void
headingSuKFMeasModel(HeadingSuKFConfig* configData)
{
    ...
    mTranspose(configData->yMeas, OPNAV_MEAS, (size_t)configData->countHalfSPs * 2 + 1, configData->yMeas);
}
 ...
 */
void
headingSuKFMeasUpdate(HeadingSuKFConfig* configData, double updateTime)
{
    ...
    mSetZero(AT, (size_t)configData->countHalfSPs * 2 + OPNAV_MEAS, OPNAV_MEAS);
    for (int i = 0; i < configData->countHalfSPs * 2; i++) {
        vScale(-1.0, yBar, OPNAV_MEAS, tempYVec);
        vAdd(tempYVec, OPNAV_MEAS, &(configData->yMeas[(i + 1) * OPNAV_MEAS]), tempYVec);
        vScale(sqrt(configData->wC[i + 1]), tempYVec, OPNAV_MEAS, tempYVec);
        memcpy(&(AT[i * OPNAV_MEAS]), tempYVec, OPNAV_MEAS * sizeof(double));
    }
    ...
    memcpy(&(AT[2 * configData->countHalfSPs * OPNAV_MEAS]), qChol, OPNAV_MEAS * OPNAV_MEAS * sizeof(double));
    ...
    /*! - Perform QR decomposition (only R again) of the above matrix to obtain the
          current Sy matrix*/
    ukfQRDJustR(AT, 2 * configData->countHalfSPs + OPNAV_MEAS, OPNAV_MEAS, rAT);
    ...
    ukfCholDownDate(sy, tempYVec, configData->wC[0], OPNAV_MEAS, updMat);
    ...
    for (int i = 0; i < 2 * configData->countHalfSPs + 1; i++) {
        vScale(-1.0, yBar, OPNAV_MEAS, tempYVec);
        vAdd(tempYVec, OPNAV_MEAS, &(configData->yMeas[i * OPNAV_MEAS]), tempYVec);
        vSubtract(
          &(configData->SP[i * (size_t)configData->numStates]), (size_t)configData->numStates, configData->xBar, xHat);
        vScale(configData->wC[i], xHat, (size_t)configData->numStates, xHat);
        mMultM(xHat, (size_t)configData->numStates, 1, tempYVec, 1, OPNAV_MEAS, kMat);
    ...
    mMultM(pXY, (size_t)configData->numStates, OPNAV_MEAS, syInv, OPNAV_MEAS, OPNAV_MEAS, kMat);
    ukfLInv(sy, OPNAV_MEAS, OPNAV_MEAS, syInv);
    mMultM(kMat, (size_t)configData->numStates, OPNAV_MEAS, syInv, OPNAV_MEAS, OPNAV_MEAS, kMat);
    ...
    mMultM(kMat, (size_t)configData->numStates, OPNAV_MEAS, tempYVec, OPNAV_MEAS, 1, xHat);
    ...
    mMultM(kMat, (size_t)configData->numStates, OPNAV_MEAS, sy, OPNAV_MEAS, OPNAV_MEAS, pXY);
    ...
    for (int i = 0; i < OPNAV_MEAS; i++) {
        ...
        mCopy(sBarT, (size_t)configData->numStates, (size_t)configData->numStates, configData->sBar);
    }
...
void
headingSuKFSwitch(double* bVec_B, double* states, double* covar)
{
...
void
headingSuKFComputeDCM_BS(double sunheading[HEAD_N_STATES], double bVec[HEAD_N_STATES], double* dcm)
{
