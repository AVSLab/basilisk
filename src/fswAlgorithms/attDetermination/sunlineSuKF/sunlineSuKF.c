 ...
 */
void
SelfInit_sunlineSuKF(SunlineSuKFConfig* configData, int64_t moduleID)
{
 ...
 */
void
Reset_sunlineSuKF(SunlineSuKFConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    mCopy(configData->covar, configData->numStates, configData->numStates, configData->sBar);
    badUpdate +=
 ...
 */
void
Update_sunlineSuKF(SunlineSuKFConfig* configData, uint64_t callTime, int64_t moduleID)
{
    double newTimeTag;
    ...
    memmove(sunlineDataOutBuffer.covar, configData->covar, SKF_N_STATES_SWITCH * SKF_N_STATES_SWITCH * sizeof(double));
...
*/
void
sunlineStateProp(double* stateInOut, double* b_Vec, double dt)
{
...
*/
int
sunlineSuKFTimeUpdate(SunlineSuKFConfig* configData, double updateTime)
{
    int Index, badUpdate;
 ...
 */
void
sunlineSuKFMeasModel(SunlineSuKFConfig* configData)
{
    ...
    for (i = 0; i < configData->numCSSTotal; i++) {
        ...
        if (configData->kellFits[i].cssKellFact > 0.0) {
            kellDelta -=
              exp(-pow(expectedMeas, configData->kellFits[i].cssKellPow) / configData->kellFits[i].cssKellFact);
        ...
        if (configData->cssSensorInBuffer.CosValue[i] > configData->sensorUseThresh ||
            ...
            for (j = 0; j < configData->countHalfSPs * 2 + 1; j++) {
                ...
                expectedMeas = expectedMeas > 0.0 ? expectedMeas : 0.0;
                configData->yMeas[obsCounter * (configData->countHalfSPs * 2 + 1) + j] = expectedMeas;
            }
            obsCounter++;
        }
    }
    /*! - yMeas matrix was set backwards deliberately so we need to transpose it through*/
    mTranspose(configData->yMeas, obsCounter, configData->countHalfSPs * 2 + 1, configData->yMeas);
 ...
 */
int
sunlineSuKFMeasUpdate(SunlineSuKFConfig* configData, double updateTime)
{
    uint32_t i;
    int32_t badUpdate;
...
void
sunlineSuKFSwitch(double* bVec_B, double* states, double* covar)
{
    ...
    mMultM(
      switchMat, SKF_N_STATES_SWITCH, SKF_N_STATES_SWITCH, covar, SKF_N_STATES_SWITCH, SKF_N_STATES_SWITCH, switchMatP);
    mTranspose(switchMat, SKF_N_STATES_SWITCH, SKF_N_STATES_SWITCH, switchMat);
    mMultM(
      switchMatP, SKF_N_STATES_SWITCH, SKF_N_STATES_SWITCH, switchMat, SKF_N_STATES_SWITCH, SKF_N_STATES_SWITCH, covar);
    return;
}

void
sunlineSuKFComputeDCM_BS(double sunheading[SKF_N_STATES_HALF], double bVec[SKF_N_STATES_HALF], double* dcm)
{
 ...
 */
void
sunlineSuKFCleanUpdate(SunlineSuKFConfig* configData)
{
    int i;
