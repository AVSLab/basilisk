 ...
 */
void
SelfInit_cssWlsEst(CSSWLSConfig* configData, int64_t moduleID)
{
 ...
 */
void
Reset_cssWlsEst(CSSWLSConfig* configData, uint64_t callTime, int64_t moduleID)
{
...
*/
void
computeWlsResiduals(double* cssMeas, CSSConfigMsgPayload* cssConfig, double* wlsEst, double* cssResiduals)
{
    double cssDotProd;
 ...
 */
int
computeWlsmn(int numActiveCss, double* H, double* W, double* y, double x[3])
{
    double m22[2 * 2];
    double m32[3 * 2];
    int status = 0;
 ...
 */
void
Update_cssWlsEst(CSSWLSConfig* configData, uint64_t callTime, int64_t moduleID)
{
    ...
    for (uint32_t i = 0; i < configData->cssConfigInBuffer.nCSS; i = i + 1) {
        if (InputBuffer.CosValue[i] > configData->sensorUseThresh) {
            v3Scale(configData->cssConfigInBuffer.cssVals[i].CBias,
                    configData->cssConfigInBuffer.cssVals[i].nHat_B,
    ...
    if (configData->numActiveCss == 0) /*! - If there is no sun, just quit*/
    {
        ...
        computeWlsResiduals(InputBuffer.CosValue,
                            &configData->cssConfigInBuffer,
                            sunlineOutBuffer.vehSunPntBdy,
                            configData->filtStatus.postFitRes);
    } else {
        ...
        status = computeWlsmn((int)configData->numActiveCss, H, W, y, sunlineOutBuffer.vehSunPntBdy);
        computeWlsResiduals(InputBuffer.CosValue,
                            &configData->cssConfigInBuffer,
                            sunlineOutBuffer.vehSunPntBdy,
                            configData->filtStatus.postFitRes);
