...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            6
        ),  # sun is not visible, vectors are not aligned, no specified omega_RN_B value
        (
            7
        ),  # sun is visible, vectors not aligned, nominal spin rate specified about sun heading vector
    ],
)
...
def sunSafePointTestFunction(show_plots, case):
    ...
    if (
        case == 2 or case == 6
    ):  # no sun visible, providing a near zero norm direction vector */
    ...
    # simulation end time.
    unitTestSim.ConfigureStopTime(mc.sec2nano(1.0))  # seconds to stop simulation
    ...
    if case == 1 or case == 7:
        ...
        trueVector = [sigmaTrue.tolist(), sigmaTrue.tolist(), sigmaTrue.tolist()]
    if case == 2 or case == 3 or case == 6:
        trueVector = [[0, 0, 0], [0, 0, 0], [0, 0, 0]]
    if case == 4:
    ...
    for i in range(0, len(trueVector)):
        ...
        if not unitTestSupport.isArrayEqual(
            dataLog.sigma_BR[i], trueVector[i], 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: "
    ...
    if case == 1 or case == 3 or case == 4 or case == 5 or case == 6:
        trueVector = [omega_BN_B.tolist(), omega_BN_B.tolist(), omega_BN_B.tolist()]
    if case == 2 or case == 7:
        trueVector = [
    ...
    for i in range(0, len(trueVector)):
        # check a vector values
        if not unitTestSupport.isArrayEqual(
            dataLog.omega_BR_B[i], trueVector[i], 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: "
    ...
    if case == 2 or case == 7:
        trueVector = [omega_RN_B_Search, omega_RN_B_Search, omega_RN_B_Search]
    # compare the module results to the truth values
    for i in range(0, len(trueVector)):
        # check a vector values
        if not unitTestSupport.isArrayEqual(
            dataLog.omega_RN_B[i], trueVector[i], 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: "
                + module.ModelTag
                + " Module failed omega_RN_B unit test at t="
                + str(dataLog.times()[i] * mc.NANO2SEC)
                + "sec\n"
            )
    ...
    for i in range(0, len(trueVector)):
        # check a vector values
        if not unitTestSupport.isArrayEqual(
            dataLog.domega_RN_B[i], trueVector[i], 3, accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: "
                + module.ModelTag
                + " Module failed domega_RN_B unit test at t="
                + str(dataLog.times()[i] * mc.NANO2SEC)
                + "sec\n"
            )
