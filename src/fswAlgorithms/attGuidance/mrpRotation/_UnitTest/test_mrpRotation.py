...
def run(show_plots, cmdStateFlag, testReset):
    ...
    sigma_RR0 = np.array([0.3, 0.5, 0.0])
    ...
    unitTestSupport.writeTeXSnippet(
        "omega_RR0_R", str(omega_RR0_R * mc.R2D) + "deg/sec", path
    )
    ...
    if cmdStateFlag:
        ...
        unitTestSupport.writeTeXSnippet(
            "omega_RR0_RCmd", str(omega_RR0_R * mc.R2D) + "deg/sec", path
        )
    ...
    if testReset:
        module.Reset(1)
        unitTestSim.ConfigureStopTime(
            mc.sec2nano(totalTestSimTime + 1.0)
        )  # seconds to stop simulation
        unitTestSim.ExecuteSimulation()
    ...
    (
        ...
        sigma_RR0, omega_RR0_R, RefStateInData, updateTime, cmdStateFlag, testReset
    )
