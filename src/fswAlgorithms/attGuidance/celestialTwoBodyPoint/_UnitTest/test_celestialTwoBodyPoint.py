...
def secBodyCelestialTwoBodyPointTestFunction(show_plots):
    ...
    # set the filtered output truth states
    trueVector = [
        [0.474475084038,  0.273938317493,  0.191443718765],
        [0.474475084038,  0.273938317493,  0.191443718765],
        [0.474475084038,  0.273938317493,  0.191443718765]
    ]
    ...
    for i in range(0, len(trueVector)):
        # check a vector values
        # print("{:.12}, {:.12}, {:.12}".format(moduleOutput[i,1], moduleOutput[i,2], moduleOutput[i,3]))
        if not unitTestSupport.isArrayEqual(moduleOutput[i], trueVector[i], 3, accuracy):
            testFailCount += 1
            testMessages.append("FAILED: " + moduleWrap.ModelTag + " Module failed " +
                                moduleOutputName + " unit test at t=" +
    ...
    # set the filtered output truth states
    trueVector = [
        [1.59336987e-04,   2.75979758e-04,   2.64539877e-04],
        [1.59336987e-04,   2.75979758e-04,   2.64539877e-04],
        [1.59336987e-04,   2.75979758e-04,   2.64539877e-04]
    ]
    # compare the module results to the truth values
    ...
    # set the filtered output truth states
    trueVector = [
        [-2.12284893e-07,   5.69968291e-08,  -4.83648052e-08],
        [-2.12284893e-07,   5.69968291e-08,  -4.83648052e-08],
        [-2.12284893e-07,   5.69968291e-08,  -4.83648052e-08]
    ]
