...
def computeCelestialTwoBodyPoint(R_P1, v_P1, a_P1, R_P2, v_P2, a_P2):
    ...
    omega_RN_R = np.array(
        [np.dot(r3_hat, dr2_hat), np.dot(r1_hat, dr3_hat), np.dot(r2_hat, dr1_hat)]
    )
    ...
    ddr2_hat = (
        np.cross(ddr3_hat, r1_hat)
        + np.cross(ddr1_hat, r3_hat)
        + 2 * np.cross(dr3_hat, dr1_hat)
    )

    # Angular Acceleration computation
    domega_RN_R = np.array(
        [
            np.dot(dr3_hat, dr2_hat)
            + np.dot(r3_hat, ddr2_hat)
            - np.dot(omega_RN_R, dr1_hat),
            np.dot(dr1_hat, dr3_hat)
            + np.dot(r1_hat, ddr3_hat)
            - np.dot(omega_RN_R, dr2_hat),
            np.dot(dr2_hat, dr1_hat)
            + np.dot(r2_hat, ddr1_hat)
            - np.dot(omega_RN_R, dr3_hat),
        ]
    )
...
def celestialTwoBodyPointTestFunction(show_plots):
    ...
    r_BN_N = np.array([0.0, 0.0, 0.0])
    v_BN_N = np.array([0.0, 0.0, 0.0])
    celPositionVec = r
    celVelocityVec = v
    ...
    sigma_RN, omega_RN_N, domega_RN_N = computeCelestialTwoBodyPoint(
        R_P1, v_P1, a_P1, R_P2, v_P2, a_P2
    )
    ...
    for i in range(0, len(moduleOutput)):
        if not unitTestSupport.isArrayEqual(moduleOutput[i], domega_RN_N, 3, accuracy):
            ...
            testMessages.append(
                "FAILED: "
                + module.ModelTag
                + " Module failed domega_RN_N unit test at t="
                + str(moduleOutput[i, 0] * macros.NANO2SEC)
                + "sec\n"
            )
...
def secBodyCelestialTwoBodyPointTestFunction(show_plots):
    ...
    secPositionVec = [500.0, 500.0, 500.0]
    SecBodyData.r_BdyZero_N = secPositionVec
    secVelocityVec = [0.0, 0.0, 0.0]
