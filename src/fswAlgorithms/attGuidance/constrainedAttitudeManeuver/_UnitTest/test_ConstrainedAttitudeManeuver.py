...
class node:
    def __init__(self, sigma_BN, constraints, **kwargs):
        ...
        if s > 1:
            self.sigma_BN = shadowSetMap(
                sigma_BN, True
            )  # mapping to shadow set if |sigma| > 1
        if np.abs(s - 1) < 1e-5:
            s = 1.0
        ...
        BN = (
            np.identity(3)
            + (8 * np.matmul(sigma_tilde, sigma_tilde) - 4 * (1 - s**2) * sigma_tilde)
            / (1 + s**2) ** 2
        )
        NB = BN.transpose()
...
def generateGrid(n_start, n_goal, N, constraints, data):
        ...
        if nodes[key1].isBoundary:
            if (-i, -j, -k) in nodes:
                for key2 in nodes[(-i, -j, -k)].neighbors:
                    if (
                        nodes[key1].isFree
                        and nodes[key2].isFree
                        and key2 not in nodes[key1].neighbors
                    ):
                        nodes[key1].neighbors[key2] = nodes[key2]
    ...
    for key in nodes:
        if nodes[key].isFree:
            ...
            if abs(d1 - ds) < 1e-6:
                if np.linalg.norm(nodes[key].sigma_BN) < np.linalg.norm(
                    nodes[key_s].sigma_BN
                ):
                    ds = d1
                    n_s = nodes[key]
                    key_s = key
...
def effortEvaluation(Output, I):
    ...
    for n in range(len(Output.T) - 1):
        ...
        effort += (
            (np.linalg.norm(L_a) + np.linalg.norm(L_b))
            * (Output.T[n + 1][0] - Output.T[n][0])
            / 2
        )
...
def test_constrainedAttitudeManeuver(
    show_plots, N, keepOutFov, keepInFov, costFcnType, accuracy
):
    r"""
    ...
    **Description of Variables Being Tested**
    ...
    [testResults, testMessage] = CAMTestFunction(
        N, keepOutFov, keepInFov, costFcnType, accuracy
    )
...
def CAMTestFunction(N, keepOutFov, keepInFov, costFcnType, accuracy):
    ...
    constraints["keepOut"].append(
        constraint(PlanetInertialPosition - SCInertialPosition, "r")
    )
    constraints["keepIn"].append(
        constraint(PlanetInertialPosition - SCInertialPosition, "g")
    )
    data = {
        "keepOut_b": keepOutBoresight_B,
        "keepOut_fov": [keepOutFov],
        "keepIn_b": keepInBoresight_B,
        "keepIn_fov": [keepInFov, keepInFov],
    }
    ...
    samplingTime = unitTestSupport.samplingTime(
        simulationTime, testProcessRate, numDataPoints
    )
            ...
            for k in range(-N, N + 1):
                if (i, j, k) in nodes:
                    ...
                    if not unitTestSupport.isVectorEqual(
                        sigma_BN, sigma_BN_BSK, accuracy
                    ):
                        testFailCount += 1
                        testMessages.append(
                            "FAILED: "
                            + testModule.ModelTag
                            + " Error in the coordinates of node ({},{},{}) \n".format(
                                i, j, k
                            )
                        )
                    if not nodes[(i, j, k)].isFree == testModule.returnNodeState(
                        [i, j, k]
                    ):
                        testFailCount += 1
                        testMessages.append(
    ...
    for p in range(len(path)):
        ...
        if not unitTestSupport.isVectorEqual(sigma_BN, sigma_BN_BSK, accuracy):
            ...
            testMessages.append(
                "FAILED: "
                + testModule.ModelTag
                + " Error in waypoint number {} in path \n".format(p)
            )
    ...
    for n in range(len(Output.T)):
        ...
        sigma_BSK = np.array(
            [
                testModule.Output.X1[n][0],
                testModule.Output.X2[n][0],
                testModule.Output.X3[n][0],
            ]
        )
        sigmaDot_BSK = np.array(
            [
                testModule.Output.XD1[n][0],
                testModule.Output.XD2[n][0],
                testModule.Output.XD3[n][0],
            ]
        )
        sigmaDDot_BSK = np.array(
            [
                testModule.Output.XDD1[n][0],
                testModule.Output.XDD2[n][0],
                testModule.Output.XDD3[n][0],
            ]
        )
        ...
        if not unitTestSupport.isDoubleEqual(T, T_BSK, accuracy):
            ...
            testMessages.append(
                "FAILED: "
                + testModule.ModelTag
                + " Error in time at index #{} in trajectory \n".format(n)
            )
        if not unitTestSupport.isVectorEqual(sigma, sigma_BSK, accuracy):
            ...
            testMessages.append(
                "FAILED: "
                + testModule.ModelTag
                + " Error in sigma at index #{} in trajectory \n".format(n)
            )
        if not unitTestSupport.isVectorEqual(sigmaDot, sigmaDot_BSK, accuracy):
            ...
            testMessages.append(
                "FAILED: "
                + testModule.ModelTag
                + " Error in sigmaDot at index #{} in trajectory \n".format(n)
            )
        if not unitTestSupport.isVectorEqual(sigmaDDot, sigmaDDot_BSK, accuracy):
            ...
            testMessages.append(
                "FAILED: "
                + testModule.ModelTag
                + " Error in sigmaDDot at index #{} in trajectory \n".format(n)
            )
    ...
    for n in range(len(Output.T)):
        ...
        if not unitTestSupport.isDoubleEqual(c1, c2, accuracy):
            ...
            testMessages.append(
                "FAILED: "
                + testModule.ModelTag
                + " Error torque norm in point {} in trajectory \n".format(n)
            )
    ...
    for n in range(len(timeData)):
        t = timeData[n]
        sigma_RN = [
            Output.getStates(t, 0, 0),
            Output.getStates(t, 0, 1),
            Output.getStates(t, 0, 2),
        ]
        sigmaDot_RN = [
            Output.getStates(t, 1, 0),
            Output.getStates(t, 1, 1),
            Output.getStates(t, 1, 2),
        ]
        sigmaDDot_RN = [
            Output.getStates(t, 2, 0),
            Output.getStates(t, 2, 1),
            Output.getStates(t, 2, 2),
        ]
        ...
        if not unitTestSupport.isVectorEqual(sigma_RN, CAMLog.sigma_RN[n], accuracy):
            ...
            testMessages.append(
                "FAILED: "
                + testModule.ModelTag
                + " Error in attitude reference message at t = {} sec \n".format(t)
            )
        if not unitTestSupport.isVectorEqual(
            omega_RN_N, CAMLog.omega_RN_N[n], accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: "
        ...
        if not unitTestSupport.isVectorEqual(
            omegaDot_RN_N, CAMLog.domega_RN_N[n], accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: "
        ...
        if not unitTestSupport.isVectorEqual(sigma_RN, CAMLogC.sigma_RN[n], accuracy):
            ...
            testMessages.append(
                "FAILED: "
                + testModule.ModelTag
                + " Error in C attitude reference message at t = {} sec \n".format(t)
            )
        if not unitTestSupport.isVectorEqual(
            omega_RN_N, CAMLogC.omega_RN_N[n], accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: "
        ...
        if not unitTestSupport.isVectorEqual(
            omegaDot_RN_N, CAMLogC.domega_RN_N[n], accuracy
        ):
            testFailCount += 1
            testMessages.append(
                "FAILED: "
