...
def opNavPointTestFunction(show_plots, case):
    ...
    #
    planet_B = [1.0, 1.0, 0.0]
    inputOpNavData = (
        messaging.OpNavMsgPayload()
    ...
    if case == 3:  # No valid measurement
        inputOpNavData.r_BN_C = [0.0, 0.0, -1.0]
    ...
    Phi = np.arccos(
        np.dot(
            -np.array(planet_B) / np.linalg.norm(-np.array(planet_B)),
            np.array(camera_Z),
        )
    )
