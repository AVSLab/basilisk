...
*/
void
SmallBodyNavEKF::Reset(uint64_t CurrentSimNanos)
{
...
*/
void
SmallBodyNavEKF::addThrusterToFilter(Message<THROutputMsgPayload>* tmpThrusterMsg)
{
...
*/
void
SmallBodyNavEKF::readMessages(uint64_t CurrentSimNanos)
{
    /* Read in the input messages */
    if ((this->navTransInMsg.timeWritten() + this->navAttInMsg.timeWritten() +
         this->asteroidEphemerisInMsg.timeWritten() - 3 * CurrentSimNanos) == 0) {
...
*/
void
SmallBodyNavEKF::predict(uint64_t CurrentSimNanos)
{
...
*/
void
SmallBodyNavEKF::aprioriState(uint64_t CurrentSimNanos)
{
    /* First RK4 step */
...
*/
void
SmallBodyNavEKF::computeEquationsOfMotion(Eigen::VectorXd x_hat, Eigen::MatrixXd Phi)
{
    ...
    x_hat_dot_k.segment(3, 3) =
      -F_ddot * o_hat_3_tilde * x_1 - 2 * F_dot * o_hat_3_tilde * x_2 -
      pow(F_dot, 2) * o_hat_3_tilde * o_hat_3_tilde * x_1 - mu_ast * x_1 / pow(x_1.norm(), 3) +
      mu_sun * (3 * (r_SO_O / r_SO_O.norm()) * (r_SO_O / r_SO_O.norm()).transpose() - I) * x_1 / pow(r_SO_O.norm(), 3) +
      C_SRP * P_0 * (1 + rho) * (A_sc / M_sc) * o_hat_1 / pow(r_SO_O.norm(), 2) + dcm_OB * thrust_B / M_sc +
      dcm_OB * cmdForce_B / M_sc;

    /* x3_dot */
    x_hat_dot_k.segment(6, 3) =
      0.25 * ((1 - pow(x_3.norm(), 2)) * I + 2 * eigenTilde(x_3) + 2 * x_3 * x_3.transpose()) * x_4;
...
*/
void
SmallBodyNavEKF::aprioriCovar(uint64_t CurrentSimNanos)
{
 ...
 */
void
SmallBodyNavEKF::checkMRPSwitching()
{
    ...
    if (sigma_AN.norm() > 1.0) {
        ...
        Lambda.block(6, 6, 3, 3) =
          2 * sigma_AN * sigma_AN.transpose() / pow(sigma_AN.norm(), 4) - I / pow(sigma_AN.norm(), 2);
    }
...
*/
void
SmallBodyNavEKF::measurementUpdate()
{
    /* Compute Kalman gain */
    Eigen::MatrixXd K_k1;
    ...
    P_k1 = (I_full - K_k1 * H_k1) * P_k1_ * (I_full - K_k1 * H_k1).transpose() +
           K_k1 * M * R * M.transpose() * K_k1.transpose();
...
*/
void
SmallBodyNavEKF::computeDynamicsMatrix(Eigen::VectorXd x_hat)
{
    /* Create temporary state vectors for readability */
    Eigen::Vector3d x_1;
    Eigen::Vector3d x_2;
    ...
    /* x_2 partial */
    A_k.block(3, 0, 3, 3) =
      -F_ddot * o_hat_3_tilde - pow(F_dot, 2) * o_hat_3_tilde * o_hat_3_tilde - mu_ast / pow(x_1.norm(), 3) * I +
    ...
    /* x_3 partial */
    A_k.block(6, 6, 3, 3) =
      0.5 * (x_3 * x_4.transpose() - x_4 * x_3.transpose() - eigenTilde(x_4) + (x_4.transpose() * x_3) * I);
...
*/
void
SmallBodyNavEKF::UpdateState(uint64_t CurrentSimNanos)
{
...
*/
void
SmallBodyNavEKF::writeMessages(uint64_t CurrentSimNanos)
{
    ...
    eigenMatrixXd2CArray(cArray2EigenVector3d(asteroidEphemerisInMsgBuffer.r_BdyZero_N) +
                           dcm_ON.transpose() * x_hat_k.segment(0, 3),
                         navTransOutMsgBuffer.r_BN_N);
    eigenMatrixXd2CArray(cArray2EigenVector3d(asteroidEphemerisInMsgBuffer.v_BdyZero_N) +
                           dcm_ON.transpose() * x_hat_k.segment(3, 3),
    ...
    v3Copy(asteroidEphemerisOutMsgBuffer.r_BdyZero_N,
           asteroidEphemerisInMsgBuffer.r_BdyZero_N); // Not an estimated parameter
    v3Copy(asteroidEphemerisOutMsgBuffer.v_BdyZero_N,
           asteroidEphemerisInMsgBuffer.v_BdyZero_N); // Not an estimated parameter
    ...
    EphemerisMsg_C_write(
      &asteroidEphemerisOutMsgBuffer, &this->asteroidEphemerisOutMsgC, this->moduleID, CurrentSimNanos);
}
