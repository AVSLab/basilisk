...
def get_docstring(item):
    if item.function.__doc__:
        return (
            '<span style="font-family:monospace;white-space:pre-wrap;word-wrap: break-word;">'
            + textwrap.dedent(str(item.function.__doc__)).replace("``", "")
            + "</span>"
        )
    else:
        return (
...
def pytest_runtest_makereport(item, call):
    ...
    if item.config.option.htmlpath:  # don't save pictures etc. if not making a report
        ...
        if len(plt.get_fignums()) > 0:
            while (
                i > -1
            ):  # this loops makes a numbered directory per run of the same test to avoid overwrite
            ...
            for f in plt.get_fignums():
                ...
                img_src = (
                    "assets" + filename.split("assets")[1]
                )  # just want a relative (to report) path here
                extra.append(
            ...
            extra.append(
                pytest_html.extras.html("<tt> This test has no images.<br></tt>")
            )
