...
def _fieldRwTypes(field: dict) -> tuple[str, str] | None:
    ...
    if kind == "array":
        ...
        if elemCtype in _INT_CTYPES:
            base = ("int", "int")
        elif elemCtype in _FLOAT_CTYPES:
            base = ("float", "float")
        else:
            return None

        ndim = len(field.get("shape", [1]))
        readT, writeT = base
        for _ in range(ndim):
            readT = f"List[{readT}]"
            writeT = f"Sequence[{writeT}]"
        return (readT, writeT)
...
def _generateExtendBlock(meta: dict, payloadType: str) -> str:
    ...
    - A precomputed ``__fields__`` classmethod returning a
    ...
    if not fields:
        ...
        namesTuple = f"({fields[0]['name']!r},)"
    else:
        namesTuple = "(" + ", ".join(repr(f["name"]) for f in fields) + ")"
