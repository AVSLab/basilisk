...
def cleanSwigType(cppType: str) -> str:
    ...
    def stripParensIfWrapped(typeStr: str) -> str:
        ...
        Returns:
            A string with parentheses removed if they are unnecessary
        """
    ...
    def parseType(index: int):
        ...
        Returns:
            (typeStr, newIndex): A tuple of parsed type string and the next token index
        """
        parts = []
    ...
    def parseBracketBlock(index: int, openSym: str, closeSym: str,
        ...
        Returns:
            (parsedString, newIndex): Parsed contents and next token index
        """
        items = []
        while index < len(tokens):
...
def extractAttributeMap(attributeListNode):
    """
    Converts an <attributelist> XML node into a Python dict.

    Parameters:
        attributeListNode (Element): The <attributelist> element
