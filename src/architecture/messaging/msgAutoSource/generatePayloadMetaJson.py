...
def _writeDepfile(depfilePath: str, outputPath: str, tu: "clang.cindex.TranslationUnit", stubDir: str) -> None:
    """Write a Make-format dependency file listing all real headers libclang read.

    Stub headers written to *stubDir* are excluded, as they are internal
    implementation details and do not exist in the source tree.
    """
    deps = []
    for inclusion in tu.get_includes():
        p = str(Path(inclusion.include.name).resolve())
        if not p.startswith(str(Path(stubDir).resolve())):
            deps.append(p)

    # Make dependency rule: output: dep1 dep2 ...
    # Spaces in paths are escaped with a backslash.
    def _escape(p: str) -> str:
        return p.replace(" ", "\\ ")

    depLine = " \\\n  ".join(_escape(d) for d in deps)
    Path(depfilePath).write_text(f"{_escape(outputPath)}: {depLine}\n")
...
def parseStruct(
    ...
    with _StubIncludeDir() as stubs:
        ...
        if depfilePath and outputPath:
            _writeDepfile(depfilePath, outputPath, tu, stubs._tmpdir.name)
...
def main() -> None:
    ...
    ap.add_argument("--depfile",     help="Write a Make-format dependency file to this path")
    args = ap.parse_args(our_argv)

    meta = parseStruct(
        args.header, args.struct_name,
        compiler_args=clang_args,
        depfilePath=args.depfile,
