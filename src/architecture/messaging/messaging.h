...
class ReadFunctor
{
  ...
  public:
    ...
    const messageType& operator()()
    {
        if (!this->initialized) {
            ...
            bskLogger.bskLog(BSK_ERROR,
                             "In C++ read functor, you are trying to read an un-connected message of type %s\nThis "
                             "program is about to self destruct.",
                             typeid(var).name());
        }
    ...
    }; // something that can be checked so that uninitialized messages aren't read.
    ...
    bool isWritten()
    {
        if (this->initialized) {
            ...
            messageType var;
            bskLogger.bskLog(BSK_ERROR,
                             "In C++ read functor, you are checking if an unconnected msg of type %s is written.",
                             typeid(var).name());
            return false;
        }
    };
    ...
    uint64_t timeWritten()
    {
        if (!this->initialized) {
            messageType var;
            bskLogger.bskLog(BSK_ERROR,
                             "In C++ read functor, you are requesting the write time of an unconnected msg of type %s.",
                             typeid(var).name());
            return 0;
        }
    ...
    int64_t moduleID()
    {
        if (!this->initialized) {
            messageType var;
            bskLogger.bskLog(BSK_ERROR,
                             "In C++ read functor, you are requesting moduleID of an unconnected msg of type %s.",
                             typeid(var).name());
            return 0;
        }
        if (!this->headerPointer->isWritten) {
            messageType var;
            bskLogger.bskLog(BSK_ERROR,
                             "In C++ read functor, you are requesting moduleID of an unwritten msg of type %s.",
                             typeid(var).name());
            return 0;
        }
...
template<typename messageType>
ReadFunctor<messageType>
Message<messageType>::addSubscriber()
{
...
template<typename messageType>
WriteFunctor<messageType>
Message<messageType>::addAuthor()
{
    return this->write;
}
...
template<typename messageType>
messageType*
Message<messageType>::subscribeRaw(MsgHeader** msgPtr)
{
    *msgPtr = &this->header;
    this->header.isLinked = 1;
    return &this->payload;
...
template<typename messageType>
messageType*
Message<messageType>::getMsgPointers(MsgHeader** msgPtr)
{
    *msgPtr = &this->header;
    return &this->payload;
}
  ...
  public:
    ...
    Recorder(ReadFunctor<messageType>* messageReader, uint64_t timeDiff = 0)
    {
        ...
        if (!messageReader->isLinked()) {
            ...
            bskLogger.bskLog(
              BSK_ERROR,
              "In C++ read functor, you are requesting to record an un-connected input message of type %s.",
              typeid(var).name());
        }
        ...
        if (CurrentSimNanos >= this->nextUpdateTime) {
            ...
            if (!this->readMessage.isLinked() || !this->readMessage.isWritten()) {
                ...
                bskLogger.bskLog(BSK_WARNING,
                                 "Recording message of type %s that is not properly initialized or written",
                                 typeid(var).name());
            }
