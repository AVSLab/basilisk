...
int32_t ukfLInv(double *sourceMat, int32_t nRow, int32_t nCol, double *destMat)
{
	...
	if (nRow != nCol)
	{
		BSK_PRINT(MSG_WARNING, "%s", "Can't get a lower-triangular inverse of non-square matrix in ukfLInv.");
		return -1;
	}
	mat_dim = nRow;
	for (i = mat_dim - 1; i >= 0; i--)
	{
        ...
        {
            BSK_PRINT(MSG_WARNING, "%s", "Can't invert zero matrix.");
            return -1;
        }
	...
	double *destMat, int32_t *indx)
{
	...
	if (nRow != nCol)
	{
		BSK_PRINT(MSG_WARNING, "%s", "Can't get a lower-triangular inverse of non-square matrix in ukfLUD.");
		return -1;
	}
	...
	for (i = 0; i < nRow; i++)
	{
		...
		if (big < TINY)
		{
			BSK_PRINT(MSG_WARNING, "%s", "Singular matrix encountered in ukfLUD LU decomposition.");
			return -1;
		}
		vv[i] = 1.0 / big;
	}
	...
	int32_t *indx, double *bmat, double *destMat)
{
	...
	if (nRow != nCol)
	{
		BSK_PRINT(MSG_WARNING, "%s", "Can't get a linear solution of non-square matrix in ukfLUBckSlv.");
		return -1;
	}
	...
	for (i = nRow - 1; i >= 0; i--)
	{
        ...
        if (sourceMat[i*nRow + i] == 0){
            BSK_PRINT(MSG_WARNING, "%s", "Dividing by zero.");
            return -1;}
	...
	double *destMat)
{
	...
	if (nRow != nCol)
	{
		BSK_PRINT(MSG_WARNING, "%s", "Can't invert a non-square matrix in ukfMatInv.");
		return -1;
	}
	...
	double *destMat)
{
	...
	if (nRow != nCol)
	{
		BSK_PRINT(MSG_WARNING, "%s", "Can't get a lower-triangular inverse of non-square matrix in ukfCholDecomp.");
		return -1;
	}
	...
	for (i = 0; i<nRow; i++)
	{
		for (j = 0; j <= i; j++)
		{
			...
			{
                if (sigma<0){
                    BSK_PRINT(MSG_WARNING, "%s", "Invalid SQRT in ukfCholDecomp, skipping value.");
                    return -1;}
	...
	double *rOut)
{
	...
	for (i = 0; i < nStates; i++)
	{
        ...
        if (rEl2 + beta/bParam * wVec[i]*wVec[i]<0){
            BSK_PRINT(MSG_WARNING, "%s", "Invalid SQRT in ukfCholDownDate, skipping value.");
            return -1;}
