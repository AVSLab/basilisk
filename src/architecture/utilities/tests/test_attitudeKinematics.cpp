...
TEST(RigidBodyKinematics, bmatrixInverseEulerParameters){
    ...
    EXPECT_TRUE((bInvQuaternion(ep) - expected).norm() < kinematicsAccuracy);
}
...
TEST(RigidBodyKinematics, bmatrixInverseMrp){
    ...
    EXPECT_TRUE(((1 + mrp.dot(mrp))*(1 + mrp.dot(mrp))*bInvMrp(mrp) - bMrp(mrp).transpose()).norm() < kinematicsAccuracy);
...
TEST(RigidBodyKinematics, bmatrixInversePrv){
    ...
    EXPECT_TRUE((bInvPrv(prv)*bPrv(prv) - Eigen::Matrix3d::Identity()).norm() < kinematicsAccuracy);
...
TEST(RigidBodyKinematics, bmatrixEulerParameter){
    ...
    EXPECT_TRUE((bQuaternion(ep).transpose()*ep).norm() < kinematicsAccuracy);
    EXPECT_TRUE((bQuaternion(ep) - expected).norm() < kinematicsAccuracy);
}
...
TEST(RigidBodyKinematics, bMatrixMrp){
    ...
    EXPECT_TRUE((bMrp(mrp).transpose()*bMrp(mrp) - (1 + mrp.dot(mrp))*(1 + mrp.dot(mrp))*
                        Eigen::Matrix3d::Identity()).norm()< kinematicsAccuracy);
    EXPECT_TRUE((bMrp(mrp) - expected).norm() < kinematicsAccuracy);
}
...
TEST(RigidBodyKinematics, bMatrixDotMrp){
    ...
    EXPECT_TRUE((bDotMrp(mrp, dmrp) - expected).norm() < kinematicsAccuracy);
}
...
TEST(RigidBodyKinematics, bMatrixPrv){
    ...
    EXPECT_TRUE((bPrv(prv) - expected).norm() < kinematicsAccuracy);
}
...
TEST(RigidBodyKinematics, bmatrixEulerAngles321){
    ...
    EXPECT_TRUE((bEulerAngles321(euler321) - expected).norm() < kinematicsAccuracy);
}
...
TEST(RigidBodyKinematics, dcmToEulerParameter){
    ...
    EXPECT_TRUE((dcm*dcm.transpose() - Eigen::Matrix3d::Identity()).norm() < kinematicsAccuracy);
    EXPECT_TRUE((dcmToQuaternion(dcm) - expected).norm() < kinematicsAccuracy);
}
...
TEST(RigidBodyKinematics, eulerToDcmParameter){
    ...
    EXPECT_TRUE((quaternionToDcm(ep)*quaternionToDcm(ep).transpose() - Eigen::Matrix3d::Identity()).norm() < kinematicsAccuracy);
    EXPECT_TRUE((quaternionToDcm(ep) - expected).norm() < kinematicsAccuracy);
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        std::make_tuple("epToMrp", EP2MRP, quaternionToMrp),
        std::make_tuple("epToPrv", EP2PRV, quaternionToPrv),
        std::make_tuple("epToEulerAngles321", EP2Euler321, quaternionToEulerAngles321)),
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        std::make_tuple("mrpToEp", MRP2EP, mrpToQuaternion),
        std::make_tuple("prvToEp", PRV2EP, prvToQuaternion),
        std::make_tuple("eulerAnglesToEp", Euler3212EP, eulerAngles321ToQuaternion)),
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        std::make_tuple("dmrp",dMRP, mrpDot),
        std::make_tuple("dmrpToOmega",dMRP2Omega, mrpDotToOmega),
        std::make_tuple("dprv",dPRV, prvDot),
        std::make_tuple("deuler321",dEuler321, euler321Dot)),
...
TEST(RigidBodyKinematics, epDerivativeTest)
{
    ...
    EXPECT_TRUE((quaternionDot(ep, omega) - expected).norm() < kinematicsAccuracy);
}
...
TEST(RigidBodyKinematics, dmrpDerivativeTest)
{
    ...
    EXPECT_TRUE((mrpDotDot(mrp, dmrp, omega, domega) - expected).norm() < kinematicsAccuracy);
}
...
TEST(RigidBodyKinematics, dmrpToOmegaDerivativeTest)
{
    ...
    EXPECT_TRUE((mrpDotDotToOmegaDot(mrp, dmrp, ddmrp) - expected).norm() < kinematicsAccuracy);
}
...
TEST(RigidBodyKinematics, mrpSwitchTest)
{
    ...
    EXPECT_TRUE((mrpInnerSet(mrp, value) - expected).norm() < kinematicsAccuracy);
}
...
TEST(RigidBodyKinematics, mrpShadowTest)
{
    ...
    EXPECT_TRUE((mrpShadowSet(mrp) - expected).norm() < kinematicsAccuracy);
}
