...
void
InputDataSet::setXDot_0(Eigen::Vector3d XDot_0)
{
    this->XDot_0 = XDot_0;
    this->XDot_0_flag = true;
    return;
}

/*! Set the first derivative of the last point (optional) */
void
InputDataSet::setXDot_N(Eigen::Vector3d XDot_N)
{
    this->XDot_N = XDot_N;
    this->XDot_N_flag = true;
    return;
}

/*! Set the second derivative of the starting point (optional) */
void
InputDataSet::setXDDot_0(Eigen::Vector3d XDDot_0)
{
    this->XDDot_0 = XDDot_0;
    this->XDDot_0_flag = true;
    return;
}

/*! Set the second derivative of the last point (optional) */
void
InputDataSet::setXDDot_N(Eigen::Vector3d XDDot_N)
{
    this->XDDot_N = XDDot_N;
    this->XDDot_N_flag = true;
    return;
}

/*! Set the time tags for each waypoint (optional). Cannot be imposed together with avg velocity norm below */
void
InputDataSet::setT(Eigen::VectorXd T)
{
    this->T = T;
    this->T_flag = true;
    this->AvgXDot_flag = false;
...
void
InputDataSet::setAvgXDot(double AvgXDot)
{
    this->AvgXDot = AvgXDot;
    this->AvgXDot_flag = true;
    this->T_flag = false;
    return;
}
...
void
InputDataSet::setW(Eigen::VectorXd W)
{
    this->W = W;
    this->W_flag = true;
    return;
...
void
OutputDataSet::getData(double T, double x[3], double xDot[3], double xDDot[3])
{
...
double
OutputDataSet::getStates(double T, int derivative, int index)
{
    int N = (int)this->T.size() - 1;
    double Ttot = this->T[N];
...
/*! This function takes the Input structure, performs the BSpline interpolation and outputs the result into Output
 * structure */
void
interpolate(InputDataSet Input, int Num, int P, OutputDataSet* Output)
{
    Output->P = P;
    ...
    // T = time tags; if not specified, it is computed from a cartesian distance assuming a constant velocity norm on
    // average
    ...
    if (Input.T_flag == true) {
        ...
        T[0] = 0;
        for (int n = 1; n < N + 1; n++) {
            T[n] = T[n - 1] + pow((pow(Input.X1[n] - Input.X1[n - 1], 2) + pow(Input.X2[n] - Input.X2[n - 1], 2) +
                                   pow(Input.X3[n] - Input.X3[n - 1], 2)),
                                  0.5);
            S += T[n] - T[n - 1];
        }
    }
    ...
    if (P > N + K) {
        std::cout << "Error in BSpline.interpolate: \n the desired polynomial order P is too high. Mass matrix A will "
                     "be singular. \n";
    }
...
/*! This function takes the Input structure, performs the BSpline LS approximation and outputs the result into Output
 * structure */
void
approximate(InputDataSet Input, int Num, int Q, int P, OutputDataSet* Output)
{
    Output->P = P;
    ...
    // The maximum polynomial order is N + K. If a higher order is requested, print a BSK_ERROR
    if (P > Q) {
        std::cout << "Error in BSpline.approximate: \n the desired polynomial order P can't be higher than the number "
                     "of control points Q. \n";
    }
...
/*! This function calculates the basis functions NN of order P, and derivatives NN1, NN2, for a given time t and knot
 * vector U */
void
basisFunction(double t, Eigen::VectorXd U, int I, int P, double* NN, double* NN1, double* NN2)
{
