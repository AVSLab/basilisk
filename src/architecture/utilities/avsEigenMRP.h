...
class MRP : public MRPBase<MRP<_Scalar, _Options>>
{
    ...
    enum
    {
        IsAligned = internal::traits<MRP>::IsAligned
    };
  ...
  public:
     ...
     */
    inline MRP(const Scalar& x, const Scalar& y, const Scalar& z)
      : m_coeffs(x, y, z)
    {
    }

    /** Constructs and initialize a MRP from the array data */
    inline MRP(const Scalar* data)
      : m_coeffs(data)
    {
    }
...
MRPBase<Derived>::operator*=(const MRPBase<OtherDerived>& other)
{
...
template<class Derived>
EIGEN_STRONG_INLINE MRPBase<Derived>&
MRPBase<Derived>::operator=(const MRPBase<Derived>& other)
{
    coeffs() = other.coeffs();
    return derived();
}
...
template<class Derived>
EIGEN_STRONG_INLINE Derived&
MRPBase<Derived>::operator=(const AngleAxisType& aa)
{
    using std::tan;
...
inline Derived&
MRPBase<Derived>::operator=(const MatrixBase<MatrixDerived>& xpr)
{
    EIGEN_STATIC_ASSERT(
...
template<typename Derived1, typename Derived2>
inline Derived&
MRPBase<Derived>::setFromTwoVectors(const MatrixBase<Derived1>& a, const MatrixBase<Derived2>& b)
{
    using std::acos;
    using std::tan;
