...
/**
   Provide the quaternion (Euler parameter) Set which corresponds to performing to successive rotations B1 and B2
...
Eigen::Vector3d addMrp(const Eigen::Vector3d& mrp1, const Eigen::Vector3d& mrp2){
    ...
    if (std::abs(denominator) < 0.1) {
        sigma1 = mrpShadowSet(mrp1); // Shadow set
...
/**
 * Return the matrix which relates the derivative of the quaternion (Euler parameter) set to the body angular
...
/**
 * Return the matrix which relates the body angular velocity vector to the derivative of the quaternion (Euler parameter) set
 * dQ/dt = 1/2 [B(Q)] omega
...
/**
 * Translate the direction cosine matrix into the corresponding quaternion (Euler parameter) set,
 * where the first component is the scalar quaternion >= 0. Transformation is done
...
Eigen::Vector3d dcmToMrp(const Eigen::Matrix3d& dcm){
    Eigen::Vector4d ep(dcmToQuaternion(dcm));
...
Eigen::Vector3d dcmToPrv(const Eigen::Matrix3d& dcm){
    return quaternionToPrv(dcmToQuaternion(dcm));
}
...
/**
 * Return the quaternion (Euler parameter) derivative for a given quaternion vector and body
...
Eigen::Vector4d quaternionDot(const Eigen::Vector4d& ep, const Eigen::Vector3d& omega){
    Eigen::Matrix<double, 4, 3> B(bQuaternion(ep));
...
Eigen::Vector3d mrpDotDot(const Eigen::Vector3d& mrp,
    ...
    Eigen::Matrix3d B(bMrp(mrp));
    Eigen::Matrix3d Bdot(bDotMrp(mrp, dmrp));
...
Eigen::Vector3d euler321Dot(const Eigen::Vector3d& euler321, const Eigen::Vector3d& omega){
    return bEulerAngles321(euler321)*omega;
}

/**
 * Return the direction cosine matrix in terms of the quaternion (Euler parameter) set. The first element is the
 * scalar quaternion, while the remain three elements form the quaternion set.
 * @param ep
 * @return Eigen::Matrix3d
 */
...
/**
 * Translate a quaternion (Euler parameter) set into an mrp vector.
 * @param ep
 * @return Eigen::Vector3d
 */
...
/**
 * Translates a quaternion (Euler parameter) set into a principal rotation vector.
 * @param ep
 * @return Eigen::Vector3d
 */
...
/**
 * Translate a quaternion (Euler parameter) set into the corresponding 321 Euler angle set.
 * @param ep
 * @return Eigen::Vector3d
 */
...
/**
 * Translate the mrp vector into the quaternion (Euler parameter) set.
 * @param mrp
 * @return Eigen::Vector4d
 */
...
Eigen::Vector3d mrpToEulerAngles321(const Eigen::Vector3d& mrp){
    return quaternionToEulerAngles321(mrpToQuaternion(mrp));
}
...
/**
 * Translate a principal rotation vector into a quaternion (Euler parameter) set.
 * @param prv
 * @return Eigen::Vector4d
 */
...
Eigen::Vector3d prvToEulerAngles321(const Eigen::Vector3d& prv){
    return quaternionToEulerAngles321(prvToQuaternion(prv));
}
...
/**
 * Translate a 321 Euler angle vector into the quaternion (Euler parameter) set.
 * @param euler321
 * @return Eigen::Vector4d
 */
...
Eigen::Vector3d eulerAngles321ToMrp(const Eigen::Vector3d& euler321){
    return quaternionToMrp(eulerAngles321ToQuaternion(euler321));
}
...
Eigen::Vector3d eulerAngles321ToPrv(const Eigen::Vector3d& euler321){
    return quaternionToPrv(eulerAngles321ToQuaternion(euler321));
}

/**
 * Provide the quaternion (Euler parameter) set which corresponds to relative rotation from B2 to B1.
 * @param ep1
 * @param ep2
 * @return Eigen::Vector4d
 */
...
Eigen::Vector3d subMrp(const Eigen::Vector3d& mrp1, const Eigen::Vector3d& mrp2){
    ...
    if (std::abs(denominator) < 0.1) {
        mrp1Shadow = mrpShadowSet(mrp1); // Shadow set
