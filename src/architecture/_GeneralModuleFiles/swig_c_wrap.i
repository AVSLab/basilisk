...
template <typename TConfig,
          ...
          const char* (*moduleDeprecationDateFun)(),
          const char* (*moduleDeprecationMessageFun)(),
...
class RustWrapper : public SysModel {
  public:
    ...
    std::string __bskModuleDeprecationDate() const {
        const char *value = moduleDeprecationDateFun();
        return value == nullptr ? std::string{} : std::string{value};
    }

    std::string __bskModuleDeprecationMessage() const {
        const char *value = moduleDeprecationMessageFun();
        return value == nullptr ? std::string{} : std::string{value};
    }
...
%define %rust_wrap_2(moduleName, configName, handleName)
    ...
    %ignore ModuleDeprecationDate_ ## moduleName;
    %ignore ModuleDeprecationMessage_ ## moduleName;
    ...
    */
    ...
    %pythonappend RustWrapper::RustWrapper %{
        ...
        if removal_date:
            ...
            deprecated.deprecationWarn(
                ...
                removal_date,
                self.__bskModuleDeprecationMessage(),
            )
    %}
    ...
    %template(moduleName) RustWrapper<
        ...
        ModuleDeprecationDate_ ## moduleName,
        ModuleDeprecationMessage_ ## moduleName,
        Destroy_ ## moduleName,
        Update_ ## moduleName,
        SelfInit_ ## moduleName,
        Reset_ ## moduleName>;
