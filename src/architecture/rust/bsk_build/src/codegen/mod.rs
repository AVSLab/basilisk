...
struct MessageCatalog {
    ...
    /// Existing Basilisk C message-port types accepted in the config view.
    c_types: BTreeSet<String>,
}

#[derive(Debug, Default, PartialEq, Eq)]
struct BindingMetadata {
    /// Existing Basilisk C message-port types used by this module.
    used_port_types: BTreeSet<String>,
...
pub fn generate_bindings(config_type: &str) {
    ...
    // cbindgen's stable library API does not expose its resolved type map.
    ...
    let generated_header = finish_header(
        generated_header,
        config_type,
        &module_name,
        &metadata.used_port_types,
    );
...
fn load_message_catalog(c_message_dirs: &[PathBuf]) -> MessageCatalog {
    ...
    message_catalog_from_headers(&headers)
}

fn message_catalog_from_headers(headers: &[PathBuf]) -> MessageCatalog {
    ...
    for header in headers {
        ...
        let message_type = c_type.strip_suffix("_C").unwrap_or_else(|| {
            panic!(
                "bsk-build: generated C message header must end in `_C.h`: {}",
                header.display()
            )
        });
        if message_type.is_empty() {
            panic!(
                "bsk-build: generated C message header has no message name: {}",
                header.display()
            );
        }
        ...
        catalog
            .port_types
            .insert(format!("{READER_PREFIX}{message_type}"), c_type.to_owned());
        catalog
            .port_types
            .insert(format!("{WRITER_PREFIX}{message_type}"), c_type.to_owned());
        catalog.c_types.insert(c_type.to_owned());
    }
    catalog
}
...
fn final_cbindgen_config(
    ...
    for (rust_type, c_type) in &message_catalog.port_types {
    ...
    config.after_includes = Some(MESSAGE_INCLUDE_MARKER.to_owned());
...
fn analyze_bindings(
    header: &str,
    config_type: &str,
    message_c_types: &BTreeSet<String>,
) -> BindingMetadata {
    ...
    for line in config_body.lines() {
        ...
        let canonical_c_type = field
            .c_type
            .strip_prefix("struct ")
            .unwrap_or(&field.c_type);
        if message_c_types.contains(canonical_c_type) {
            metadata.used_port_types.insert(canonical_c_type.to_owned());
            metadata.port_fields.insert(field.name.clone());
            if !field.dimensions.is_empty() {
                metadata.port_arrays.insert(
                    field.name,
                    PortArray {
                        c_type: canonical_c_type.to_owned(),
                    },
                );
            }
            continue;
...
fn config_field_declaration(declaration: &str) -> Option<ConfigField> {
    // Ignore cbindgen's C++-only deleted constructors and any other
    // non-field declarations embedded in the structure body.
...
fn finish_header(
    ...
    let message_includes = used_port_types
        .iter()
        .map(|c_type| format!("#include \"cMsgCInterface/{c_type}.h\""))
        .collect::<Vec<_>>()
        .join("\n");
...
fn render_swig_interface(
    ...
    let wrapper_type = format!(
        "RustWrapper<{config_type}, {config_type}Handle, \
         ...
         ModuleDeprecationDate_{module_name}, \
         ModuleDeprecationMessage_{module_name}, \
...
typedef struct ExampleConfig {
  ...
  double matrix[2][3];
  bool flags[2];
  InputMsg_C inputInMsg;
  InputMsg_C inputInMsgs[2];
  OutputMsg_C outputOutMsg;
  OutputMsg_C outputOutMsgs[2];
} ExampleConfig;
"#;

    fn message_c_types() -> BTreeSet<String> {
        BTreeSet::from(["InputMsg_C".to_owned(), "OutputMsg_C".to_owned()])
    }

    #[test]
    fn builds_port_rename_catalog_from_c_message_headers() {
        let catalog = message_catalog_from_headers(&[
            PathBuf::from("/generated/InputMsg_C.h"),
            PathBuf::from("/generated/OutputMsg_C.h"),
        ]);
        ...
        assert_eq!(
            ...
            BTreeMap::from([
                ...
                ("MsgReader_OutputMsg".to_owned(), "OutputMsg_C".to_owned()),
                ("MsgWriter_InputMsg".to_owned(), "InputMsg_C".to_owned()),
    ...
    fn extracts_only_basilisk_specific_metadata() {
        let metadata = analyze_bindings(FINAL_HEADER, "ExampleConfig", &message_c_types());
        assert_eq!(
            metadata.used_port_types,
            BTreeSet::from(["InputMsg_C".to_owned(), "OutputMsg_C".to_owned()])
        );
    ...
    #[test]
    fn inserts_message_includes_and_lifecycle_inside_header_guard() {
        let header = "#ifndef EXAMPLE_H\n#define EXAMPLE_H\n\
                      /* BSK_RUST_MESSAGE_INCLUDES */\n\
                      typedef struct ExampleConfig {\n\
                      \x20 double gain;\n\
                      } ExampleConfig;\n\
                      #endif /* EXAMPLE_H */\n";
        let header = finish_header(
            header.to_owned(),
            "ExampleConfig",
            "example",
            &message_c_types(),
