...
def test_rigidBodyKinematics(show_plots):
    ...
    v1 = np.array(
        [0.45226701686665, 0.75377836144441, 0.15075567228888, 0.45226701686665]
    )
    v2 = np.array(
        [-0.18663083698528, 0.46657709246321, 0.83983876643378, -0.20529392068381]
    )
    v_true = np.array(
        [-0.46986547690254, -0.34044145332460, 0.71745926113861, 0.38545850500388]
    )
    v = rbk.addEP(v1, v2)
    ...
    C_true = np.array(
        [
            ...
            [-0.16636567534280, 0.96592582628907, 0.0],
            [-0.62088515301485, -0.25881904510252, 0.0],
        ]
    )
    ...
    C_true = np.array(
        [
            [0.73994211169385, 0.25881904510252, 0.0],
            [-0.19826689127415, 0.96592582628907, 0.0],
    ...
    C_true = np.array(
        [
            [0.73994211169385, -0.25881904510252, 0.0],
            [0.64278760968654, 0.0, 1.00000000000000],
            [0.19826689127415, 0.96592582628907, 0.0],
        ]
    )
    ...
    C = rbk.BinvGibbs(v)
    C_true = np.array([[0.64, -0.32, -0.32], [0.32, 0.64, 0.16], [0.32, -0.16, 0.64]])
    e_count += equalCheck(C, C_true, "BinvGibbs")

    v = np.array([0.25, 0.5, -0.5])
    C = rbk.BinvMRP(v)
    C_true = np.array(
        [[0.2304, -0.3072, -0.512], [0.512, 0.384, 0], [0.3072, -0.4096, 0.3840]]
    )
    ...
    C_true = np.array(
        [
            [1.0, 0.0, 0.0],
            [0.0, 0.2674988286245874, 0.963558185417193],
            [0.0, -0.963558185417193, 0.2674988286245874],
        ]
    )
    ...
    assert e_count < 1, (
        str(e_count) + " functions failed in RigidBodyKinematics.py script"
    )
