...
def run(saveFigures, case, show_plots):
    ...
    # Every MonteCarlo simulation must define a function that creates
    # the `SimulationBaseClass` to execute and returns it. Within this
    # function, the simulation is created and configured
    monteCarlo.setSimulationFunction(createScenarioAttitudeFeedbackRW)

    # Also, every MonteCarlo simulation must define a function which
    # executes the simulation that was created.
    monteCarlo.setExecutionFunction(executeScenario)
    ...
    # Statistical dispersions can be applied to initial parameters
    # using the MonteCarlo module
    dispMRPInit = 'TaskList[0].TaskModels[0].hub.sigma_BNInit'
    dispOmegaInit = 'TaskList[0].TaskModels[0].hub.omega_BN_BInit'
    dispMass = 'TaskList[0].TaskModels[0].hub.mHub'
    dispCoMOff = 'TaskList[0].TaskModels[0].hub.r_BcB_B'
    ...
    # A `RetentionPolicy` is used to define what data from the simulation
    # should be retained. A `RetentionPolicy` is a list of messages and
    # variables to log from each simulation run. It also has a callback,
...
def createScenarioAttitudeFeedbackRW():
    ...
    gravBodies = gravFactory.createBodies(['earth', 'mars barycenter', 'sun'])

    # set Earth gravity body as central body
    gravBodies['earth'].isCentralBody = True
    mu = gravBodies['earth'].mu

    # attach gravity model to spaceCraftPlus
    scObject.gravField.gravBodies = spacecraftPlus.GravBodyVector(gravBodies.values())

    spice = spice_interface.SpiceInterface()
    spiceKernelFileNames = ['de430.bsp', 'naif0012.tls', 'de-403-masses.tpc', 'pck00010.tpc']
    spicePlanetNames = ['earth', 'mars barycenter', 'sun']
    spice.ModelTag = "SpiceInterfaceData"
    dataPath = bskPath + "/supportData/EphemerisData/"
    spice.SPICEDataPath = dataPath
    spice.outputBufferCount = 10000
    spice.planetNames = spice_interface.StringVector(spicePlanetNames)
    spice.zeroBase = 'Earth'
    spice.UTCCalInit = "2012 MAY 1 00:28:30.0"
    for fileName in spiceKernelFileNames:
        spice.loadSpiceKernel(fileName, dataPath)
    spice.SPICELoaded = True

    # add spice interface object to task list
    scSim.AddModelToTask(simTaskName, spice, None, -1)
    ...
    scSim.additionalReferences = [rwVoltageIO, fswRWVoltageWrap, scObject, gravBodies, rwMotorTorqueWrap, mrpControlWrap, attErrorWrap, inertial3DWrap]
