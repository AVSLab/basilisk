...
This Python change-only mode is only available for message payload types that support field-wise equality comparison.  Basilisk generates this Python/SWIG support for payloads whose fields can be compared directly.  The generated comparison support is not part of the native C++ messaging API.  C++ code that needs change-only recording must provide a visible ``PayloadEqualityTraits`` specialization only when the payload comparison behavior can be defined safely.  If a Python payload contains an unknown field type, or a pointer field without explicit comparison semantics, then ``recordOnChange()`` emits an error instead of silently falling back to normal interval recording.  Use the regular recorder mode for these payloads.  For example, ``CameraImageMsgPayload`` supports change-only recording through a shallow metadata comparison.  Its image pointer is compared by address, but the pointed-to image buffer is not deep-copied or compared by the recorder.
