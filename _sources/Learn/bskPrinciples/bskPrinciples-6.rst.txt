...
Sometimes it is convenient to record a Basilisk module variable, not just the input or output message.
This can be done via a python command as shown below.  However, note that such module variable recording
will slow down the simulation as this is done in the python layer.  The primary use of this
module variable logging is for debugging and unit tests.

The simulation setup is shown in the figure below.  Both a C and C++ module are created and added to
...
``someVariable`` of module ``someModule`` is set in python using ``someModule.someVariable = ...``
