...
def run(numOrbits):
    ...
    m_prop = 1.0  #  [kg]
    max_thruster_lifetime = 25200  #  [hr] +/= 210 hours for the cathode degradations over time (both filaments) estimated every 100 firings is < 4% of the total lifetime
    ...
    def thrust_time_per_orbit(t_sec, dt, force_mag, T_orbit, thr_thresh=0.0) -> tuple:
        ...
        Returns:
        thrust_time_s   : total thrust-on time (s) in each orbit
        duty_cycle      : thrust_time_s / T_orbit
        orbit_edges_sec : orbit boundary times (s)
        """
