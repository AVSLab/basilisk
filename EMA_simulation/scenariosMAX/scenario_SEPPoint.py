...
class scenario_SEPPoint(BSKSim, BSKScenario):
    def __init__(self, numberSpacecraft, prescribed):
        super(scenario_SEPPoint, self).__init__(numberSpacecraft, fswRate=0.1, dynRate=0.01, envRate=0.01)
    ...
    def log_outputs(self):
        ...
        for sc in range(self.numberSpacecraft):
            ...
            self.inertialBoresightLog.append(DynModels[sc].inertialBoresight.angOutMsg.recorder(self.samplingTime))
            ...
            if self.prescribed:
                ...
                self.platformAngleLogs.append(DynModels[sc].platform.spinningBodyOutMsgs[0].recorder(self.samplingTime))
                self.AddModelToTask(DynModels[sc].taskName, self.platformAngleLogs[0])
                self.platformAngleLogs.append(DynModels[sc].platform.spinningBodyOutMsgs[1].recorder(self.samplingTime))
                self.AddModelToTask(DynModels[sc].taskName, self.platformAngleLogs[1])

                # log platform reference angles
                self.platformRefAngleLogs.append(FswModels[sc].platformRotationData.SpinningBodyRef1OutMsg.recorder(self.samplingTime))
                self.AddModelToTask(DynModels[sc].taskName, self.platformRefAngleLogs[0])
                self.platformRefAngleLogs.append(FswModels[sc].platformRotationData.SpinningBodyRef2OutMsg.recorder(self.samplingTime))
                self.AddModelToTask(DynModels[sc].taskName, self.platformRefAngleLogs[1])

            # log platform torques
            if not self.prescribed:
                for item in range(len(FswModels[sc].platformControllerDataList)):
                    self.platformTorqueLogs.append(FswModels[sc].platformControllerDataList[item].motorTorqueOutMsg.recorder(self.samplingTime))
                    self.AddModelToTask(DynModels[sc].taskName, self.platformTorqueLogs[item])

                # log platform torques combined
                self.platformTorqueJoinedLog.append(FswModels[sc].platformTorqueMsg.recorder(self.samplingTime))
    ...
    def pull_outputs(self, show_plots, spacecraftIndex):
        ...
        if self.prescribed:
            ...
            dataPlatformTorqueSwitch = self.platformTorqueJoinedLog[spacecraftIndex].motorLockFlag

        # very cumbersome routine to compute the offset between thruster and CM direction
        dataThrusterOffset = []
        MB = rbk.MRP2C(FswModels[spacecraftIndex].platformRotationData.sigma_MB)
        r_BM_M = FswModels[spacecraftIndex].platformRotationData.r_BM_M
        r_BM_B = np.matmul(MB.transpose(), r_BM_M)
        r_FM_F = FswModels[spacecraftIndex].platformRotationData.r_FM_F
        r_TF_F = FswModels[spacecraftIndex].platformRotationData.r_TF_F
        T_F = FswModels[spacecraftIndex].platformRotationData.T_F
        ...
        if not self.prescribed:
            plt.plot_platform_torques(timeLineSetMin, dataPlatTorque, dataPlatformTorqueSwitch, dataPlatformReqTorques, 2, 12)
