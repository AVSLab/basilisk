...
class scenario_CruisePoint(BSKSim, BSKScenario):
    def __init__(self, numberSpacecraft, prescribed):
        super(scenario_CruisePoint, self).__init__(numberSpacecraft, fswRate=0.1, dynRate=0.01, envRate=0.01)
        self.name = 'scenario_CruisePoint'
        ...
        for i in range(self.numberSpacecraft):
            ...
            bodyDict = {}
            if prescribed:
                bodyDict[self.DynModels[i].platform.ModelTag] = self.DynModels[
                    i].platform.prescribedMotionConfigLogOutMsg
...
def runScenario(scenario):
    # Configure FSW mode
    scenario.FSWModels[0].modeRequest = "cruisePointing"
...
def run(show_plots, numberSpacecraft, prescribed):
    ...
    TheScenario = scenario_CruisePoint(numberSpacecraft, prescribed)
